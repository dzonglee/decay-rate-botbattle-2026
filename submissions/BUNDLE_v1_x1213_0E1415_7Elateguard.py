...
# ============ ELITE-TALLY MODE SWITCH (Chris's design, 2026-07-15) ============
# Four parameter modes (1E/3E/5E/7E) bundled in one bot. In the GYM these
# overlays are EMPTY -> switching is a no-op and behavior is bit-exact.
...
# mode escalates 1E -> 3E (tally>=3) -> 5E (>=5) -> 7E (>=7). Escalate-only.
...
def _mode_for(tally):
    return 7 if tally >= 7 else 5 if tally >= 5 else 3 if tally >= 3 else 1
...
def _apply_mode(m):
    ...
    try:
        import sys as _s
        print(f"[mode] switched to {m}E (elites seen: {len(_SEEN_ELITES)})", file=_s.stderr, flush=True)
    except Exception:
        pass
...
def _mode_tick(visible_blobs):
    """Per-tick elite tally + mode selection (Chris 2026-07-15 final design):
    START in 0E (pure farm). First elite sighting escalates instantly:
    1 elite -> 1E, tally>=3 -> 3E, >=5 -> 5E, >=7 -> 7E. Escalate-only —
    0E is abandoned permanently on first contact. No-op when ELITE_TEAM_IDS
    is empty (gym) or after cutover (pinned 7E)."""
...
def apply_architecture(game: Game, tracker: Tracker, info: dict,
    ...
    _kills = sum(1 for _e in _new if getattr(_e, "event_type", "") == "event_player_eaten")
                            ...
                            if tracker.tick - _ct <= 10:
                                try:
                                    for _e in game.state.event_history[game.state.new_events:]:
                                        if getattr(_e, "event_type", "") == "event_player_eaten" \

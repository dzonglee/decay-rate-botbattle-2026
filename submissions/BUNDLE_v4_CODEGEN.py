"""CODEGEN build — 195 CONFIG lookups constant-folded, 49 kept dynamic (mode-overlay keys). Behavior-identical; generated by tools/codegen.py."""
...
def compute_forces(game: Game, tracker: Tracker) -> tuple[float, float, dict]:
    ...
    if 0 > 0.5 or 0 > 0.5:
        ...
        for _e in st.event_history[st.new_events:]:
            ...
            if 0 > 0.5:
                if _eater is None and _epos is not None and mass(_erad) >= 2:
                    tracker.kg_pings.append((_rnd, _epos[0], _epos[1], mass(_erad)))
                elif _eater is not None and _eater != me.player_id and 0.5 > 0:
                    tracker.aggro[_eater] = tracker.aggro.get(_eater, 0.0) + 0.05 * 0.5
            if 0 > 0.5 and _victim is not None and _victim != me.player_id:
                tracker.rc_dead[_victim] = _rnd
        if 0 > 0.5 and tracker.kg_pings:
            ...
            for _r0, _px, _py, _vm in tracker.kg_pings:
                ...
                if _d < EPS or _d > 30.7329:
                    continue
                _decay = 1.0 - (_rnd - _r0) / max(1.0, 25)
        ...
        if mass(b.radius) >= mass(my_smallest) * 1.15:
            ...
            if 0 > 0.5:
                ...
                if (_died is not None
                        and 30 <= (getattr(st, "round", 0) or 0) - _died <= 30 + 90

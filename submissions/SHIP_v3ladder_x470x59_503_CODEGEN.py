"""CODEGEN build of ladder genome #503 (x470x59) — constant-folded chassis.
Behavior-identical to SHIP_v3ladder_x470x59_503.py; CONFIG dict retained only for
the graph's dynamic node lookups. Generated 2026-07-12."""
...
CONFIG = {
    ...
    "CYCLE_THREAT_CLEAR": 14,   # only cycle when threats are FAR (avoid deaths-while-fragmented); bounds (2,30)
    ...
    "CORNER_SKIP_ON": 0.393561,        # >0.5 on; bounds (0,1)
    ...
    "ARCH_ON": 1,
    "ARCH_AUTHORITY": 0.6,       # graph share of final steering; evolve in [0.35, 1.0]
    ...
    "ARCH_N0_ON": 1,
    "ARCH_N0_OP": 1,
    "ARCH_N0_A": 13,       # late_phase
    "ARCH_N0_B": 13,        # prey_opportunity
    "ARCH_N0_K": 1.58997,
    "ARCH_N0_BIAS": 0.0698569,
    "ARCH_N0_TARGET": 2,   # prey channel logit
    "ARCH_N0_GAIN": 0.3,

    "ARCH_N1_ON": 1,
    "ARCH_N1_OP": 3,
    "ARCH_N1_A": 0,        # threat_pressure
    "ARCH_N1_B": 15,        # wealth
    "ARCH_N1_K": 3.51518,
    "ARCH_N1_BIAS": 0.15314,
    "ARCH_N1_TARGET": 3,   # escape channel logit
    "ARCH_N1_GAIN": 0.0711527,

    "ARCH_N2_ON": 1,
    "ARCH_N2_OP": 8,
    "ARCH_N2_A": 13,       # feast_ready
    "ARCH_N2_B": 5,        # threat_pressure
    "ARCH_N2_K": 1.29986,
    "ARCH_N2_BIAS": -0.616423,
    "ARCH_N2_TARGET": 7,   # guarded ordinary split vote
    "ARCH_N2_GAIN": 0.573858,
...
def compute_forces(game: Game, tracker: Tracker) -> tuple[float, float, dict]:
    ...
    for b in st.visible_blobs:
        ...
        if mass(b.radius) >= mass(my_smallest) * 1.15:
            ...
            if 0.393561 > 0.5:
                ...
                if wdx < 0.654277 and wdy < 0.654277:
            ...
            if 0.000374918 > 0 and tracker.blob_count.get(b.player_id, 1) >= 5.04291:
                f *= 1.0 + 0.000374918
            fx += ux * f
            fy += uy * f
    ...
    if 0 > 0:
        hunter_near = any(d < 3.50539
                          and mass(t.radius) >= total_mass * 0
                          for t, d in info["threats"])
    else:
        hunter_near = any(d < 3.50539 for _, d in info["threats"])
    ...
    for v in st.visible_viruses:
        ...
        if feast_posture and not hunter_near:
            ...
            fx += ux * f
            fy += uy * f
        elif d < 1.12378 * (0.75 if hunter_near else 1.0):
...
# --- OMNI-2.1 corner refuge: the mirror of the corner fix (gate = weight > 0) ---
    ...
    _camping_now = (0 > 0 and total_mass < 60 and any(
...
def apply_architecture(game: Game, tracker: Tracker, info: dict,
    ...
    active_used = 0
    active_cap = max(0, min(11, int(round(8))))
    ...
    for i in range(11):
        ...
        if not enabled:
            ...
            continue
        active_used += 1
        max_input = 18 + i  # base 0..18 (audit sensors 16-18) plus previous node outputs
    ...
    authority = max(0.35, min(1.0, 0.6))
            ...
            match query:
                case QueryMovePlayer():
                    ...
                    try:
                        ...
                        if False and total_m_ > 1.02335 and info["threats"]:
                            env = []
                        ...
                        if arch_split_bias <= -1.2:
                            ordinary_split = False
                        elif (arch_split_bias >= 1.2
                              and architecture_safe_split(game, info)):
                            ordinary_split = True
                        if arch_cycle_bias <= -1.2:
                            cycle_split = False
                        elif (arch_cycle_bias >= 1.2
                              and architecture_safe_split(game, info)):
                            cycle_split = True

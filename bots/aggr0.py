...
CONFIG = {
    ...
    "THREAT_PANIC_MULT": 0.0186,
    "THREAT_IGNORE_DIST": 26.4814,
    "SPLIT_REACH": 51.3259,
    "W_SPLIT_ZONE": 0.0992,
    "WALL_MARGIN": 0.0052,
    "FRESH_MASS": 1.4497,
    ...
    "VETO_MARGIN": 0.0095,
    "VETO_HORIZON": 3.8834,
    "VETO_SOFT_MASS": 2.1832,
    "LOCK_ENABLED": False,
    "LOCK_MIN_VALUE": 0.0379,
    "LOCK_TICKS_MAX": 43.1233,
    "LOCK_ABANDON_T": 10,
    "W_LOCK": 2.9746,
    "LOCK_THREAT_BREAK": 0.0252,
    "LUNGE_DISC_PANIC": 0.795288,
    "SPLIT_RUN_ENABLED": False,
    "SPLIT_RUN_MAX_MASS": 0.0014,
    "SPLIT_RUN_TRIGGER": 0.0013,
    "EAT_RATIO": 1.2,
    "SAFETY_RATIO": 1.3062,
    "LEAD_TICKS": 1,
    ...
    "HUNTER_AVOID_MULT": 0.8707,     # virus-avoid radius mult when hunter near (was 1.5)
    "HUNTER_REPEL_MULT": 2.3017,     # virus repulsion mult when hunter near (was 2.0)
    "WALL_ESC_MARGIN_MULT": 0.6,  # wall-escape activation margin (was 1.2)
    "WALL_ESC_NUDGE": 0.1496,        # outward nudge strength (was 0.2)
    ...
    "VIRUS_FEAST_FALLOFF": 0.7672,   # feast attraction falloff exponent (was 1/d)
    "CYCLE_TARGET_BLOBS": 4,   # cycle split ceiling (was 16)
    # ORGAN: corner-siege fix (uncatchable corner-tucked prey -> zero force)
    "CORNER_SKIP_ON": 0.0822,        # >0.5 on; bounds (0,1)
    "CORNER_TUCK": 1.6029,
    "CORNER_MARGIN": 0.0941,
    ...
    "W_FRAG_HUNT": 0.1175,           # 0=off; prey-force mult vs players at high blob count
    "FRAG_HUNT_MIN_BLOBS": 3.1986,   # enemy blob count to trigger
    # ORGAN: piece guard (post-consumption regroup surge; heals the loss30 wound)
    "W_PIECE_GUARD": 0.0186,         # 0=off; extra regroup for GUARD_TICKS after our shatter
    "PIECE_GUARD_TICKS": 3.3814,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 0.4146,          # 0=off; regroup when fragmented + no viruses visible
    ...
    "FOOD_HUNGER_EXP": 0.0781,       # 0=off; food force *= (1/total_mass)^exp
    ...
    "ENDGAME_START": 0.4269,         # fraction of EXPECTED_ROUNDS after which endgame fear applies; 1.0=off
    ...
    "W_ENVELOPE_SCALE": 0.0052,      # 0=off; scales threat force by kill-envelope proximity
    # ORGAN: per-player aggression memory (avoid players who hunt US specifically)
    "AGGRO_ON": 0.3965,              # >0.5 on
    ...
    "W_WEALTH_FEAR": 4.0257,         # 0=off; threat force mult grows with own mass above WEALTH_START
    "WEALTH_START": 47.491,         # own total mass where wealth fear begins
    "WEALTH_EXP": 0.3849,            # curve shape
    # ORGAN W2: VIRUS-RESPAWN CAMPING — engine law: virus respawns ~30 rounds after consumption
    "W_CAMP": 0.3527,                # 0=off; attraction to due-respawn sites inside the window
    "CAMP_WINDOW_LO": 11.3668,       # rounds since consumption when attraction starts
    "CAMP_WINDOW_HI": 30,       # ... and ends (site pruned)
    ...
    "W_RANK_GUARD": 0.0395,          # 0=off; threat mult when leading the visible field
    "W_RANK_AGGRO": 0.1007,          # 0=off; prey mult when trailing the visible field
    # ORGAN W5: VIRUS SLOT TIMING — pieces=max(1,17-blobs): saturated consumption is free mass
    "VIRUS_SLOT_EXP": 0.0615,        # 0=off; feast force *= (blobs/16)^exp
    ...
    "PROF_RADIUS": 17.8977,            # elite-proximity radius gating feast boldness
    ...
    "PROF_FEAST_BOLD": 0.3713,         # virus-feast/cycle boldness when no ELITE within PROF_RADIUS
    # ===== OMNI-2.1 corner refuge (gate = weight > 0) =====
    "W_CORNER_REFUGE": 0.3475,         # when hunted by an uncatchable-for-us giant: tuck into ITS unreachable corner
    "AGGRO_DECAY": 0.999,
}

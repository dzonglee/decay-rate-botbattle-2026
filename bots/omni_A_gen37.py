...
CONFIG = {
    ...
    "W_VIRUS_BIG": 43.8511,
    "W_WALL": 0.0287122,
    "W_REGROUP": 1.34981e-05,
    "FOOD_FALLOFF": 1.68366,
    "PREY_FALLOFF": 0.0796328,
    "THREAT_FALLOFF": 0.09903,
    "THREAT_PANIC_DIST": 0.303604,
    "THREAT_PANIC_MULT": 0.0324699,
    "THREAT_IGNORE_DIST": 25.6683,
    "SPLIT_REACH": 51.3259,
    "W_SPLIT_ZONE": 0.0750202,
    "WALL_MARGIN": 0.00541669,
    "FRESH_MASS": 1.38809,
    "FRESH_CAUTION": 0.0451252,
    "VETO_ENABLED": False,
    "VETO_MARGIN": 0.188795,
    "VETO_HORIZON": 3.2438,
    "VETO_SOFT_MASS": 8.64423,
    "LOCK_ENABLED": False,
    "LOCK_MIN_VALUE": 0.341042,
    "LOCK_TICKS_MAX": 50.4153,
    "LOCK_ABANDON_T": 24.6468,
    "W_LOCK": 0.495189,
    "LOCK_THREAT_BREAK": 0.0475001,
    "LUNGE_DISC_PANIC": 0.795288,
    "SPLIT_RUN_ENABLED": False,
    "SPLIT_RUN_MAX_MASS": 0.000658129,
    "SPLIT_RUN_TRIGGER": 0.0167571,
    "EAT_RATIO": 1.2,
    "SAFETY_RATIO": 1.39916,
    "LEAD_TICKS": 1.16491,
    "LUNGE_ALARM_DIST": 0.0140699,
    "W_LUNGE_ALARM": 0.345687,
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 44.3121,
    "SPLIT_SAFETY_RATIO": 0.00361148,
    "SPLIT_MAX_BLOBS": 1.41421,
    "SPLIT_THREAT_CLEARANCE": 0.0431638,
    "SPLIT_MIN_MASS": 2.0,
    "SPLIT_MAX_MASS": 138.719,
    "VIRUS_DANGER_MASS_RATIO": 1.2,
    "VIRUS_AVOID_DIST": 2.62331,
    "VIRUS_FEAST_CLEAR": 5.25372,
    # (BOOT_* and W_VIRUS_FOOD amputated 07-09: dead code paths, wasted mutations)
    "FEAST_MIN_MASS": 45,
    "FEAST_SLOT_SAT": 0.449489,
    "W_VIRUS_FEAST": 0.873745,
    # --- SPLIT-FEAST CYCLE ORGAN (Team-15 virus economy; matches 1719/1721/1723) ---
    "SPLIT_CYCLE_ON": 0.20379,        # vetoable master switch (>0.5 = on); bounds (0,1)
    "CYCLE_MIN_MASS": 15.231,       # only cycle once BIG (don't fragment a small bank); bounds (4,40)
    "CYCLE_THREAT_CLEAR": 7.64648,   # only cycle when threats are FAR (avoid deaths-while-fragmented); bounds (2,30)
    ...
    "HUNTER_REPEL_MULT": 1.14988,     # virus repulsion mult when hunter near (was 2.0)
    "WALL_ESC_MARGIN_MULT": 1.54203,  # wall-escape activation margin (was 1.2)
    "WALL_ESC_NUDGE": 0.148368,        # outward nudge strength (was 0.2)
    "LUNGE_DISC_FRAC": 0.879123,       # deep-disc panic fraction (was 0.9)
    "SPLIT_ALIGN_MIN": 0.35,       # prey-lunge alignment gate (was 0.7)
    "DEATH_COST_SCALE": 20.8153,     # bank-size fear scaling divisor (was 25)
    "VIRUS_FEAST_FALLOFF": 0.723793,   # feast attraction falloff exponent (was 1/d)
    "CYCLE_TARGET_BLOBS": 8.7025,   # cycle split ceiling (was 16)
    # ORGAN: corner-siege fix (uncatchable corner-tucked prey -> zero force)
    "CORNER_SKIP_ON": 0.540475,        # >0.5 on; bounds (0,1)
    "CORNER_TUCK": 0.5,
    "CORNER_MARGIN": 0.155083,
    # ORGAN: sized threat gate (only count watchers that can actually eat us)
    "THREAT_SIZE_GATE": 0.333923,      # 0=all threats count (legacy); >0: hunter_near only if threat mass >= ours*gate
    # ORGAN: virus shield (hide behind viruses; big pursuers pop crossing them)
    "W_VIRUS_SHIELD": 0.0168207,        # 0=off; attraction to viruses when fleeing while small
    "SHIELD_MAX_MASS": 5.078,       # only shield when total below this (we pass under gate)
    # ORGAN: fragment hunting (counter the split-feast meta: eat cyclers mid-cycle)
    "W_FRAG_HUNT": 0.0368548,           # 0=off; prey-force mult vs players at high blob count
    "FRAG_HUNT_MIN_BLOBS": 6.23664,   # enemy blob count to trigger
    # ORGAN: piece guard (post-consumption regroup surge; heals the loss30 wound)
    "W_PIECE_GUARD": 0.00249746,         # 0=off; extra regroup for GUARD_TICKS after our shatter
    "PIECE_GUARD_TICKS": 3.27882,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 0.261839,          # 0=off; regroup when fragmented + no viruses visible
    # ORGAN: map positioning (center vs edge preference; evolution picks sign)
    "W_CENTER": 0.115641,              # 0=off; + seeks center, - seeks edges
    # ORGAN: hunger-scaled food greed (small bots chase pellets harder)
    "FOOD_HUNGER_EXP": 0.262452,       # 0=off; food force *= (1/total_mass)^exp
    # ORGAN: stall kick (anti-deadlock: displacement ~0 while mass decays -> kick)
    "STALL_KICK_ON": 0.227542,         # >0.5 on
    "STALL_TICKS": 33.9163,
    "STALL_DIST": 0.368036,
    "W_STALL_KICK": 1.17094,
    # ORGAN: endgame posture (protect the bank late; final mass IS the score)
    "ENDGAME_START": 0.331443,         # fraction of EXPECTED_ROUNDS after which endgame fear applies; 1.0=off
    "ENDGAME_FEAR_MULT": 1.64531,     # threat-force mult in endgame; 1.0=neutral
    "EXPECTED_ROUNDS": 1400.0,
    # ORGAN: speed-aware threat pricing (slow giants can't catch us outside envelope)
    "W_ENVELOPE_SCALE": 0.18123,      # 0=off; scales threat force by kill-envelope proximity
    # ORGAN: per-player aggression memory (avoid players who hunt US specifically)
    "AGGRO_ON": 0.340553,              # >0.5 on
    "W_AGGRO": 1.7265,               # extra threat weight vs measured aggressors
    "AGGRO_DECAY": 0.9,
}

...
CONFIG = {
    ...
    "W_VIRUS_BIG": 2.05454,
    "W_WALL": 0.00147136,
    "W_REGROUP": 1.33065e-10,
    "FOOD_FALLOFF": 1.40452,
    "PREY_FALLOFF": 0.000474348,
    "THREAT_FALLOFF": 1.27045e-05,
    "THREAT_PANIC_DIST": 1.14918e-06,
    "THREAT_PANIC_MULT": 0.522557,
    "THREAT_IGNORE_DIST": 27.039,
    "SPLIT_REACH": 51.3259,
    "W_SPLIT_ZONE": 0.000826131,
    "WALL_MARGIN": 3.17129e-06,
    "FRESH_MASS": 0.81,
    "FRESH_CAUTION": 0.000229422,
    "VETO_ENABLED": False,
    "VETO_MARGIN": 0.0347238,
    "VETO_HORIZON": 2.03119,
    "VETO_SOFT_MASS": 1.25825,
    "LOCK_ENABLED": False,
    "LOCK_MIN_VALUE": 0.0124582,
    ...
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 23.0926,
    "SPLIT_SAFETY_RATIO": 0.000195,
    "SPLIT_MAX_BLOBS": 1.7911,
    "SPLIT_THREAT_CLEARANCE": 0.00389681,
    "SPLIT_MIN_MASS": 2.0,
    "SPLIT_MAX_MASS": 367.848,
    ...
    "FEAST_MIN_MASS": 6.78538,
    "FEAST_SLOT_SAT": 0.341752,
    "W_VIRUS_FEAST": 2.83074,
    # --- SPLIT-FEAST CYCLE ORGAN (Team-15 virus economy; matches 1719/1721/1723) ---
    "SPLIT_CYCLE_ON": 0.778485,        # vetoable master switch (>0.5 = on); bounds (0,1)
    ...
    "WALL_ESC_MARGIN_MULT": 1.49466,  # wall-escape activation margin (was 1.2)
    ...
    "VIRUS_FEAST_FALLOFF": 0.7167,   # feast attraction falloff exponent (was 1/d)
    "CYCLE_TARGET_BLOBS": 6.0039,   # cycle split ceiling (was 16)
    # ORGAN: corner-siege fix (uncatchable corner-tucked prey -> zero force)
    "CORNER_SKIP_ON": 0.0267842,        # >0.5 on; bounds (0,1)
    "CORNER_TUCK": 1.20645,
    "CORNER_MARGIN": 0.127615,
    # ORGAN: sized threat gate (only count watchers that can actually eat us)
    "THREAT_SIZE_GATE": 0.00023087,      # 0=all threats count (legacy); >0: hunter_near only if threat mass >= ours*gate
    # ORGAN: virus shield (hide behind viruses; big pursuers pop crossing them)
    "W_VIRUS_SHIELD": 0.000223297,        # 0=off; attraction to viruses when fleeing while small
    "SHIELD_MAX_MASS": 3.40171,       # only shield when total below this (we pass under gate)
    ...
    "W_PIECE_GUARD": 0.00205108,         # 0=off; extra regroup for GUARD_TICKS after our shatter
    "PIECE_GUARD_TICKS": 27.3635,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 9.75359e-05,          # 0=off; regroup when fragmented + no viruses visible
    ...
    "FOOD_HUNGER_EXP": 0.000780979,       # 0=off; food force *= (1/total_mass)^exp
    # ORGAN: stall kick (anti-deadlock: displacement ~0 while mass decays -> kick)
    "STALL_KICK_ON": 0.000972507,         # >0.5 on
    "STALL_TICKS": 18.8238,
    "STALL_DIST": 0.338803,
    "W_STALL_KICK": 0.0813418,
    # ORGAN: endgame posture (protect the bank late; final mass IS the score)
    "ENDGAME_START": 0.380087,         # fraction of EXPECTED_ROUNDS after which endgame fear applies; 1.0=off

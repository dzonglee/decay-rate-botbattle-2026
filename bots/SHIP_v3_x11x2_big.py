...
CONFIG = {
    ...
    "FOOD_FALLOFF": 2.5537,
    "PREY_FALLOFF": 0.7647,
    "THREAT_FALLOFF": 0.029,
    "THREAT_PANIC_DIST": 0.8287,
    "THREAT_PANIC_MULT": 0.0616,
    "THREAT_IGNORE_DIST": 34.0449,
    "SPLIT_REACH": 51.3259,
    "W_SPLIT_ZONE": 0.1725,
    "WALL_MARGIN": 0.0292,
    "FRESH_MASS": 1.8518,
    ...
    "VETO_MARGIN": 0.5057,
    "VETO_HORIZON": 4.8383,
    "VETO_SOFT_MASS": 1.3761,
    "LOCK_ENABLED": False,
    "LOCK_MIN_VALUE": 0.4309,
    "LOCK_TICKS_MAX": 20,
    "LOCK_ABANDON_T": 12.7703,
    "W_LOCK": 2.3408,
    "LOCK_THREAT_BREAK": 0.0125,
    "LUNGE_DISC_PANIC": 0.795288,
    "SPLIT_RUN_ENABLED": False,
    "SPLIT_RUN_MAX_MASS": 0.0034,
    "SPLIT_RUN_TRIGGER": 0.0175,
    "EAT_RATIO": 1.2,
    "SAFETY_RATIO": 1.15,
    "LEAD_TICKS": 1,
    "LUNGE_ALARM_DIST": 0.0123,
    "W_LUNGE_ALARM": 0.4513,
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 24.6269,
    "SPLIT_SAFETY_RATIO": 0.0078,
    "SPLIT_MAX_BLOBS": 1.5309,
    "SPLIT_THREAT_CLEARANCE": 0.0433,
    "SPLIT_MIN_MASS": 2.0,
    "SPLIT_MAX_MASS": 106.807,
    "VIRUS_DANGER_MASS_RATIO": 1.2,
    "VIRUS_AVOID_DIST": 2.388,
    "VIRUS_FEAST_CLEAR": 2.1655,
    # (BOOT_* and W_VIRUS_FOOD amputated 07-09: dead code paths, wasted mutations)
    "FEAST_MIN_MASS": 4,
    "FEAST_SLOT_SAT": 0.4381,
    "W_VIRUS_FEAST": 4.3469,
    # --- SPLIT-FEAST CYCLE ORGAN (Team-15 virus economy; matches 1719/1721/1723) ---
    "SPLIT_CYCLE_ON": 0.8379,        # vetoable master switch (>0.5 = on); bounds (0,1)
    ...
    "HUNTER_AVOID_MULT": 1.7997,     # virus-avoid radius mult when hunter near (was 1.5)
    "HUNTER_REPEL_MULT": 1.208,     # virus repulsion mult when hunter near (was 2.0)
    "WALL_ESC_MARGIN_MULT": 1.8,  # wall-escape activation margin (was 1.2)
    "WALL_ESC_NUDGE": 0.2416,        # outward nudge strength (was 0.2)
    "LUNGE_DISC_FRAC": 0.9042,       # deep-disc panic fraction (was 0.9)
    "SPLIT_ALIGN_MIN": 0.35,       # prey-lunge alignment gate (was 0.7)
    "DEATH_COST_SCALE": 16.3421,     # bank-size fear scaling divisor (was 25)
    "VIRUS_FEAST_FALLOFF": 0.5969,   # feast attraction falloff exponent (was 1/d)
    "CYCLE_TARGET_BLOBS": 10.8908,   # cycle split ceiling (was 16)
    # ORGAN: corner-siege fix (uncatchable corner-tucked prey -> zero force)
    "CORNER_SKIP_ON": 0.0851,        # >0.5 on; bounds (0,1)
    "CORNER_TUCK": 1.3094,
    ...
    "W_VIRUS_SHIELD": 0.0149,        # 0=off; attraction to viruses when fleeing while small
    "SHIELD_MAX_MASS": 10.3313,       # only shield when total below this (we pass under gate)
    # ORGAN: fragment hunting (counter the split-feast meta: eat cyclers mid-cycle)
    "W_FRAG_HUNT": 0.3069,           # 0=off; prey-force mult vs players at high blob count
    "FRAG_HUNT_MIN_BLOBS": 5.7331,   # enemy blob count to trigger
    # ORGAN: piece guard (post-consumption regroup surge; heals the loss30 wound)
    "W_PIECE_GUARD": 0.2177,         # 0=off; extra regroup for GUARD_TICKS after our shatter
    "PIECE_GUARD_TICKS": 3.91,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 0.5354,          # 0=off; regroup when fragmented + no viruses visible
    # ORGAN: map positioning (center vs edge preference; evolution picks sign)
    "W_CENTER": 0.1554,              # 0=off; + seeks center, - seeks edges
    # ORGAN: hunger-scaled food greed (small bots chase pellets harder)
    "FOOD_HUNGER_EXP": 0.0763,       # 0=off; food force *= (1/total_mass)^exp
    ...
    "ENDGAME_START": 1,         # fraction of EXPECTED_ROUNDS after which endgame fear applies; 1.0=off
    "ENDGAME_FEAR_MULT": 1.2556,     # threat-force mult in endgame; 1.0=neutral
    "EXPECTED_ROUNDS": 1400.0,
    # ORGAN: speed-aware threat pricing (slow giants can't catch us outside envelope)
    "W_ENVELOPE_SCALE": 0.0128,      # 0=off; scales threat force by kill-envelope proximity
    # ORGAN: per-player aggression memory (avoid players who hunt US specifically)
    "AGGRO_ON": 0.1849,              # >0.5 on
    "W_AGGRO": 1.7163,               # extra threat weight vs measured aggressors
    ...
    "W_WEALTH_FEAR": 0.0444,         # 0=off; threat force mult grows with own mass above WEALTH_START
    "WEALTH_START": 85.4582,         # own total mass where wealth fear begins
    "WEALTH_EXP": 0.3,            # curve shape
    # ORGAN W2: VIRUS-RESPAWN CAMPING — engine law: virus respawns ~30 rounds after consumption
    "W_CAMP": 0.2316,                # 0=off; attraction to due-respawn sites inside the window
    "CAMP_WINDOW_LO": 18.9647,       # rounds since consumption when attraction starts
    "CAMP_WINDOW_HI": 60,       # ... and ends (site pruned)
    "CAMP_MAX_MASS": 42.8176,        # too rich to camp (banking dominates)
    # ORGAN W3: GRUDGE MEMORY — per-opponent caution after they cost us mass
    "W_GRUDGE": 0.0803,              # 0=off; extra threat weight vs players who ate our blobs
    "GRUDGE_DECAY": 0.98,        # per-tick decay
    # ORGAN W4: RANK POSTURE — visible-field standings condition greed/fear
    "W_RANK_GUARD": 0.0451,          # 0=off; threat mult when leading the visible field
    "W_RANK_AGGRO": 0.1539,          # 0=off; prey mult when trailing the visible field
    # ORGAN W5: VIRUS SLOT TIMING — pieces=max(1,17-blobs): saturated consumption is free mass
    "VIRUS_SLOT_EXP": 0.1348,        # 0=off; feast force *= (blobs/16)^exp
    # ===== OMNI-2.1 PROFILER (in-body stupid/mid/elite classifier; Chris's order 2026-07-10) =====
    "PROF_ON": 0.2671,                 # MASTER GATE: <=0.5 -> entire profiler off (zero compute, zero state)
    "PROF_ELITE_T": 0.5,           # EMA score above -> ELITE
    "PROF_STUPID_T": 0.3199,          # EMA score below -> STUPID
    "PROF_RADIUS": 17.3093,            # elite-proximity radius gating feast boldness
    "PROF_PREY_STUPID": 0,        # extra prey force vs STUPID targets
    "PROF_PREY_ELITE_DISC": 0.3383,    # prey force discount vs ELITE targets (0..1)
    "PROF_THREAT_STUPID_DISC": 0.4679, # threat force discount for STUPID giants (0..1)
    "PROF_THREAT_ELITE_MULT": 0.0134,  # extra threat force vs proven-ELITE hunters
    "PROF_FEAST_BOLD": 0.1866,         # virus-feast/cycle boldness when no ELITE within PROF_RADIUS
    # ===== OMNI-2.1 corner refuge (gate = weight > 0) =====
    "W_CORNER_REFUGE": 0.0874,         # when hunted by an uncatchable-for-us giant: tuck into ITS unreachable corner
    "AGGRO_DECAY": 0.999,
}

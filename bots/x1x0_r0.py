...
CONFIG = {
    ...
    "FOOD_FALLOFF": 2.5537,
    "PREY_FALLOFF": 0.7647,
    "THREAT_FALLOFF": 0.0427,
    "THREAT_PANIC_DIST": 0.8544,
    "THREAT_PANIC_MULT": 0.0669,
    ...
    "SPLIT_SAFETY_RATIO": 0.0043,
    "SPLIT_MAX_BLOBS": 1.1018,
    "SPLIT_THREAT_CLEARANCE": 0.0355,
    "SPLIT_MIN_MASS": 2.0,
    ...
    "WALL_ESC_MARGIN_MULT": 1.1974,  # wall-escape activation margin (was 1.2)
    "WALL_ESC_NUDGE": 0.1647,        # outward nudge strength (was 0.2)
    "LUNGE_DISC_FRAC": 0.8668,       # deep-disc panic fraction (was 0.9)
    ...
    "CYCLE_TARGET_BLOBS": 5.7466,   # cycle split ceiling (was 16)
    # ORGAN: corner-siege fix (uncatchable corner-tucked prey -> zero force)
    "CORNER_SKIP_ON": 1,        # >0.5 on; bounds (0,1)
    "CORNER_TUCK": 1.2968,
    "CORNER_MARGIN": 0.3233,
    # ORGAN: sized threat gate (only count watchers that can actually eat us)
    "THREAT_SIZE_GATE": 0.0089,      # 0=all threats count (legacy); >0: hunter_near only if threat mass >= ours*gate
    ...
    "SHIELD_MAX_MASS": 7.0828,       # only shield when total below this (we pass under gate)
    ...
    "PIECE_GUARD_TICKS": 13.7579,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 0.6768,          # 0=off; regroup when fragmented + no viruses visible
    ...
    "ENDGAME_START": 0.796,         # fraction of EXPECTED_ROUNDS after which endgame fear applies; 1.0=off
    ...
    "W_ENVELOPE_SCALE": 0.0043,      # 0=off; scales threat force by kill-envelope proximity
    ...
    "WEALTH_EXP": 0.3989,            # curve shape
    # ORGAN W2: VIRUS-RESPAWN CAMPING — engine law: virus respawns ~30 rounds after consumption
    "W_CAMP": 0.2103,                # 0=off; attraction to due-respawn sites inside the window
    "CAMP_WINDOW_LO": 12.8161,       # rounds since consumption when attraction starts
    ...
    "W_RANK_GUARD": 0.0776,          # 0=off; threat mult when leading the visible field
    ...
    "VIRUS_SLOT_EXP": 0.1964,        # 0=off; feast force *= (blobs/16)^exp
    ...
    "PROF_STUPID_T": 0.287,          # EMA score below -> STUPID
    ...
    "PROF_FEAST_BOLD": 0.005,         # virus-feast/cycle boldness when no ELITE within PROF_RADIUS
    # ===== OMNI-2.1 corner refuge (gate = weight > 0) =====
    "W_CORNER_REFUGE": 0.0936,         # when hunted by an uncatchable-for-us giant: tuck into ITS unreachable corner
    "AGGRO_DECAY": 0.999,
}

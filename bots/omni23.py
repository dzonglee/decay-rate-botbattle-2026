...
CONFIG = {
    ...
    "SAFETY_RATIO": 1.15,
    "LEAD_TICKS": 1.8833,
    "LUNGE_ALARM_DIST": 0.000121754,
    "W_LUNGE_ALARM": 0.0107445,
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 22.3817,
    "SPLIT_SAFETY_RATIO": 4.68492e-05,
    "SPLIT_MAX_BLOBS": 1.06671,
    "SPLIT_THREAT_CLEARANCE": 6.84094e-05,
    "SPLIT_MIN_MASS": 2.0,
    ...
    "CYCLE_THREAT_CLEAR": 2,   # only cycle when threats are FAR (avoid deaths-while-fragmented); bounds (2,30)
    ...
    "AGGRO_DECAY": 0.9,
    # ===== OMNI-2.3 RETENTION STACK (2026-07-11, Chris's order: 3 architectural organs) =====
    # ORGAN R1: CONSOLIDATION — fear anchored to most exposed blob + regroup surge while fragmented
    "CONS_ON": 1,               # MASTER GATE: <=0.5 -> zero compute, identical to base
    "CONS_REGROUP": 0.8,        # regroup force weight while fragmented AND threatened
    # ORGAN R2: WEALTH LOCKDOWN — hard late mode: no splits, guarded feast, bank the lead
    "LKD_ON": 1,                # MASTER GATE
    ...
    "LKD_PREY": 0.5,            # prey force mult in lockdown (skip risky chases)
    # ORGAN R3: SPLIT PROJECTION GATE — veto splits punishable within the merge window
    "SPG_ON": 1,                # MASTER GATE

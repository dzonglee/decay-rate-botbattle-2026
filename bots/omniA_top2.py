...
CONFIG = {
    ...
    "FEAST_SLOT_SAT": 0.66278,
    "W_VIRUS_FEAST": 0.29909,
    # --- SPLIT-FEAST CYCLE ORGAN (Team-15 virus economy; matches 1719/1721/1723) ---
    "SPLIT_CYCLE_ON": 0.5,        # vetoable master switch (>0.5 = on); bounds (0,1)
    "CYCLE_MIN_MASS": 60,       # only cycle once BIG (don't fragment a small bank); bounds (4,40)
    "CYCLE_THREAT_CLEAR": 2.28519,   # only cycle when threats are FAR (avoid deaths-while-fragmented); bounds (2,30)
    ...
    "HUNTER_REPEL_MULT": 1.62584,     # virus repulsion mult when hunter near (was 2.0)
    ...
    "SPLIT_ALIGN_MIN": 0.461151,       # prey-lunge alignment gate (was 0.7)
    ...
    "W_VIRUS_SHIELD": 0.0969073,        # 0=off; attraction to viruses when fleeing while small
    "SHIELD_MAX_MASS": 4.37223,       # only shield when total below this (we pass under gate)
    ...
    "FRAG_HUNT_MIN_BLOBS": 3.30049,   # enemy blob count to trigger
    ...
    "PIECE_GUARD_TICKS": 7.80559,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 0,          # 0=off; regroup when fragmented + no viruses visible
    ...
    "W_ENVELOPE_SCALE": 0.0954199,      # 0=off; scales threat force by kill-envelope proximity
    # ORGAN: per-player aggression memory (avoid players who hunt US specifically)
    "AGGRO_ON": 0.149951,              # >0.5 on
    ...
    "WEALTH_START": 24.4172,         # own total mass where wealth fear begins
    "WEALTH_EXP": 0.30683,            # curve shape
    ...
    "CAMP_WINDOW_LO": 7.99898,       # rounds since consumption when attraction starts
    "CAMP_WINDOW_HI": 60,       # ... and ends (site pruned)
    "CAMP_MAX_MASS": 39.6452,        # too rich to camp (banking dominates)
    ...
    "W_RANK_GUARD": 0.0904259,          # 0=off; threat mult when leading the visible field
    ...
    "PROF_ON": 0.162401,                 # MASTER GATE: <=0.5 -> entire profiler off (zero compute, zero state)
    "PROF_ELITE_T": 0.85,           # EMA score above -> ELITE
    "PROF_STUPID_T": 0.193628,          # EMA score below -> STUPID
    "PROF_RADIUS": 11.348,            # elite-proximity radius gating feast boldness
    ...
    "PROF_THREAT_STUPID_DISC": 0.138304, # threat force discount for STUPID giants (0..1)
    "PROF_THREAT_ELITE_MULT": 0.0792509,  # extra threat force vs proven-ELITE hunters
    "PROF_FEAST_BOLD": 0.12171,         # virus-feast/cycle boldness when no ELITE within PROF_RADIUS

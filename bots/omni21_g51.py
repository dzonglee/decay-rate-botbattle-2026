...
CONFIG = {
    ...
    "W_VIRUS_BIG": 154.507,
    "W_WALL": 0.158627,
    "W_REGROUP": 2.52424e-05,
    "FOOD_FALLOFF": 2.92988,
    "PREY_FALLOFF": 0.817129,
    "THREAT_FALLOFF": 0.00346401,
    "THREAT_PANIC_DIST": 0.669451,
    "THREAT_PANIC_MULT": 0.0157217,
    "THREAT_IGNORE_DIST": 25.8621,
    "SPLIT_REACH": 51.3259,
    "W_SPLIT_ZONE": 0.0586903,
    "WALL_MARGIN": 0.0020022,
    "FRESH_MASS": 0.81,
    "FRESH_CAUTION": 0.0166328,
    "VETO_ENABLED": False,
    "VETO_MARGIN": 0.0238839,
    "VETO_HORIZON": 5.37402,
    "VETO_SOFT_MASS": 1.17953,
    "LOCK_ENABLED": False,
    "LOCK_MIN_VALUE": 0.0515339,
    "LOCK_TICKS_MAX": 31.6171,
    "LOCK_ABANDON_T": 10,
    "W_LOCK": 1.74878,
    "LOCK_THREAT_BREAK": 0.0403404,
    "LUNGE_DISC_PANIC": 0.795288,
    "SPLIT_RUN_ENABLED": False,
    "SPLIT_RUN_MAX_MASS": 0.00130117,
    "SPLIT_RUN_TRIGGER": 0.00122918,
    "EAT_RATIO": 1.2,
    "SAFETY_RATIO": 1.15,
    "LEAD_TICKS": 1,
    "LUNGE_ALARM_DIST": 0.0123591,
    "W_LUNGE_ALARM": 0.453493,
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 18.3256,
    "SPLIT_SAFETY_RATIO": 0.00953539,
    "SPLIT_MAX_BLOBS": 1.01004,
    "SPLIT_THREAT_CLEARANCE": 0.0131927,
    "SPLIT_MIN_MASS": 2.0,
    "SPLIT_MAX_MASS": 28.0865,
    "VIRUS_DANGER_MASS_RATIO": 1.2,
    "VIRUS_AVOID_DIST": 0.554713,
    "VIRUS_FEAST_CLEAR": 8.83304,
    # (BOOT_* and W_VIRUS_FOOD amputated 07-09: dead code paths, wasted mutations)
    "FEAST_MIN_MASS": 4.65106,
    "FEAST_SLOT_SAT": 0.663162,
    "W_VIRUS_FEAST": 0.646096,
    # --- SPLIT-FEAST CYCLE ORGAN (Team-15 virus economy; matches 1719/1721/1723) ---
    "SPLIT_CYCLE_ON": 0.709435,        # vetoable master switch (>0.5 = on); bounds (0,1)
    "CYCLE_MIN_MASS": 40,       # only cycle once BIG (don't fragment a small bank); bounds (4,40)
    "CYCLE_THREAT_CLEAR": 6.11033,   # only cycle when threats are FAR (avoid deaths-while-fragmented); bounds (2,30)
    ...
    "HUNTER_AVOID_MULT": 0.841349,     # virus-avoid radius mult when hunter near (was 1.5)
    "HUNTER_REPEL_MULT": 1.19155,     # virus repulsion mult when hunter near (was 2.0)
    "WALL_ESC_MARGIN_MULT": 0.913556,  # wall-escape activation margin (was 1.2)
    "WALL_ESC_NUDGE": 0.3,        # outward nudge strength (was 0.2)
    "LUNGE_DISC_FRAC": 1.35,       # deep-disc panic fraction (was 0.9)
    "SPLIT_ALIGN_MIN": 0.411322,       # prey-lunge alignment gate (was 0.7)
    "DEATH_COST_SCALE": 12.5,     # bank-size fear scaling divisor (was 25)
    "VIRUS_FEAST_FALLOFF": 0.577396,   # feast attraction falloff exponent (was 1/d)
    "CYCLE_TARGET_BLOBS": 5.66729,   # cycle split ceiling (was 16)
    # ORGAN: corner-siege fix (uncatchable corner-tucked prey -> zero force)
    "CORNER_SKIP_ON": 0.287249,        # >0.5 on; bounds (0,1)
    "CORNER_TUCK": 1.39113,
    "CORNER_MARGIN": 0.109613,
    # ORGAN: sized threat gate (only count watchers that can actually eat us)
    "THREAT_SIZE_GATE": 0.191749,      # 0=all threats count (legacy); >0: hunter_near only if threat mass >= ours*gate
    # ORGAN: virus shield (hide behind viruses; big pursuers pop crossing them)
    "W_VIRUS_SHIELD": 8,        # 0=off; attraction to viruses when fleeing while small
    "SHIELD_MAX_MASS": 4.77518,       # only shield when total below this (we pass under gate)
    # ORGAN: fragment hunting (counter the split-feast meta: eat cyclers mid-cycle)
    "W_FRAG_HUNT": 0.124392,           # 0=off; prey-force mult vs players at high blob count
    "FRAG_HUNT_MIN_BLOBS": 3.7374,   # enemy blob count to trigger
    # ORGAN: piece guard (post-consumption regroup surge; heals the loss30 wound)
    "W_PIECE_GUARD": 0.189611,         # 0=off; extra regroup for GUARD_TICKS after our shatter
    "PIECE_GUARD_TICKS": 3.38142,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 0.116619,          # 0=off; regroup when fragmented + no viruses visible
    # ORGAN: map positioning (center vs edge preference; evolution picks sign)
    "W_CENTER": 0.0423747,              # 0=off; + seeks center, - seeks edges
    # ORGAN: hunger-scaled food greed (small bots chase pellets harder)
    "FOOD_HUNGER_EXP": 0.194387,       # 0=off; food force *= (1/total_mass)^exp
    # ORGAN: stall kick (anti-deadlock: displacement ~0 while mass decays -> kick)
    "STALL_KICK_ON": 0.262259,         # >0.5 on
    "STALL_TICKS": 58.3557,
    "STALL_DIST": 2.52228,
    "W_STALL_KICK": 0.0318942,
    # ORGAN: endgame posture (protect the bank late; final mass IS the score)
    "ENDGAME_START": 0.316374,         # fraction of EXPECTED_ROUNDS after which endgame fear applies; 1.0=off
    "ENDGAME_FEAR_MULT": 0.631401,     # threat-force mult in endgame; 1.0=neutral
    "EXPECTED_ROUNDS": 1400.0,
    # ORGAN: speed-aware threat pricing (slow giants can't catch us outside envelope)
    "W_ENVELOPE_SCALE": 0.0148156,      # 0=off; scales threat force by kill-envelope proximity
    # ORGAN: per-player aggression memory (avoid players who hunt US specifically)
    "AGGRO_ON": 0.342165,              # >0.5 on
    "W_AGGRO": 0.31817,               # extra threat weight vs measured aggressors
    ...
    "W_WEALTH_FEAR": 3.97383,         # 0=off; threat force mult grows with own mass above WEALTH_START
    "WEALTH_START": 31.8434,         # own total mass where wealth fear begins
    "WEALTH_EXP": 0.487068,            # curve shape
    # ORGAN W2: VIRUS-RESPAWN CAMPING — engine law: virus respawns ~30 rounds after consumption
    "W_CAMP": 0.237045,                # 0=off; attraction to due-respawn sites inside the window
    "CAMP_WINDOW_LO": 9.67751,       # rounds since consumption when attraction starts
    "CAMP_WINDOW_HI": 38.331,       # ... and ends (site pruned)
    "CAMP_MAX_MASS": 10,        # too rich to camp (banking dominates)
    # ORGAN W3: GRUDGE MEMORY — per-opponent caution after they cost us mass
    "W_GRUDGE": 0.021348,              # 0=off; extra threat weight vs players who ate our blobs
    "GRUDGE_DECAY": 0.98,        # per-tick decay
    # ORGAN W4: RANK POSTURE — visible-field standings condition greed/fear
    "W_RANK_GUARD": 0.0979861,          # 0=off; threat mult when leading the visible field
    "W_RANK_AGGRO": 0.012047,          # 0=off; prey mult when trailing the visible field
    # ORGAN W5: VIRUS SLOT TIMING — pieces=max(1,17-blobs): saturated consumption is free mass
    "VIRUS_SLOT_EXP": 0.176255,        # 0=off; feast force *= (blobs/16)^exp
    # ===== OMNI-2.1 PROFILER (in-body stupid/mid/elite classifier; Chris's order 2026-07-10) =====
    "PROF_ON": 0.944008,                 # MASTER GATE: <=0.5 -> entire profiler off (zero compute, zero state)
    "PROF_ELITE_T": 0.500121,           # EMA score above -> ELITE
    "PROF_STUPID_T": 0.15,          # EMA score below -> STUPID
    "PROF_RADIUS": 13.1597,            # elite-proximity radius gating feast boldness
    "PROF_PREY_STUPID": 0.0582626,        # extra prey force vs STUPID targets
    "PROF_PREY_ELITE_DISC": 0.250956,    # prey force discount vs ELITE targets (0..1)
    "PROF_THREAT_STUPID_DISC": 0.8, # threat force discount for STUPID giants (0..1)
    "PROF_THREAT_ELITE_MULT": 0.0420987,  # extra threat force vs proven-ELITE hunters
    "PROF_FEAST_BOLD": 0.358909,         # virus-feast/cycle boldness when no ELITE within PROF_RADIUS
    # ===== OMNI-2.1 corner refuge (gate = weight > 0) =====
    "W_CORNER_REFUGE": 0.162709,         # when hunted by an uncatchable-for-us giant: tuck into ITS unreachable corner
    "AGGRO_DECAY": 0.9,
}

...
CONFIG = {
    ...
    "THREAT_PANIC_DIST": 1.0665,
    "THREAT_PANIC_MULT": 0.0669,
    "THREAT_IGNORE_DIST": 32.3924,
    "SPLIT_REACH": 51.3259,
    "W_SPLIT_ZONE": 0.1877,
    "WALL_MARGIN": 0.0165,
    "FRESH_MASS": 0.81,
    "FRESH_CAUTION": 0.0218,
    "VETO_ENABLED": False,
    "VETO_MARGIN": 1.1582,
    "VETO_HORIZON": 9.3177,
    "VETO_SOFT_MASS": 5.1338,
    "LOCK_ENABLED": False,
    "LOCK_MIN_VALUE": 0.3674,
    "LOCK_TICKS_MAX": 134.096,
    "LOCK_ABANDON_T": 16.1095,
    "W_LOCK": 1.7663,
    "LOCK_THREAT_BREAK": 0.0574,
    "LUNGE_DISC_PANIC": 0.795288,
    "SPLIT_RUN_ENABLED": False,
    "SPLIT_RUN_MAX_MASS": 0.0019,
    "SPLIT_RUN_TRIGGER": 0.0069,
    "EAT_RATIO": 1.2,
    "SAFETY_RATIO": 1.15,
    "LEAD_TICKS": 1.1066,
    "LUNGE_ALARM_DIST": 0.0111,
    "W_LUNGE_ALARM": 0.3578,
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 19.4509,
    "SPLIT_SAFETY_RATIO": 0.0079,
    "SPLIT_MAX_BLOBS": 1.0613,
    "SPLIT_THREAT_CLEARANCE": 0.0393,
    "SPLIT_MIN_MASS": 2.0,
    "SPLIT_MAX_MASS": 111.939,
    "VIRUS_DANGER_MASS_RATIO": 1.2,
    "VIRUS_AVOID_DIST": 1.8909,
    "VIRUS_FEAST_CLEAR": 2.3326,
    # (BOOT_* and W_VIRUS_FOOD amputated 07-09: dead code paths, wasted mutations)
    "FEAST_MIN_MASS": 22.6924,
    "FEAST_SLOT_SAT": 0.3,
    ...
    "CYCLE_MIN_MASS": 57.8397,       # only cycle once BIG (don't fragment a small bank); bounds (4,40)
    ...
    "SPLIT_ALIGN_MIN": 0.4781,       # prey-lunge alignment gate (was 0.7)
    "DEATH_COST_SCALE": 19.607,     # bank-size fear scaling divisor (was 25)
    "VIRUS_FEAST_FALLOFF": 0.9058,   # feast attraction falloff exponent (was 1/d)
    "CYCLE_TARGET_BLOBS": 15.5241,   # cycle split ceiling (was 16)
    # ORGAN: corner-siege fix (uncatchable corner-tucked prey -> zero force)
    "CORNER_SKIP_ON": 0.8246,        # >0.5 on; bounds (0,1)
    "CORNER_TUCK": 1.1997,
    "CORNER_MARGIN": 0.3647,
    # ORGAN: sized threat gate (only count watchers that can actually eat us)
    "THREAT_SIZE_GATE": 0.0233,      # 0=all threats count (legacy); >0: hunter_near only if threat mass >= ours*gate
    # ORGAN: virus shield (hide behind viruses; big pursuers pop crossing them)
    "W_VIRUS_SHIELD": 0,        # 0=off; attraction to viruses when fleeing while small
    "SHIELD_MAX_MASS": 7.6752,       # only shield when total below this (we pass under gate)
    # ORGAN: fragment hunting (counter the split-feast meta: eat cyclers mid-cycle)
    "W_FRAG_HUNT": 0.2248,           # 0=off; prey-force mult vs players at high blob count
    ...
    "PIECE_GUARD_TICKS": 19.1429,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 0.5468,          # 0=off; regroup when fragmented + no viruses visible
    ...
    "ENDGAME_START": 0.7436,         # fraction of EXPECTED_ROUNDS after which endgame fear applies; 1.0=off
    "ENDGAME_FEAR_MULT": 1.4104,     # threat-force mult in endgame; 1.0=neutral
    ...
    "CAMP_WINDOW_LO": 20.1118,       # rounds since consumption when attraction starts
    ...
    "PROF_ON": 0.6596,                 # MASTER GATE: <=0.5 -> entire profiler off (zero compute, zero state)
    "PROF_ELITE_T": 0.5,           # EMA score above -> ELITE
    "PROF_STUPID_T": 0.3093,          # EMA score below -> STUPID
    "PROF_RADIUS": 15.1857,            # elite-proximity radius gating feast boldness
    ...
    "PROF_THREAT_ELITE_MULT": 0.0181,  # extra threat force vs proven-ELITE hunters

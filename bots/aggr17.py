...
CONFIG = {
    ...
    "FOOD_FALLOFF": 2.9359,
    "PREY_FALLOFF": 0.8705,
    "THREAT_FALLOFF": 0.0073,
    "THREAT_PANIC_DIST": 2.8868,
    "THREAT_PANIC_MULT": 0.0184,
    "THREAT_IGNORE_DIST": 25.3605,
    "SPLIT_REACH": 51.3259,
    "W_SPLIT_ZONE": 0.1014,
    "WALL_MARGIN": 0.0057,
    "FRESH_MASS": 1.4497,
    ...
    "VETO_MARGIN": 0.0095,
    "VETO_HORIZON": 3.9474,
    "VETO_SOFT_MASS": 2.2291,
    "LOCK_ENABLED": False,
    "LOCK_MIN_VALUE": 0.0479,
    "LOCK_TICKS_MAX": 24.6918,
    "LOCK_ABANDON_T": 11.7195,
    "W_LOCK": 1.8164,
    "LOCK_THREAT_BREAK": 0.0193,
    "LUNGE_DISC_PANIC": 0.795288,
    "SPLIT_RUN_ENABLED": False,
    "SPLIT_RUN_MAX_MASS": 0.0014,
    "SPLIT_RUN_TRIGGER": 0.0012,
    "EAT_RATIO": 1.2,
    "SAFETY_RATIO": 1.6041,
    "LEAD_TICKS": 1,
    "LUNGE_ALARM_DIST": 0.0252,
    "W_LUNGE_ALARM": 0.3517,
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 18.3256,
    "SPLIT_SAFETY_RATIO": 0.0087,
    "SPLIT_MAX_BLOBS": 1.2357,
    "SPLIT_THREAT_CLEARANCE": 0.0281,
    "SPLIT_MIN_MASS": 2.0,
    "SPLIT_MAX_MASS": 20.7673,
    "VIRUS_DANGER_MASS_RATIO": 1.2,
    "VIRUS_AVOID_DIST": 1.0187,
    "VIRUS_FEAST_CLEAR": 11.1359,
    # (BOOT_* and W_VIRUS_FOOD amputated 07-09: dead code paths, wasted mutations)
    "FEAST_MIN_MASS": 6.4649,
    "FEAST_SLOT_SAT": 0.4353,
    "W_VIRUS_FEAST": 1.3903,
    # --- SPLIT-FEAST CYCLE ORGAN (Team-15 virus economy; matches 1719/1721/1723) ---
    "SPLIT_CYCLE_ON": 0.5824,        # vetoable master switch (>0.5 = on); bounds (0,1)
    "CYCLE_MIN_MASS": 51.7129,       # only cycle once BIG (don't fragment a small bank); bounds (4,40)
    "CYCLE_THREAT_CLEAR": 9.4174,   # only cycle when threats are FAR (avoid deaths-while-fragmented); bounds (2,30)
    ...
    "HUNTER_AVOID_MULT": 0.905,     # virus-avoid radius mult when hunter near (was 1.5)
    "HUNTER_REPEL_MULT": 2.3059,     # virus repulsion mult when hunter near (was 2.0)
    "WALL_ESC_MARGIN_MULT": 0.8428,  # wall-escape activation margin (was 1.2)
    "WALL_ESC_NUDGE": 0.1686,        # outward nudge strength (was 0.2)
    "LUNGE_DISC_FRAC": 1.35,       # deep-disc panic fraction (was 0.9)
    "SPLIT_ALIGN_MIN": 0.4097,       # prey-lunge alignment gate (was 0.7)
    "DEATH_COST_SCALE": 12.5,     # bank-size fear scaling divisor (was 25)
    "VIRUS_FEAST_FALLOFF": 0.6038,   # feast attraction falloff exponent (was 1/d)
    "CYCLE_TARGET_BLOBS": 4,   # cycle split ceiling (was 16)
    # ORGAN: corner-siege fix (uncatchable corner-tucked prey -> zero force)
    "CORNER_SKIP_ON": 0.0761,        # >0.5 on; bounds (0,1)
    "CORNER_TUCK": 1.5361,
    "CORNER_MARGIN": 0.065,
    # ORGAN: sized threat gate (only count watchers that can actually eat us)
    "THREAT_SIZE_GATE": 0.3519,      # 0=all threats count (legacy); >0: hunter_near only if threat mass >= ours*gate
    ...
    "SHIELD_MAX_MASS": 2.8528,       # only shield when total below this (we pass under gate)
    # ORGAN: fragment hunting (counter the split-feast meta: eat cyclers mid-cycle)
    "W_FRAG_HUNT": 0.1426,           # 0=off; prey-force mult vs players at high blob count
    "FRAG_HUNT_MIN_BLOBS": 4.1932,   # enemy blob count to trigger
    # ORGAN: piece guard (post-consumption regroup surge; heals the loss30 wound)
    "W_PIECE_GUARD": 0.1389,         # 0=off; extra regroup for GUARD_TICKS after our shatter
    "PIECE_GUARD_TICKS": 3.3814,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 0.2726,          # 0=off; regroup when fragmented + no viruses visible
    # ORGAN: map positioning (center vs edge preference; evolution picks sign)
    "W_CENTER": 0.0696,              # 0=off; + seeks center, - seeks edges
    # ORGAN: hunger-scaled food greed (small bots chase pellets harder)
    "FOOD_HUNGER_EXP": 0.0089,       # 0=off; food force *= (1/total_mass)^exp
    # ORGAN: stall kick (anti-deadlock: displacement ~0 while mass decays -> kick)
    "STALL_KICK_ON": 0.2936,         # >0.5 on
    "STALL_TICKS": 54.5846,
    "STALL_DIST": 3.068,
    "W_STALL_KICK": 0.1928,
    # ORGAN: endgame posture (protect the bank late; final mass IS the score)
    "ENDGAME_START": 0.3591,         # fraction of EXPECTED_ROUNDS after which endgame fear applies; 1.0=off
    "ENDGAME_FEAR_MULT": 0.955,     # threat-force mult in endgame; 1.0=neutral
    "EXPECTED_ROUNDS": 1400.0,
    # ORGAN: speed-aware threat pricing (slow giants can't catch us outside envelope)
    "W_ENVELOPE_SCALE": 0,      # 0=off; scales threat force by kill-envelope proximity
    # ORGAN: per-player aggression memory (avoid players who hunt US specifically)
    "AGGRO_ON": 0.3261,              # >0.5 on
    "W_AGGRO": 1.3461,               # extra threat weight vs measured aggressors
    ...
    "W_WEALTH_FEAR": 4.0657,         # 0=off; threat force mult grows with own mass above WEALTH_START
    "WEALTH_START": 46.8045,         # own total mass where wealth fear begins
    "WEALTH_EXP": 0.3849,            # curve shape
    # ORGAN W2: VIRUS-RESPAWN CAMPING — engine law: virus respawns ~30 rounds after consumption
    "W_CAMP": 0.1828,                # 0=off; attraction to due-respawn sites inside the window
    "CAMP_WINDOW_LO": 11.74,       # rounds since consumption when attraction starts
    "CAMP_WINDOW_HI": 33.0024,       # ... and ends (site pruned)
    "CAMP_MAX_MASS": 40.4077,        # too rich to camp (banking dominates)
    # ORGAN W3: GRUDGE MEMORY — per-opponent caution after they cost us mass
    "W_GRUDGE": 0.0246,              # 0=off; extra threat weight vs players who ate our blobs
    "GRUDGE_DECAY": 0.9995,        # per-tick decay
    # ORGAN W4: RANK POSTURE — visible-field standings condition greed/fear
    "W_RANK_GUARD": 0.0799,          # 0=off; threat mult when leading the visible field
    "W_RANK_AGGRO": 0.0334,          # 0=off; prey mult when trailing the visible field
    # ORGAN W5: VIRUS SLOT TIMING — pieces=max(1,17-blobs): saturated consumption is free mass
    "VIRUS_SLOT_EXP": 0.0747,        # 0=off; feast force *= (blobs/16)^exp
    # ===== OMNI-2.1 PROFILER (in-body stupid/mid/elite classifier; Chris's order 2026-07-10) =====
    "PROF_ON": 0.2367,                 # MASTER GATE: <=0.5 -> entire profiler off (zero compute, zero state)
    "PROF_ELITE_T": 0.5182,           # EMA score above -> ELITE
    "PROF_STUPID_T": 0.2416,          # EMA score below -> STUPID
    "PROF_RADIUS": 19.2734,            # elite-proximity radius gating feast boldness
    "PROF_PREY_STUPID": 0.0889,        # extra prey force vs STUPID targets
    "PROF_PREY_ELITE_DISC": 0.3804,    # prey force discount vs ELITE targets (0..1)
    "PROF_THREAT_STUPID_DISC": 0.7631, # threat force discount for STUPID giants (0..1)
    "PROF_THREAT_ELITE_MULT": 0.0799,  # extra threat force vs proven-ELITE hunters
    "PROF_FEAST_BOLD": 0.4486,         # virus-feast/cycle boldness when no ELITE within PROF_RADIUS
    # ===== OMNI-2.1 corner refuge (gate = weight > 0) =====
    "W_CORNER_REFUGE": 0.333,         # when hunted by an uncatchable-for-us giant: tuck into ITS unreachable corner
    "AGGRO_DECAY": 0.9157,
}

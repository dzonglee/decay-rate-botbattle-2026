...
CONFIG = {
    ...
    "W_VIRUS_BIG": 15.6066,
    "W_WALL": 0.00295725,
    "W_REGROUP": 9.06748e-10,
    "FOOD_FALLOFF": 2.52666,
    "PREY_FALLOFF": 0.000598999,
    "THREAT_FALLOFF": 1.20151e-05,
    "THREAT_PANIC_DIST": 0.000311685,
    "THREAT_PANIC_MULT": 1.75957,
    "THREAT_IGNORE_DIST": 26.2119,
    "SPLIT_REACH": 51.3259,
    "W_SPLIT_ZONE": 0.00330586,
    "WALL_MARGIN": 0.000443885,
    "FRESH_MASS": 1.09288,
    "FRESH_CAUTION": 0.00116838,
    "VETO_ENABLED": False,
    "VETO_MARGIN": 1.6095,
    "VETO_HORIZON": 3.32342,
    "VETO_SOFT_MASS": 1.06713,
    "LOCK_ENABLED": False,
    "LOCK_MIN_VALUE": 0.00303882,
    "LOCK_TICKS_MAX": 29.4247,
    "LOCK_ABANDON_T": 19.3515,
    "W_LOCK": 0.028043,
    "LOCK_THREAT_BREAK": 8.88862e-05,
    "LUNGE_DISC_PANIC": 0.795288,
    "SPLIT_RUN_ENABLED": False,
    "SPLIT_RUN_MAX_MASS": 8.53983e-06,
    "SPLIT_RUN_TRIGGER": 3.45294e-05,
    "EAT_RATIO": 1.2,
    "SAFETY_RATIO": 1.59341,
    "LEAD_TICKS": 1.44393,
    "LUNGE_ALARM_DIST": 0.000484636,
    "W_LUNGE_ALARM": 0.124105,
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 19.7864,
    "SPLIT_SAFETY_RATIO": 9.81352e-05,
    "SPLIT_MAX_BLOBS": 1.30209,
    "SPLIT_THREAT_CLEARANCE": 0.00884017,
    "SPLIT_MIN_MASS": 2.0,
    "SPLIT_MAX_MASS": 375.174,
    "VIRUS_DANGER_MASS_RATIO": 1.2,
    "VIRUS_AVOID_DIST": 1.55923,
    "VIRUS_FEAST_CLEAR": 4.9575,
    # (BOOT_* and W_VIRUS_FOOD amputated 07-09: dead code paths, wasted mutations)
    "FEAST_MIN_MASS": 12.0423,
    "FEAST_SLOT_SAT": 0.950297,
    "W_VIRUS_FEAST": 1.18541,
    # --- SPLIT-FEAST CYCLE ORGAN (Team-15 virus economy; matches 1719/1721/1723) ---
    "SPLIT_CYCLE_ON": 0.609285,        # vetoable master switch (>0.5 = on); bounds (0,1)
    "CYCLE_MIN_MASS": 54.5488,       # only cycle once BIG (don't fragment a small bank); bounds (4,40)
    "CYCLE_THREAT_CLEAR": 2.79739,   # only cycle when threats are FAR (avoid deaths-while-fragmented); bounds (2,30)
    ...
    "HUNTER_AVOID_MULT": 1.27913,     # virus-avoid radius mult when hunter near (was 1.5)
    "HUNTER_REPEL_MULT": 1.21395,     # virus repulsion mult when hunter near (was 2.0)
    "WALL_ESC_MARGIN_MULT": 0.735199,  # wall-escape activation margin (was 1.2)
    "WALL_ESC_NUDGE": 0.1,        # outward nudge strength (was 0.2)
    "LUNGE_DISC_FRAC": 0.633064,       # deep-disc panic fraction (was 0.9)
    "SPLIT_ALIGN_MIN": 0.35,       # prey-lunge alignment gate (was 0.7)
    "DEATH_COST_SCALE": 12.5,     # bank-size fear scaling divisor (was 25)
    "VIRUS_FEAST_FALLOFF": 0.792315,   # feast attraction falloff exponent (was 1/d)
    "CYCLE_TARGET_BLOBS": 5.16166,   # cycle split ceiling (was 16)
    # ORGAN: corner-siege fix (uncatchable corner-tucked prey -> zero force)
    "CORNER_SKIP_ON": 0.0350695,        # >0.5 on; bounds (0,1)
    "CORNER_TUCK": 0.984295,
    "CORNER_MARGIN": 0.0100434,
    # ORGAN: sized threat gate (only count watchers that can actually eat us)
    "THREAT_SIZE_GATE": 0.00418438,      # 0=all threats count (legacy); >0: hunter_near only if threat mass >= ours*gate
    # ORGAN: virus shield (hide behind viruses; big pursuers pop crossing them)
    "W_VIRUS_SHIELD": 8.67543e-05,        # 0=off; attraction to viruses when fleeing while small
    "SHIELD_MAX_MASS": 3.98356,       # only shield when total below this (we pass under gate)
    # ORGAN: fragment hunting (counter the split-feast meta: eat cyclers mid-cycle)
    "W_FRAG_HUNT": 0.000235691,           # 0=off; prey-force mult vs players at high blob count
    "FRAG_HUNT_MIN_BLOBS": 4.20123,   # enemy blob count to trigger
    # ORGAN: piece guard (post-consumption regroup surge; heals the loss30 wound)
    "W_PIECE_GUARD": 0.000329817,         # 0=off; extra regroup for GUARD_TICKS after our shatter
    "PIECE_GUARD_TICKS": 2.90142,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 0.00265673,          # 0=off; regroup when fragmented + no viruses visible
    # ORGAN: map positioning (center vs edge preference; evolution picks sign)
    "W_CENTER": 2.95977e-05,              # 0=off; + seeks center, - seeks edges
    # ORGAN: hunger-scaled food greed (small bots chase pellets harder)
    "FOOD_HUNGER_EXP": 0.0014889,       # 0=off; food force *= (1/total_mass)^exp
    # ORGAN: stall kick (anti-deadlock: displacement ~0 while mass decays -> kick)
    "STALL_KICK_ON": 0.00380919,         # >0.5 on
    "STALL_TICKS": 58.1226,
    "STALL_DIST": 0.340499,
    "W_STALL_KICK": 0.918584,
    # ORGAN: endgame posture (protect the bank late; final mass IS the score)
    "ENDGAME_START": 0.580157,         # fraction of EXPECTED_ROUNDS after which endgame fear applies; 1.0=off
    "ENDGAME_FEAR_MULT": 0.815624,     # threat-force mult in endgame; 1.0=neutral
    "EXPECTED_ROUNDS": 1400.0,
    # ORGAN: speed-aware threat pricing (slow giants can't catch us outside envelope)
    "W_ENVELOPE_SCALE": 0.000192784,      # 0=off; scales threat force by kill-envelope proximity
    # ORGAN: per-player aggression memory (avoid players who hunt US specifically)
    "AGGRO_ON": 0.00632958,              # >0.5 on
    "W_AGGRO": 0.00184073,               # extra threat weight vs measured aggressors
    "AGGRO_DECAY": 0.9,
}

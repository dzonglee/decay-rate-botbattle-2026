...
CONFIG = {
    ...
    "FOOD_FALLOFF": 2.5369,
    "PREY_FALLOFF": 1.0239,
    "THREAT_FALLOFF": 0.0085,
    "THREAT_PANIC_DIST": 1.6835,
    "THREAT_PANIC_MULT": 0.0319,
    "THREAT_IGNORE_DIST": 18.5502,
    "SPLIT_REACH": 51.3259,
    "W_SPLIT_ZONE": 0.1956,
    "WALL_MARGIN": 0.0165,
    "FRESH_MASS": 1.1865,
    "FRESH_CAUTION": 0.0099,
    "VETO_ENABLED": False,
    "VETO_MARGIN": 0.8447,
    "VETO_HORIZON": 4.8383,
    "VETO_SOFT_MASS": 0.9425,
    "LOCK_ENABLED": False,
    "LOCK_MIN_VALUE": 0.3337,
    "LOCK_TICKS_MAX": 20.3268,
    "LOCK_ABANDON_T": 17.9108,
    "W_LOCK": 2.3612,
    "LOCK_THREAT_BREAK": 0.0093,
    "LUNGE_DISC_PANIC": 0.795288,
    "SPLIT_RUN_ENABLED": False,
    "SPLIT_RUN_MAX_MASS": 0.0037,
    "SPLIT_RUN_TRIGGER": 0.0107,
    "EAT_RATIO": 1.2,
    "SAFETY_RATIO": 1.196,
    "LEAD_TICKS": 1.5904,
    "LUNGE_ALARM_DIST": 0.0093,
    "W_LUNGE_ALARM": 0.4183,
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 16.2401,
    "SPLIT_SAFETY_RATIO": 0.0074,
    "SPLIT_MAX_BLOBS": 1.5751,
    "SPLIT_THREAT_CLEARANCE": 0.0419,
    "SPLIT_MIN_MASS": 2.0,
    "SPLIT_MAX_MASS": 101.269,
    "VIRUS_DANGER_MASS_RATIO": 1.2,
    "VIRUS_AVOID_DIST": 2.5281,
    "VIRUS_FEAST_CLEAR": 2.3018,
    # (BOOT_* and W_VIRUS_FOOD amputated 07-09: dead code paths, wasted mutations)
    "FEAST_MIN_MASS": 36.1205,
    "FEAST_SLOT_SAT": 0.3,
    "W_VIRUS_FEAST": 2.0403,
    # --- SPLIT-FEAST CYCLE ORGAN (Team-15 virus economy; matches 1719/1721/1723) ---
    "SPLIT_CYCLE_ON": 0.6584,        # vetoable master switch (>0.5 = on); bounds (0,1)
    "CYCLE_MIN_MASS": 60,       # only cycle once BIG (don't fragment a small bank); bounds (4,40)
    "CYCLE_THREAT_CLEAR": 20.1764,   # only cycle when threats are FAR (avoid deaths-while-fragmented); bounds (2,30)
    ...
    "HUNTER_AVOID_MULT": 2.2171,     # virus-avoid radius mult when hunter near (was 1.5)
    "HUNTER_REPEL_MULT": 1,     # virus repulsion mult when hunter near (was 2.0)
    "WALL_ESC_MARGIN_MULT": 0.7735,  # wall-escape activation margin (was 1.2)
    "WALL_ESC_NUDGE": 0.1673,        # outward nudge strength (was 0.2)
    "LUNGE_DISC_FRAC": 0.9272,       # deep-disc panic fraction (was 0.9)
    "SPLIT_ALIGN_MIN": 0.5553,       # prey-lunge alignment gate (was 0.7)
    ...
    "CYCLE_TARGET_BLOBS": 7.216,   # cycle split ceiling (was 16)
    # ORGAN: corner-siege fix (uncatchable corner-tucked prey -> zero force)
    "CORNER_SKIP_ON": 0.1084,        # >0.5 on; bounds (0,1)
    "CORNER_TUCK": 1.4918,
    "CORNER_MARGIN": 0.3209,
    # ORGAN: sized threat gate (only count watchers that can actually eat us)
    "THREAT_SIZE_GATE": 0.0908,      # 0=all threats count (legacy); >0: hunter_near only if threat mass >= ours*gate
    # ORGAN: virus shield (hide behind viruses; big pursuers pop crossing them)
    "W_VIRUS_SHIELD": 0.1245,        # 0=off; attraction to viruses when fleeing while small
    "SHIELD_MAX_MASS": 7.7812,       # only shield when total below this (we pass under gate)
    # ORGAN: fragment hunting (counter the split-feast meta: eat cyclers mid-cycle)
    "W_FRAG_HUNT": 0.1693,           # 0=off; prey-force mult vs players at high blob count
    "FRAG_HUNT_MIN_BLOBS": 3.4662,   # enemy blob count to trigger
    # ORGAN: piece guard (post-consumption regroup surge; heals the loss30 wound)
    "W_PIECE_GUARD": 0.1543,         # 0=off; extra regroup for GUARD_TICKS after our shatter
    "PIECE_GUARD_TICKS": 14.5609,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 0.3471,          # 0=off; regroup when fragmented + no viruses visible
    # ORGAN: map positioning (center vs edge preference; evolution picks sign)
    "W_CENTER": 0.097,              # 0=off; + seeks center, - seeks edges
    # ORGAN: hunger-scaled food greed (small bots chase pellets harder)
    "FOOD_HUNGER_EXP": 0.1614,       # 0=off; food force *= (1/total_mass)^exp
    ...
    "ENDGAME_START": 0.5803,         # fraction of EXPECTED_ROUNDS after which endgame fear applies; 1.0=off
    "ENDGAME_FEAR_MULT": 1.4802,     # threat-force mult in endgame; 1.0=neutral
    "EXPECTED_ROUNDS": 1400.0,
    # ORGAN: speed-aware threat pricing (slow giants can't catch us outside envelope)
    "W_ENVELOPE_SCALE": 0.3525,      # 0=off; scales threat force by kill-envelope proximity
    # ORGAN: per-player aggression memory (avoid players who hunt US specifically)
    "AGGRO_ON": 0.2033,              # >0.5 on
    "W_AGGRO": 0.3477,               # extra threat weight vs measured aggressors
    ...
    "CAMP_WINDOW_LO": 14.6311,       # rounds since consumption when attraction starts
    "CAMP_WINDOW_HI": 39.0409,       # ... and ends (site pruned)
    ...
    "W_GRUDGE": 0.0386,              # 0=off; extra threat weight vs players who ate our blobs
    "GRUDGE_DECAY": 0.98,        # per-tick decay
    # ORGAN W4: RANK POSTURE — visible-field standings condition greed/fear
    "W_RANK_GUARD": 2.0239,          # 0=off; threat mult when leading the visible field
    "W_RANK_AGGRO": 0.1115,          # 0=off; prey mult when trailing the visible field
    # ORGAN W5: VIRUS SLOT TIMING — pieces=max(1,17-blobs): saturated consumption is free mass
    "VIRUS_SLOT_EXP": 0.0686,        # 0=off; feast force *= (blobs/16)^exp
    # ===== OMNI-2.1 PROFILER (in-body stupid/mid/elite classifier; Chris's order 2026-07-10) =====
    "PROF_ON": 0.6783,                 # MASTER GATE: <=0.5 -> entire profiler off (zero compute, zero state)
    "PROF_ELITE_T": 0.6516,           # EMA score above -> ELITE
    "PROF_STUPID_T": 0.4211,          # EMA score below -> STUPID
    "PROF_RADIUS": 9.253,            # elite-proximity radius gating feast boldness
    "PROF_PREY_STUPID": 0.0889,        # extra prey force vs STUPID targets
    "PROF_PREY_ELITE_DISC": 0.2923,    # prey force discount vs ELITE targets (0..1)
    "PROF_THREAT_STUPID_DISC": 0.3761, # threat force discount for STUPID giants (0..1)
    "PROF_THREAT_ELITE_MULT": 0.0726,  # extra threat force vs proven-ELITE hunters
    "PROF_FEAST_BOLD": 0.0366,         # virus-feast/cycle boldness when no ELITE within PROF_RADIUS
    # ===== OMNI-2.1 corner refuge (gate = weight > 0) =====
    "W_CORNER_REFUGE": 0.6219,         # when hunted by an uncatchable-for-us giant: tuck into ITS unreachable corner
    "AGGRO_DECAY": 0.9,
}

...
CONFIG = {
    ...
    "W_REGROUP": 1.252e-05,
    "FOOD_FALLOFF": 3.0251,
    "PREY_FALLOFF": 0.548549,
    "THREAT_FALLOFF": 0.0135819,
    "THREAT_PANIC_DIST": 1.23759,
    "THREAT_PANIC_MULT": 0.131719,
    "THREAT_IGNORE_DIST": 41.1835,
    "SPLIT_REACH": 51.3259,
    "W_SPLIT_ZONE": 0.0702142,
    "WALL_MARGIN": 0.00639213,
    "FRESH_MASS": 1.23977,
    "FRESH_CAUTION": 0.0157667,
    "VETO_ENABLED": False,
    "VETO_MARGIN": 0.110064,
    "VETO_HORIZON": 13.1954,
    "VETO_SOFT_MASS": 0.81,
    "LOCK_ENABLED": False,
    "LOCK_MIN_VALUE": 0.142099,
    "LOCK_TICKS_MAX": 34.963,
    "LOCK_ABANDON_T": 10,
    "W_LOCK": 0.673986,
    "LOCK_THREAT_BREAK": 0.065843,
    "LUNGE_DISC_PANIC": 0.795288,
    "SPLIT_RUN_ENABLED": False,
    "SPLIT_RUN_MAX_MASS": 1.25927e-05,
    "SPLIT_RUN_TRIGGER": 0.00717607,
    "EAT_RATIO": 1.2,
    "SAFETY_RATIO": 1.15,
    "LEAD_TICKS": 1,
    "LUNGE_ALARM_DIST": 0.0427701,
    "W_LUNGE_ALARM": 0.10548,
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 59.9065,
    "SPLIT_SAFETY_RATIO": 0.00360955,
    "SPLIT_MAX_BLOBS": 1.34828,
    "SPLIT_THREAT_CLEARANCE": 0.025387,
    "SPLIT_MIN_MASS": 2.0,
    "SPLIT_MAX_MASS": 123.434,
    "VIRUS_DANGER_MASS_RATIO": 1.2,
    "VIRUS_AVOID_DIST": 0.339937,
    "VIRUS_FEAST_CLEAR": 2.57249,
    # (BOOT_* and W_VIRUS_FOOD amputated 07-09: dead code paths, wasted mutations)
    "FEAST_MIN_MASS": 12.6374,
    "FEAST_SLOT_SAT": 0.559328,
    "W_VIRUS_FEAST": 1.1058,
    # --- SPLIT-FEAST CYCLE ORGAN (Team-15 virus economy; matches 1719/1721/1723) ---
    "SPLIT_CYCLE_ON": 0.0211666,        # vetoable master switch (>0.5 = on); bounds (0,1)
    "CYCLE_MIN_MASS": 4,       # only cycle once BIG (don't fragment a small bank); bounds (4,40)
    "CYCLE_THREAT_CLEAR": 5.81187,   # only cycle when threats are FAR (avoid deaths-while-fragmented); bounds (2,30)
    ...
    "HUNTER_AVOID_MULT": 1.06533,     # virus-avoid radius mult when hunter near (was 1.5)
    "HUNTER_REPEL_MULT": 1.22258,     # virus repulsion mult when hunter near (was 2.0)
    "WALL_ESC_MARGIN_MULT": 0.703554,  # wall-escape activation margin (was 1.2)
    "WALL_ESC_NUDGE": 0.142548,        # outward nudge strength (was 0.2)
    "LUNGE_DISC_FRAC": 0.599627,       # deep-disc panic fraction (was 0.9)
    "SPLIT_ALIGN_MIN": 0.661009,       # prey-lunge alignment gate (was 0.7)
    "DEATH_COST_SCALE": 15.6299,     # bank-size fear scaling divisor (was 25)
    "VIRUS_FEAST_FALLOFF": 0.939575,   # feast attraction falloff exponent (was 1/d)
    "CYCLE_TARGET_BLOBS": 11.866,   # cycle split ceiling (was 16)
    # ORGAN: corner-siege fix (uncatchable corner-tucked prey -> zero force)
    "CORNER_SKIP_ON": 0.205963,        # >0.5 on; bounds (0,1)
    "CORNER_TUCK": 2.51639,
    "CORNER_MARGIN": 0.158336,
    # ORGAN: sized threat gate (only count watchers that can actually eat us)
    "THREAT_SIZE_GATE": 0.123584,      # 0=all threats count (legacy); >0: hunter_near only if threat mass >= ours*gate
    # ORGAN: virus shield (hide behind viruses; big pursuers pop crossing them)
    "W_VIRUS_SHIELD": 0.019418,        # 0=off; attraction to viruses when fleeing while small
    "SHIELD_MAX_MASS": 19.8257,       # only shield when total below this (we pass under gate)
    # ORGAN: fragment hunting (counter the split-feast meta: eat cyclers mid-cycle)
    "W_FRAG_HUNT": 0.330391,           # 0=off; prey-force mult vs players at high blob count
    "FRAG_HUNT_MIN_BLOBS": 3.1523,   # enemy blob count to trigger
    # ORGAN: piece guard (post-consumption regroup surge; heals the loss30 wound)
    "W_PIECE_GUARD": 1.53322,         # 0=off; extra regroup for GUARD_TICKS after our shatter
    "PIECE_GUARD_TICKS": 13.2896,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 0.000128607,          # 0=off; regroup when fragmented + no viruses visible
    # ORGAN: map positioning (center vs edge preference; evolution picks sign)
    "W_CENTER": 0.00115898,              # 0=off; + seeks center, - seeks edges
    # ORGAN: hunger-scaled food greed (small bots chase pellets harder)
    "FOOD_HUNGER_EXP": 0.0290761,       # 0=off; food force *= (1/total_mass)^exp
    # ORGAN: stall kick (anti-deadlock: displacement ~0 while mass decays -> kick)
    "STALL_KICK_ON": 1,         # >0.5 on
    "STALL_TICKS": 12.6913,
    "STALL_DIST": 0.330798,
    "W_STALL_KICK": 5.35136,
    # ORGAN: endgame posture (protect the bank late; final mass IS the score)
    "ENDGAME_START": 0.338808,         # fraction of EXPECTED_ROUNDS after which endgame fear applies; 1.0=off
    "ENDGAME_FEAR_MULT": 0.627891,     # threat-force mult in endgame; 1.0=neutral
    "EXPECTED_ROUNDS": 1400.0,
    # ORGAN: speed-aware threat pricing (slow giants can't catch us outside envelope)
    "W_ENVELOPE_SCALE": 0.0446458,      # 0=off; scales threat force by kill-envelope proximity
    # ORGAN: per-player aggression memory (avoid players who hunt US specifically)
    "AGGRO_ON": 0.00380211,              # >0.5 on
    "W_AGGRO": 0.0468611,               # extra threat weight vs measured aggressors
    "AGGRO_DECAY": 0.999,
}

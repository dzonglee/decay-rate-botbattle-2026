"""
SYNCS Bot Battle 2026 — potential-field bot. ALGORITHM v0.1

v0.1 (2026-07-06) = the six-times-defended v0 baseline plus two structural
additions, each CONFIG-gated and individually testable:

  * ENDGAME RANK PROTECTION — in the final ENDGAME_ROUNDS, if our largest
    blob out-radiuses every visible enemy blob, stop gambling: prey pull is
    scaled by ENDGAME_PREY_MULT, threats are respected ENDGAME_IGNORE_MULT
    further out, and splitting is disabled. The drafted scoring pays for
    consistent high finishes; a failed fight at the buzzer can't be re-farmed.
  * EMA VELOCITY SMOOTHING — the tracker blends each frame's position delta
    into an exponential moving average (VEL_EMA_ALPHA) instead of using raw
    single-frame deltas. Motivated by the intercept variant's 26-4 loss:
    prediction geometry was fine, jittery velocity estimates were not.
    (VEL_EMA_ALPHA=1.0 restores v0 behaviour exactly.)

Architecture (single file, as required for submission):
  1. CONFIG        — every tunable weight lives here. Tune nothing else.
  2. Tracker       — remembers enemy blob positions across ticks -> smoothed
                     velocity estimates for predictive interception/fleeing.
...
Engine facts baked in (verified in agario-public src, v2026.1.7):
  arena 60x60, 8 players, 1400 rounds @ 0.1s, eat ratio 1.2x (by radius,
  centre inside eater), speed = max(0.25, 1.1/(1+0.08r)) — hyperbolic,
  mass decay 0.2%/tick, mass ~ radius^2, split needs mass >= 2.0 and splits
  EVERY qualifying blob (18-tick merge cooldown, no split cooldown),
...
CONFIG = {
    ...
    "VEL_EMA_ALPHA": 1.0,    # velocity EMA weight for newest frame delta (1.0 = raw)
    ...
    # --- endgame rank protection ---
    "ENDGAME_ROUNDS": 150,      # final N rounds: protect rank, stop gambling (0 = off)
    "ENDGAME_PREY_MULT": 0.3,   # prey attraction multiplier when leading late
    "ENDGAME_IGNORE_MULT": 1.5, # respect threats this much further when leading late
}
...
def endgame_leading(st) -> bool:
    """Final rounds AND our largest blob out-radiuses every visible enemy blob
    (we can't see the leaderboard; visible sizes are the only proxy)."""
    if st.max_rounds <= 0 or st.round < st.max_rounds - CONFIG["ENDGAME_ROUNDS"]:
        return False
    my_largest = max((b.radius for b in st.me.blobs.values()), default=st.me.radius)
    for b in st.visible_blobs:
        if b.player_id != st.me.player_id and b.radius >= my_largest:
            return False
    return True


# ----------------------------------------------------------------------------
# 2. Velocity tracker for enemy blobs (EMA-smoothed).
# ----------------------------------------------------------------------------
class Tracker:
    ...
    def update(self, blobs, my_player_id: int) -> None:
        ...
        for b in blobs:
            ...
            if key in self._last:
                ...
                dx, dy = b.pos[0] - lx, b.pos[1] - ly
                # EMA: raw single-frame deltas are noisy (sank the intercept
                # variant); blend with history so predictions don't jitter
                pvx, pvy = self.velocity.get(key, (dx, dy))
                alpha = CONFIG["VEL_EMA_ALPHA"]
                self.velocity[key] = (
                    pvx * (1 - alpha) + dx * alpha,
                    pvy * (1 - alpha) + dy * alpha,
                )
...
def compute_forces(game: Game, tracker: Tracker) -> tuple[float, float, dict]:
    ...
    # endgame: protect the lead instead of growing it
    protect = endgame_leading(st)
    prey_w = CONFIG["W_PREY"] * (CONFIG["ENDGAME_PREY_MULT"] if protect else 1.0)
    ignore_d = CONFIG["THREAT_IGNORE_DIST"] * (
        CONFIG["ENDGAME_IGNORE_MULT"] if protect else 1.0
    )

    fx, fy = 0.0, 0.0
    info = {"threats": [], "prey": [], "protect": protect}
    ...
    for b in st.visible_blobs:
        ...
        if b.radius >= my_smallest * CONFIG["SAFETY_RATIO"]:
            ...
            f = prey_w * mass(b.radius) / (d ** CONFIG["PREY_FALLOFF"] + EPS)
            fx += ux * f
            fy += uy * f
...
def should_split(game: Game, info: dict, fx: float, fy: float) -> bool:
    if not CONFIG["SPLIT_ENABLED"]:
        return False
    if info.get("protect"):
        return False  # endgame while leading: a lunge can only lose rank
    me = game.state.me
    my_blobs = list(me.blobs.values())

...
and position) and combined weighted by blob mass. The engine applies one
direction to all blobs, so the output is still a single vector — but a giant
that is prey to our big blob no longer attracts our small blob to its death,
and food/wall distances are no longer distorted by the centroid.
...
def compute_forces(game: Game, tracker: Tracker) -> tuple[float, float, dict]:
    ...
    for myb in my_blobs:
        ...
        gx, gy = 0.0, 0.0  # this blob's own force sum

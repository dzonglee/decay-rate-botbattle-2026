"""
SYNCS Bot Battle 2026 — potential-field bot, ENDGAME variant.

Same as my_bot.py plus endgame awareness: in the final ENDGAME_ROUNDS of the
match (game.state.round vs max_rounds), if we appear to be leading (our
largest blob out-radiuses every visible enemy blob), we stop gambling —
prey attraction is scaled down, threats are respected from further away,
and splitting is disabled outright. Rationale: a failed fight in the last
~15 seconds can't be re-farmed before the clock ends, and the drafted
scoring pays for consistent high finishes, not for extra mass.

Engine facts baked in (verified in agario-public src, v2026.1.7): see README.
"""
...
CONFIG = {
    ...
    # --- endgame ---
    "ENDGAME_ROUNDS": 150,      # final N rounds: protect rank, stop gambling

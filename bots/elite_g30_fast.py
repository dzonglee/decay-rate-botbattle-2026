...
def compute_forces(game: Game, tracker: Tracker) -> tuple[float, float, dict]:
    ...
    # elite_g30_fast: cap threat/prey sets to the 12 nearest -> bounds the
    # veto sweep O(16*threats) and split loops in dense late-game states
    # (far blobs cannot reach us this tick, so pruning is behaviour-neutral).
    if len(info["threats"]) > 12:
        info["threats"] = sorted(info["threats"], key=lambda x: x[1])[:12]
    if len(info["prey"]) > 12:
        info["prey"] = sorted(info["prey"], key=lambda x: x[1])[:12]

...
CONFIG = {
    ...
    "W_VIRUS_BIG": 174.947,
    "W_WALL": 0.131824,
    "W_REGROUP": 0.00150078,
    "FOOD_FALLOFF": 1.12484,
    "PREY_FALLOFF": 0.0178422,
    "THREAT_FALLOFF": 0.224461,
    "THREAT_PANIC_DIST": 0.00572088,
    "THREAT_PANIC_MULT": 0.252523,
    "THREAT_IGNORE_DIST": 7.83458,
    "SPLIT_REACH": 51.3259,
    "W_SPLIT_ZONE": 0.553852,
    "WALL_MARGIN": 6.63471,
    "FRESH_MASS": 0.83749,
    "FRESH_CAUTION": 0.947867,
    "VETO_ENABLED": False,
    "VETO_MARGIN": 0.919379,
    "VETO_HORIZON": 4.96298,
    "VETO_SOFT_MASS": 5.27534,
    "LOCK_ENABLED": False,
    "LOCK_MIN_VALUE": 0.0967597,
    "LOCK_TICKS_MAX": 300,
    "LOCK_ABANDON_T": 80.4652,
    "W_LOCK": 10.9406,
    "LOCK_THREAT_BREAK": 0.464595,
    "LUNGE_DISC_PANIC": 0.795288,
    "SPLIT_RUN_ENABLED": False,
    "SPLIT_RUN_MAX_MASS": 0.0257744,
    "SPLIT_RUN_TRIGGER": 0.429301,
    "EAT_RATIO": 1.2,
    "SAFETY_RATIO": 1.18,
    "LEAD_TICKS": 1.47727,
    "LUNGE_ALARM_DIST": 0.0327687,
    "W_LUNGE_ALARM": 1.83886,
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 38.665,
    "SPLIT_SAFETY_RATIO": 0.0106403,
    "SPLIT_MAX_BLOBS": 1.46474,
    "SPLIT_THREAT_CLEARANCE": 1.61367,
    "SPLIT_MIN_MASS": 2.0,
    "SPLIT_MAX_MASS": 78.5677,
    "VIRUS_DANGER_MASS_RATIO": 1.2,
    "VIRUS_AVOID_DIST": 7.4658,
    "W_VIRUS_FOOD": 0.0352581,
    "VIRUS_FEAST_CLEAR": 3.92503,
    "BOOT_MASS": 0.250671,
...
def compute_forces(game: Game, tracker: Tracker) -> tuple[float, float, dict]:
    ...
    for food in st.visible_food:
        ...
        if d < EPS:
            continue
        f = (CONFIG["W_FOOD"] * (CONFIG["OPEN_GREED"] if opening else 1.0)) / (d ** CONFIG["FOOD_FALLOFF"] + EPS)
        fx += ux * f
        fy += uy * f
    ...
    for b in st.visible_blobs:
        ...
        if b.radius >= my_smallest * CONFIG["SAFETY_RATIO"]:
            ...
            f = caution * (CONFIG["OPEN_FEAR"] if opening else 1.0) * CONFIG["W_THREAT"] * mass(b.radius) / (d ** CONFIG["THREAT_FALLOFF"] + EPS)
            if d < CONFIG["THREAT_PANIC_DIST"]:
                f *= CONFIG["THREAT_PANIC_MULT"]
            fx -= ux * f
            fy -= uy * f
        elif my_largest >= b.radius * CONFIG["EAT_RATIO"]:
            edge = my_largest >= b.radius * CONFIG["EDGE_HUNT_RATIO"]

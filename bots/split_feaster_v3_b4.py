"""
split_feaster_v3_b4 - v3 + CYCLE_TARGET_BLOBS=4 (cycle splits to 4 blobs not 16). TWO-PHASE (accumulate then feast). Below PHASE_MASS=45: food x3,
prey pursuit x0.5 (local only; lunge kept - v2b proved it feeds growth), threat caution
x1.6, cycle LOCKED. At >=45: multipliers snap to 1.0 -> exact split_feaster (proven cycle).
One phase boundary, existing force genes.
...
CONFIG = {
    ...
    "W_VIRUS_FOOD": 0.000469358,
    "VIRUS_FEAST_CLEAR": 2,
    "BOOT_MASS": 1.88599,
    "BOOT_VIRUS_MULT": 0.787118,
    "BOOT_VIRUS_CLEAR": 4.32462,
    "FEAST_MIN_MASS": 45,
    "FEAST_SLOT_SAT": 0.3,
    ...
    "CYCLE_MIN_MASS": 45.0,       # v3: cycle LOCKED below the phase boundary
    "CYCLE_THREAT_CLEAR": 28.0,   # only cycle when threats are FAR (avoid deaths-while-fragmented); bounds (2,30)
    # --- v3 ACCUMULATION PHASE (total_mass < PHASE_MASS) ---
    "PHASE_MASS": 45.0,           # boundary: below = accumulate safely, at/above = exact split_feaster
    "ACCUM_FOOD_MULT": 3.0,       # food attraction amplified when small
    "ACCUM_PREY_MULT": 0.5,       # prey pursuit local-only (don't chase across map; lunge still fires close)
    "ACCUM_THREAT_MULT": 1.6,     # threat caution moderately up when small
}
...
def compute_forces(game: Game, tracker: Tracker) -> tuple[float, float, dict]:
    ...
    accum = total_mass < CONFIG["PHASE_MASS"]   # v3: accumulation phase
    ...
    for food in st.visible_food:
        ...
        if d < EPS:
            continue
        f = CONFIG["W_FOOD"] * (CONFIG["ACCUM_FOOD_MULT"] if accum else 1.0) / (d ** CONFIG["FOOD_FALLOFF"] + EPS)
        fx += ux * f
        fy += uy * f
    ...
    for b in st.visible_blobs:
        ...
        if mass(b.radius) >= mass(my_smallest) * CONFIG["SAFETY_RATIO"]:
            ...
            f = (CONFIG["ACCUM_THREAT_MULT"] if accum else 1.0) * caution * CONFIG["W_THREAT"] * mass(b.radius) / (d ** CONFIG["THREAT_FALLOFF"] + EPS)
            if d < CONFIG["THREAT_PANIC_DIST"]:
                f *= CONFIG["THREAT_PANIC_MULT"]
            fx -= ux * f
            fy -= uy * f
        elif mass(my_largest) >= mass(b.radius) * CONFIG["EAT_RATIO"]:
            f = (CONFIG["ACCUM_PREY_MULT"] if accum else 1.0) * (CONFIG["W_PREY"] / caution) * mass(b.radius) / (d ** CONFIG["PREY_FALLOFF"] + EPS)
            fx += ux * f
            fy += uy * f
    ...
    for v in st.visible_viruses:
        ...
        if feast_posture and not hunter_near:
            f = CONFIG["W_VIRUS_FEAST"] / (d + EPS)
            fx += ux * f
            fy += uy * f
        elif d < CONFIG["VIRUS_AVOID_DIST"] * (1.5 if hunter_near else 1.0):
...
def should_cycle_split(game: Game, info: dict) -> bool:
    ...
    my_blobs = list(me.blobs.values())
    if len(my_blobs) >= 4:
        return False  # v3_b4: CYCLE_TARGET_BLOBS=4 (evolution's discovery: split to 4, not 16 -> bigger viable pieces)

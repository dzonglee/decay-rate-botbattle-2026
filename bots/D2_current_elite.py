...
CONFIG = {
    ...
    "W_PREY": 5.99972,
    "W_THREAT": 5.27378,
    "W_VIRUS_BIG": 0.000686844,
    "W_WALL": 9.84723e-07,
    "W_REGROUP": 2.73674e-09,
    "FOOD_FALLOFF": 2.88242,
    "PREY_FALLOFF": 0.0176887,
    "THREAT_FALLOFF": 8.09849e-08,
    "THREAT_PANIC_DIST": 1.13771e-05,
    "THREAT_PANIC_MULT": 2.45211e-05,
    "THREAT_IGNORE_DIST": 181.092,
    "SPLIT_REACH": 51.3259,
    "W_SPLIT_ZONE": 5.4721e-05,
    "WALL_MARGIN": 6.38532e-08,
    "FRESH_MASS": 2.17884,
    "FRESH_CAUTION": 3.20742e-06,
    "VETO_ENABLED": False,
    "VETO_MARGIN": 9.47301e-07,
    "VETO_HORIZON": 6.90464,
    "VETO_SOFT_MASS": 0.993121,
    "LOCK_ENABLED": False,
    "LOCK_MIN_VALUE": 6.05342e-05,
    "LOCK_TICKS_MAX": 93.8424,
    "LOCK_ABANDON_T": 95.6921,
    "W_LOCK": 0.000970389,
    "LOCK_THREAT_BREAK": 0.000169262,
    "LUNGE_DISC_PANIC": 0.795288,
    "SPLIT_RUN_ENABLED": False,
    "SPLIT_RUN_MAX_MASS": 1.95038e-09,
    "SPLIT_RUN_TRIGGER": 0.0238602,
    "EAT_RATIO": 1.2,
    "SAFETY_RATIO": 1.31718,
    "LEAD_TICKS": 1.80087,
    "LUNGE_ALARM_DIST": 0.000151892,
    "W_LUNGE_ALARM": 0.0157196,
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 292.515,
    "SPLIT_SAFETY_RATIO": 0.000395358,
    "SPLIT_MAX_BLOBS": 1.57742,
    "SPLIT_THREAT_CLEARANCE": 9.7791e-08,
    "SPLIT_MIN_MASS": 2.0,
    "SPLIT_MAX_MASS": 121.684,
    "VIRUS_DANGER_MASS_RATIO": 1.2,
    "VIRUS_AVOID_DIST": 1.02459,
    "VIRUS_FEAST_CLEAR": 3.53256,
    # (BOOT_* and W_VIRUS_FOOD amputated 07-09: dead code paths, wasted mutations)
    "FEAST_MIN_MASS": 5.40817,
    "FEAST_SLOT_SAT": 0.767786,
    "W_VIRUS_FEAST": 0.516891,
    ...
    "CYCLE_THREAT_CLEAR": 3.19211,   # only cycle when threats are FAR (avoid deaths-while-fragmented); bounds (2,30)
    ...
    "HUNTER_AVOID_MULT": 0.816008,     # virus-avoid radius mult when hunter near (was 1.5)
    "HUNTER_REPEL_MULT": 1,     # virus repulsion mult when hunter near (was 2.0)
    "WALL_ESC_MARGIN_MULT": 0.636429,  # wall-escape activation margin (was 1.2)
    "WALL_ESC_NUDGE": 0.1,        # outward nudge strength (was 0.2)
    "LUNGE_DISC_FRAC": 0.45,       # deep-disc panic fraction (was 0.9)
    "SPLIT_ALIGN_MIN": 0.417167,       # prey-lunge alignment gate (was 0.7)
    "DEATH_COST_SCALE": 12.9138,     # bank-size fear scaling divisor (was 25)
    "VIRUS_FEAST_FALLOFF": 1.04729,   # feast attraction falloff exponent (was 1/d)
    "CYCLE_TARGET_BLOBS": 4,   # cycle split ceiling (was 16)
    # ORGAN: corner-siege fix (uncatchable corner-tucked prey -> zero force)
    "CORNER_SKIP_ON": 0.152707,        # >0.5 on; bounds (0,1)
    "CORNER_TUCK": 2.04685,
    "CORNER_MARGIN": 0.00795362,
    ...
    "W_VIRUS_SHIELD": 0.089038,        # 0=off; attraction to viruses when fleeing while small
    "SHIELD_MAX_MASS": 10.4143,       # only shield when total below this (we pass under gate)
    # ORGAN: fragment hunting (counter the split-feast meta: eat cyclers mid-cycle)
    "W_FRAG_HUNT": 0.91941,           # 0=off; prey-force mult vs players at high blob count
    "FRAG_HUNT_MIN_BLOBS": 2,   # enemy blob count to trigger
    # ORGAN: piece guard (post-consumption regroup surge; heals the loss30 wound)
    "W_PIECE_GUARD": 0.0801886,         # 0=off; extra regroup for GUARD_TICKS after our shatter
    "PIECE_GUARD_TICKS": 5.73884,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 0.300672,          # 0=off; regroup when fragmented + no viruses visible
    ...
    "FOOD_HUNGER_EXP": 0.149538,       # 0=off; food force *= (1/total_mass)^exp
    # ORGAN: stall kick (anti-deadlock: displacement ~0 while mass decays -> kick)
    "STALL_KICK_ON": 0.0294645,         # >0.5 on
    "STALL_TICKS": 13.7113,
    "STALL_DIST": 1.03531,
    "W_STALL_KICK": 0.118261,
    # ORGAN: endgame posture (protect the bank late; final mass IS the score)
    "ENDGAME_START": 0.778745,         # fraction of EXPECTED_ROUNDS after which endgame fear applies; 1.0=off
    "ENDGAME_FEAR_MULT": 0.512877,     # threat-force mult in endgame; 1.0=neutral
    "EXPECTED_ROUNDS": 1400.0,
    # ORGAN: speed-aware threat pricing (slow giants can't catch us outside envelope)
    "W_ENVELOPE_SCALE": 0.185007,      # 0=off; scales threat force by kill-envelope proximity
    # ORGAN: per-player aggression memory (avoid players who hunt US specifically)
    "AGGRO_ON": 0,              # >0.5 on
    "W_AGGRO": 0.0586663,               # extra threat weight vs measured aggressors
    ...
    "W_WEALTH_FEAR": 0.111437,         # 0=off; threat force mult grows with own mass above WEALTH_START
    "WEALTH_START": 42.6506,         # own total mass where wealth fear begins
    "WEALTH_EXP": 0.315078,            # curve shape
    ...
    "CAMP_WINDOW_LO": 16.5365,       # rounds since consumption when attraction starts
    "CAMP_WINDOW_HI": 43.2686,       # ... and ends (site pruned)
    "CAMP_MAX_MASS": 57.3124,        # too rich to camp (banking dominates)
    # ORGAN W3: GRUDGE MEMORY — per-opponent caution after they cost us mass
    "W_GRUDGE": 0.183118,              # 0=off; extra threat weight vs players who ate our blobs
    ...
    "W_RANK_AGGRO": 0.346503,          # 0=off; prey mult when trailing the visible field
    # ORGAN W5: VIRUS SLOT TIMING — pieces=max(1,17-blobs): saturated consumption is free mass
    "VIRUS_SLOT_EXP": 0.301752,        # 0=off; feast force *= (blobs/16)^exp
    # ===== OMNI-2.1 PROFILER (in-body stupid/mid/elite classifier; Chris's order 2026-07-10) =====
    "PROF_ON": 0.171698,                 # MASTER GATE: <=0.5 -> entire profiler off (zero compute, zero state)
    "PROF_ELITE_T": 0.85,           # EMA score above -> ELITE
    "PROF_STUPID_T": 0.15,          # EMA score below -> STUPID
    "PROF_RADIUS": 8.59021,            # elite-proximity radius gating feast boldness
    "PROF_PREY_STUPID": 0.306816,        # extra prey force vs STUPID targets
    "PROF_PREY_ELITE_DISC": 0.16418,    # prey force discount vs ELITE targets (0..1)
    "PROF_THREAT_STUPID_DISC": 0.0850283, # threat force discount for STUPID giants (0..1)
    "PROF_THREAT_ELITE_MULT": 0.0583827,  # extra threat force vs proven-ELITE hunters
    "PROF_FEAST_BOLD": 0.112686,         # virus-feast/cycle boldness when no ELITE within PROF_RADIUS
    # ===== OMNI-2.1 corner refuge (gate = weight > 0) =====
    "W_CORNER_REFUGE": 0.890901,         # when hunted by an uncatchable-for-us giant: tuck into ITS unreachable corner
    "AGGRO_DECAY": 0.999,
}

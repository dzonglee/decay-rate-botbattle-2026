...
CONFIG = {
    ...
    "W_VIRUS_BIG": 104.175,
    "W_WALL": 0.156293,
    "W_REGROUP": 2.52629e-05,
    "FOOD_FALLOFF": 3.71455,
    "PREY_FALLOFF": 0.547838,
    "THREAT_FALLOFF": 0.0268089,
    "THREAT_PANIC_DIST": 0.869529,
    "THREAT_PANIC_MULT": 0.0696105,
    "THREAT_IGNORE_DIST": 29.3688,
    "SPLIT_REACH": 51.3259,
    "W_SPLIT_ZONE": 0.147816,
    "WALL_MARGIN": 0.012308,
    "FRESH_MASS": 0.925877,
    "FRESH_CAUTION": 0.0217471,
    "VETO_ENABLED": False,
    "VETO_MARGIN": 1.14516,
    "VETO_HORIZON": 9.60909,
    "VETO_SOFT_MASS": 3.1913,
    "LOCK_ENABLED": False,
    "LOCK_MIN_VALUE": 0.330397,
    "LOCK_TICKS_MAX": 84.3171,
    "LOCK_ABANDON_T": 14.7304,
    "W_LOCK": 1.45052,
    "LOCK_THREAT_BREAK": 0.057075,
    "LUNGE_DISC_PANIC": 0.795288,
    "SPLIT_RUN_ENABLED": False,
    "SPLIT_RUN_MAX_MASS": 0.00142209,
    "SPLIT_RUN_TRIGGER": 0.0082183,
    "EAT_RATIO": 1.2,
    "SAFETY_RATIO": 1.1717,
    "LEAD_TICKS": 1.1034,
    "LUNGE_ALARM_DIST": 0.0392531,
    "W_LUNGE_ALARM": 0.297697,
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 22.9996,
    "SPLIT_SAFETY_RATIO": 0.00587377,
    "SPLIT_MAX_BLOBS": 1.05117,
    "SPLIT_THREAT_CLEARANCE": 0.0376177,
    "SPLIT_MIN_MASS": 2.0,
    "SPLIT_MAX_MASS": 150.743,
    "VIRUS_DANGER_MASS_RATIO": 1.2,
    "VIRUS_AVOID_DIST": 1.4209,
    "VIRUS_FEAST_CLEAR": 2,
    # (BOOT_* and W_VIRUS_FOOD amputated 07-09: dead code paths, wasted mutations)
    "FEAST_MIN_MASS": 45,
    "FEAST_SLOT_SAT": 0.3,
    "W_VIRUS_FEAST": 1.72679,
    # --- SPLIT-FEAST CYCLE ORGAN (Team-15 virus economy; matches 1719/1721/1723) ---
    "SPLIT_CYCLE_ON": 1.0,        # vetoable master switch (>0.5 = on); bounds (0,1)
    "CYCLE_MIN_MASS": 30.0,       # only cycle once BIG (don't fragment a small bank); bounds (4,40)
    "CYCLE_THREAT_CLEAR": 28.0,   # only cycle when threats are FAR (avoid deaths-while-fragmented); bounds (2,30)
    ...
    "HUNTER_AVOID_MULT": 1.5,     # virus-avoid radius mult when hunter near (was 1.5)
    "HUNTER_REPEL_MULT": 2.0,     # virus repulsion mult when hunter near (was 2.0)
    "WALL_ESC_MARGIN_MULT": 1.2,  # wall-escape activation margin (was 1.2)
    "WALL_ESC_NUDGE": 0.2,        # outward nudge strength (was 0.2)
    "LUNGE_DISC_FRAC": 0.9,       # deep-disc panic fraction (was 0.9)
    "SPLIT_ALIGN_MIN": 0.7,       # prey-lunge alignment gate (was 0.7)
    "DEATH_COST_SCALE": 25.0,     # bank-size fear scaling divisor (was 25)
    "VIRUS_FEAST_FALLOFF": 1.0,   # feast attraction falloff exponent (was 1/d)
    "CYCLE_TARGET_BLOBS": 16.0,   # cycle split ceiling (was 16)
    # ORGAN: corner-siege fix (uncatchable corner-tucked prey -> zero force)
    "CORNER_SKIP_ON": 1.0,        # >0.5 on; bounds (0,1)
    "CORNER_TUCK": 1.6,
    "CORNER_MARGIN": 0.5,
    # ORGAN: sized threat gate (only count watchers that can actually eat us)
    "THREAT_SIZE_GATE": 0.0,      # 0=all threats count (legacy); >0: hunter_near only if threat mass >= ours*gate
    # ORGAN: virus shield (hide behind viruses; big pursuers pop crossing them)
    "W_VIRUS_SHIELD": 0.0,        # 0=off; attraction to viruses when fleeing while small
    "SHIELD_MAX_MASS": 8.0,       # only shield when total below this (we pass under gate)
    # ORGAN: fragment hunting (counter the split-feast meta: eat cyclers mid-cycle)
    "W_FRAG_HUNT": 0.0,           # 0=off; prey-force mult vs players at high blob count
    "FRAG_HUNT_MIN_BLOBS": 6.0,   # enemy blob count to trigger
    # ORGAN: piece guard (post-consumption regroup surge; heals the loss30 wound)
    "W_PIECE_GUARD": 0.0,         # 0=off; extra regroup for GUARD_TICKS after our shatter
    "PIECE_GUARD_TICKS": 20.0,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 0.0,          # 0=off; regroup when fragmented + no viruses visible
    # ORGAN: map positioning (center vs edge preference; evolution picks sign)
    "W_CENTER": 0.0,              # 0=off; + seeks center, - seeks edges
    # ORGAN: hunger-scaled food greed (small bots chase pellets harder)
    "FOOD_HUNGER_EXP": 0.0,       # 0=off; food force *= (1/total_mass)^exp
    # ORGAN: stall kick (anti-deadlock: displacement ~0 while mass decays -> kick)
    "STALL_KICK_ON": 0.0,         # >0.5 on
    "STALL_TICKS": 60.0,
    "STALL_DIST": 1.0,
    "W_STALL_KICK": 2.0,
    # ORGAN: endgame posture (protect the bank late; final mass IS the score)
    "ENDGAME_START": 1.0,         # fraction of EXPECTED_ROUNDS after which endgame fear applies; 1.0=off
    "ENDGAME_FEAR_MULT": 1.0,     # threat-force mult in endgame; 1.0=neutral
    "EXPECTED_ROUNDS": 1400.0,
    # ORGAN: speed-aware threat pricing (slow giants can't catch us outside envelope)
    "W_ENVELOPE_SCALE": 0.0,      # 0=off; scales threat force by kill-envelope proximity
    # ORGAN: per-player aggression memory (avoid players who hunt US specifically)
    "AGGRO_ON": 0.0,              # >0.5 on
    "W_AGGRO": 0.5,               # extra threat weight vs measured aggressors
    ...
    "W_WEALTH_FEAR": 0.0,         # 0=off; threat force mult grows with own mass above WEALTH_START
    "WEALTH_START": 40.0,         # own total mass where wealth fear begins
    "WEALTH_EXP": 1.0,            # curve shape
    # ORGAN W2: VIRUS-RESPAWN CAMPING — engine law: virus respawns ~30 rounds after consumption
    "W_CAMP": 0.0,                # 0=off; attraction to due-respawn sites inside the window
    "CAMP_WINDOW_LO": 18.0,       # rounds since consumption when attraction starts
    "CAMP_WINDOW_HI": 45.0,       # ... and ends (site pruned)
    "CAMP_MAX_MASS": 60.0,        # too rich to camp (banking dominates)
    # ORGAN W3: GRUDGE MEMORY — per-opponent caution after they cost us mass
    "W_GRUDGE": 0.0,              # 0=off; extra threat weight vs players who ate our blobs
    "GRUDGE_DECAY": 0.995,        # per-tick decay
    # ORGAN W4: RANK POSTURE — visible-field standings condition greed/fear
    "W_RANK_GUARD": 0.0,          # 0=off; threat mult when leading the visible field
    "W_RANK_AGGRO": 0.0,          # 0=off; prey mult when trailing the visible field
    # ORGAN W5: VIRUS SLOT TIMING — pieces=max(1,17-blobs): saturated consumption is free mass
    "VIRUS_SLOT_EXP": 0.0,        # 0=off; feast force *= (blobs/16)^exp
    # ===== OMNI-2.1 PROFILER (in-body stupid/mid/elite classifier; Chris's order 2026-07-10) =====
    "PROF_ON": 0.0,                 # MASTER GATE: <=0.5 -> entire profiler off (zero compute, zero state)
    "PROF_ELITE_T": 0.62,           # EMA score above -> ELITE
    "PROF_STUPID_T": 0.42,          # EMA score below -> STUPID
    "PROF_RADIUS": 14.0,            # elite-proximity radius gating feast boldness
    "PROF_PREY_STUPID": 0.0,        # extra prey force vs STUPID targets
    "PROF_PREY_ELITE_DISC": 0.0,    # prey force discount vs ELITE targets (0..1)
    "PROF_THREAT_STUPID_DISC": 0.0, # threat force discount for STUPID giants (0..1)
    "PROF_THREAT_ELITE_MULT": 0.0,  # extra threat force vs proven-ELITE hunters
    "PROF_FEAST_BOLD": 0.0,         # virus-feast/cycle boldness when no ELITE within PROF_RADIUS
    # ===== OMNI-2.2 LG OPPORTUNITY GATE (Chris's morning build 2026-07-11) =====
    # Late-game conditional harvest: from the team/Banana forensics — winners keep
    # growing late toward SAFE opportunity and refuse only risk. Master-gated.
    "LG_ON": 0.0,                 # MASTER GATE: <=0.5 -> zero compute
    "LG_START": 0.62,             # phase threshold (fraction of EXPECTED_ROUNDS)
    "LG_THRESH": 3.0,             # opportunity-minus-risk break-even point
    "LG_SHARP": 0.35,             # sigmoid sharpness
    "LG_RADIUS": 16.0,            # local scan radius for harvest/risk scoring
    "LG_PREY_HIGH": 2.0,          # prey+feast force mult at full opportunity (1.0=neutral)
    "LG_FEAR_LOW": 0.55,          # threat force mult at full opportunity (1.0=neutral)
    # ===== OMNI-2.1 corner refuge (gate = weight > 0) =====
    "W_CORNER_REFUGE": 0.0,         # when hunted by an uncatchable-for-us giant: tuck into ITS unreachable corner
    "AGGRO_DECAY": 0.995,
    # ===== OMNI-2.3 RETENTION STACK (2026-07-11, Chris's order; Phase-4 organs) =====
    "CONS_ON": 0,               # MASTER GATE R1 consolidation: <=0.5 -> zero compute
    "CONS_REGROUP": 0.8,        # regroup force weight while fragmented AND threatened
    "LKD_ON": 0,                # MASTER GATE R2 wealth lockdown
    "LKD_START": 0.55,          # game fraction when lockdown can engage
    "LKD_MIN_MASS": 25.0,       # min own mass to lock down
    "LKD_RANK_FRAC": 0.8,       # engage when own mass >= frac * biggest visible rival
    "LKD_CLEAR": 12.0,          # in lockdown, feast only if no threat within this radius
    "LKD_FEAR": 1.6,            # threat force mult in lockdown
    "LKD_PREY": 0.5,            # prey force mult in lockdown
    "SPG_ON": 0,                # MASTER GATE R3 split projection
    "SPG_WINDOW": 25.0,         # projection horizon (ticks ~ merge cooldown)
}
...
def compute_forces(game: Game, tracker: Tracker) -> tuple[float, float, dict]:
    ...
    # OMNI-2.3 R2 lockdown state (master-gated; hard mode, not a weight)
    lockdown = False
    if (CONFIG["LKD_ON"] > 0.5 and tracker.tick > CONFIG["LKD_START"] * CONFIG["EXPECTED_ROUNDS"]
            and total_mass >= CONFIG["LKD_MIN_MASS"]):
        _rival = max((v for _pid, v in _vis.items() if _pid != me.player_id), default=0.0)
        lockdown = total_mass >= CONFIG["LKD_RANK_FRAC"] * _rival
    ...
    # OMNI-2.2 LG GATE pre-pass (master-gated): late-phase opportunity vs risk
    lg_prey_mult = 1.0
    lg_fear_mult = 1.0
    if CONFIG["LG_ON"] > 0.5 and tracker.tick > CONFIG["LG_START"] * CONFIG["EXPECTED_ROUNDS"]:
        _harvest = 0.0
        _risk = 0.0
        for _b in st.visible_blobs:
            _d = math.hypot(_b.pos[0] - cx, _b.pos[1] - cy)
            if _d > CONFIG["LG_RADIUS"]:
                continue
            _bm = mass(_b.radius)
            if mass(my_largest) >= _bm * CONFIG["EAT_RATIO"]:
                _harvest += _bm
            elif _bm >= mass(my_smallest) * CONFIG["EAT_RATIO"]:
                _risk += _bm
        _harvest += 2.25 * sum(1 for _v in st.visible_viruses
                               if math.hypot(_v.pos[0] - cx, _v.pos[1] - cy) < CONFIG["LG_RADIUS"])
        _z = CONFIG["LG_SHARP"] * (_harvest - _risk - CONFIG["LG_THRESH"])
        _opp = 1.0 / (1.0 + math.exp(-max(-30.0, min(30.0, _z))))
        lg_prey_mult = 1.0 + (CONFIG["LG_PREY_HIGH"] - 1.0) * _opp
        lg_fear_mult = 1.0 + (CONFIG["LG_FEAR_LOW"] - 1.0) * _opp
    ...
    for b in st.visible_blobs:
        ...
        if mass(b.radius) >= mass(my_smallest) * CONFIG["SAFETY_RATIO"]:
            ...
            # OMNI-2.3 R1: fear anchored to our most exposed blob, not the centroid
            if CONFIG["CONS_ON"] > 0.5 and len(my_blobs) > 1:
                _dv = min((math.hypot(b.pos[0] - mb.pos[0], b.pos[1] - mb.pos[1])
                           for mb in my_blobs
                           if mass(b.radius) >= mass(mb.radius) * CONFIG["SAFETY_RATIO"]),
                          default=d)
                d = min(d, _dv)
            ...
            # OMNI-2.2 LG gate: at high late-game opportunity, fear less
            if CONFIG["LG_ON"] > 0.5:
                f *= lg_fear_mult
            fx -= ux * f
            fy -= uy * f
            ...
            # OMNI-2.2 LG gate: harvest late opportunity harder
            if CONFIG["LG_ON"] > 0.5:
                f *= lg_prey_mult
            ...
            # OMNI-2.3 R2: lockdown — don't chase, bank
            if lockdown:
                f *= CONFIG["LKD_PREY"]
            fx += ux * f
            fy += uy * f
    ...
    # OMNI-2.3 R2: lockdown feast guard — feast only with LKD_CLEAR clearance
    if lockdown and any(_d < CONFIG["LKD_CLEAR"] for _, _d in info["threats"]):
        hunter_near = True
    ...
    for v in st.visible_viruses:
        ...
        if feast_posture and not hunter_near:
            ...
            # OMNI-2.2 LG gate: late safe-feast boost
            if CONFIG["LG_ON"] > 0.5:
                f *= lg_prey_mult
            fx += ux * f
            fy += uy * f
...
# --- OMNI-2.1 corner refuge: the mirror of the corner fix (gate = weight > 0) ---
    ...
    # OMNI-2.3 R1: consolidation surge — fragmented and threatened -> converge hard
    if CONFIG["CONS_ON"] > 0.5 and len(my_blobs) > 1 and info["threats"]:
        gx = sum(b.pos[0] * mass(b.radius) for b in my_blobs) / total_mass
        gy = sum(b.pos[1] * mass(b.radius) for b in my_blobs) / total_mass
        ux, uy, d = unit(gx - cx, gy - cy)
        fx += ux * CONFIG["CONS_REGROUP"] * d
        fy += uy * CONFIG["CONS_REGROUP"] * d
...
def should_split(game: Game, info: dict, fx: float, fy: float) -> bool:
    ...
    for _, d in info["threats"]:
        if d < CONFIG["SPLIT_THREAT_CLEARANCE"]:
            return False
    # OMNI-2.3 R2: lockdown -> never fragment
    if CONFIG["LKD_ON"] > 0.5 and info.get("lockdown"):
        return False
    # OMNI-2.3 R3: veto splits any threat can punish within the merge window
    if CONFIG["SPG_ON"] > 0.5:
        _half_m = mass(largest / math.sqrt(2))
        for _t, _d in info["threats"]:
            if mass(_t.radius) >= _half_m * CONFIG["SAFETY_RATIO"]:
                _reach = blob_speed(_t.radius) * CONFIG["SPG_WINDOW"] + _t.radius
                if mass(_t.radius / math.sqrt(2)) >= _half_m * CONFIG["EAT_RATIO"]:
                    _reach += CONFIG["SPLIT_REACH"]
                if _d < _reach:
                    return False
...
def should_cycle_split(game: Game, info: dict) -> bool:
    ...
    if CONFIG["LKD_ON"] > 0.5 and info.get("lockdown"):
        return False
    # OMNI-2.3 R3: same projection veto for cycle splits
    if CONFIG["SPG_ON"] > 0.5:
        _half_m = mass(largest / math.sqrt(2))
        for _t, _d in info["threats"]:
            if mass(_t.radius) >= _half_m * CONFIG["SAFETY_RATIO"]:
                if _d < blob_speed(_t.radius) * CONFIG["SPG_WINDOW"] + _t.radius:
                    return False
    return True

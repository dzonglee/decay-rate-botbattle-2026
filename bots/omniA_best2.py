...
CONFIG = {
    ...
    "W_VIRUS_BIG": 6.59297,
    "W_WALL": 0.00211419,
    "W_REGROUP": 4.88494e-10,
    "FOOD_FALLOFF": 2.8311,
    "PREY_FALLOFF": 0.000271462,
    "THREAT_FALLOFF": 9.12868e-06,
    "THREAT_PANIC_DIST": 0.000103745,
    "THREAT_PANIC_MULT": 3.26103,
    "THREAT_IGNORE_DIST": 33.3987,
    "SPLIT_REACH": 51.3259,
    "W_SPLIT_ZONE": 0.00263824,
    "WALL_MARGIN": 1.78755e-05,
    "FRESH_MASS": 1.78362,
    "FRESH_CAUTION": 0.000322372,
    "VETO_ENABLED": False,
    "VETO_MARGIN": 1.10097,
    "VETO_HORIZON": 7.86941,
    "VETO_SOFT_MASS": 0.81,
    "LOCK_ENABLED": False,
    "LOCK_MIN_VALUE": 0.0122831,
    "LOCK_TICKS_MAX": 50.3317,
    "LOCK_ABANDON_T": 179.552,
    "W_LOCK": 0.00465257,
    "LOCK_THREAT_BREAK": 0.000150336,
    "LUNGE_DISC_PANIC": 0.795288,
    "SPLIT_RUN_ENABLED": False,
    "SPLIT_RUN_MAX_MASS": 2.18946e-06,
    "SPLIT_RUN_TRIGGER": 2.7292e-05,
    "EAT_RATIO": 1.2,
    "SAFETY_RATIO": 1.4992,
    "LEAD_TICKS": 1.21276,
    "LUNGE_ALARM_DIST": 0.000731274,
    "W_LUNGE_ALARM": 0.0542471,
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 32.6708,
    "SPLIT_SAFETY_RATIO": 4.2378e-05,
    "SPLIT_MAX_BLOBS": 1.73554,
    "SPLIT_THREAT_CLEARANCE": 0.00435632,
    "SPLIT_MIN_MASS": 2.0,
    "SPLIT_MAX_MASS": 70.5532,
    "VIRUS_DANGER_MASS_RATIO": 1.2,
    "VIRUS_AVOID_DIST": 1.12312,
    "VIRUS_FEAST_CLEAR": 3.98208,
    # (BOOT_* and W_VIRUS_FOOD amputated 07-09: dead code paths, wasted mutations)
    "FEAST_MIN_MASS": 4.02666,
    "FEAST_SLOT_SAT": 0.32678,
    "W_VIRUS_FEAST": 7.46525,
    ...
    "HUNTER_AVOID_MULT": 0.963191,     # virus-avoid radius mult when hunter near (was 1.5)
    "HUNTER_REPEL_MULT": 1.31467,     # virus repulsion mult when hunter near (was 2.0)
    "WALL_ESC_MARGIN_MULT": 0.767998,  # wall-escape activation margin (was 1.2)
    "WALL_ESC_NUDGE": 0.1,        # outward nudge strength (was 0.2)
    "LUNGE_DISC_FRAC": 0.4587,       # deep-disc panic fraction (was 0.9)
    "SPLIT_ALIGN_MIN": 0.469807,       # prey-lunge alignment gate (was 0.7)
    "DEATH_COST_SCALE": 17.9516,     # bank-size fear scaling divisor (was 25)
    "VIRUS_FEAST_FALLOFF": 0.944247,   # feast attraction falloff exponent (was 1/d)
    "CYCLE_TARGET_BLOBS": 5.02842,   # cycle split ceiling (was 16)
    # ORGAN: corner-siege fix (uncatchable corner-tucked prey -> zero force)
    "CORNER_SKIP_ON": 0.0303979,        # >0.5 on; bounds (0,1)
    "CORNER_TUCK": 4,
    "CORNER_MARGIN": 0.046182,
    # ORGAN: sized threat gate (only count watchers that can actually eat us)
    "THREAT_SIZE_GATE": 0.000602158,      # 0=all threats count (legacy); >0: hunter_near only if threat mass >= ours*gate
    # ORGAN: virus shield (hide behind viruses; big pursuers pop crossing them)
    "W_VIRUS_SHIELD": 5.58664e-05,        # 0=off; attraction to viruses when fleeing while small
    "SHIELD_MAX_MASS": 3.72639,       # only shield when total below this (we pass under gate)
    # ORGAN: fragment hunting (counter the split-feast meta: eat cyclers mid-cycle)
    "W_FRAG_HUNT": 0.000645496,           # 0=off; prey-force mult vs players at high blob count
    "FRAG_HUNT_MIN_BLOBS": 2,   # enemy blob count to trigger
    # ORGAN: piece guard (post-consumption regroup surge; heals the loss30 wound)
    "W_PIECE_GUARD": 0.000331578,         # 0=off; extra regroup for GUARD_TICKS after our shatter
    "PIECE_GUARD_TICKS": 23.9671,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 0.000705636,          # 0=off; regroup when fragmented + no viruses visible
    # ORGAN: map positioning (center vs edge preference; evolution picks sign)
    "W_CENTER": 1.75761e-05,              # 0=off; + seeks center, - seeks edges
    # ORGAN: hunger-scaled food greed (small bots chase pellets harder)
    "FOOD_HUNGER_EXP": 9.07815e-05,       # 0=off; food force *= (1/total_mass)^exp
    # ORGAN: stall kick (anti-deadlock: displacement ~0 while mass decays -> kick)
    "STALL_KICK_ON": 0.00216425,         # >0.5 on
    "STALL_TICKS": 13.8107,
    "STALL_DIST": 0.2,
    "W_STALL_KICK": 0.0899216,
    # ORGAN: endgame posture (protect the bank late; final mass IS the score)
    "ENDGAME_START": 0.434041,         # fraction of EXPECTED_ROUNDS after which endgame fear applies; 1.0=off
    ...
    "W_ENVELOPE_SCALE": 0.000346259,      # 0=off; scales threat force by kill-envelope proximity
    # ORGAN: per-player aggression memory (avoid players who hunt US specifically)
    "AGGRO_ON": 0.00978478,              # >0.5 on
    "W_AGGRO": 0.00277337,               # extra threat weight vs measured aggressors
    "AGGRO_DECAY": 0.9,
}

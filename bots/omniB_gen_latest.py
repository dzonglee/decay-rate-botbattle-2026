...
CONFIG = {
    ...
    "FEAST_MIN_MASS": 6.3203,
    "FEAST_SLOT_SAT": 0.3,
    "W_VIRUS_FEAST": 0.0176766,
    # --- SPLIT-FEAST CYCLE ORGAN (Team-15 virus economy; matches 1719/1721/1723) ---
    "SPLIT_CYCLE_ON": 1.75888e-05,        # vetoable master switch (>0.5 = on); bounds (0,1)
    "CYCLE_MIN_MASS": 5.2279,       # only cycle once BIG (don't fragment a small bank); bounds (4,40)
    "CYCLE_THREAT_CLEAR": 13.9989,   # only cycle when threats are FAR (avoid deaths-while-fragmented); bounds (2,30)
    ...
    "HUNTER_AVOID_MULT": 1.11746,     # virus-avoid radius mult when hunter near (was 1.5)
    "HUNTER_REPEL_MULT": 2.03363,     # virus repulsion mult when hunter near (was 2.0)
    ...
    "CYCLE_TARGET_BLOBS": 9.976,   # cycle split ceiling (was 16)
    ...
    "THREAT_SIZE_GATE": 0.00224553,      # 0=all threats count (legacy); >0: hunter_near only if threat mass >= ours*gate
    ...
    "W_ENVELOPE_SCALE": 0.0244073,      # 0=off; scales threat force by kill-envelope proximity

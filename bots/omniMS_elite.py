...
CONFIG = {
    ...
    "W_PREY": 4.11213,
    "W_THREAT": 4.30832,
    "W_VIRUS_BIG": 0.000451042,
    "W_WALL": 1.29543e-06,
    "W_REGROUP": 4.14947e-09,
    "FOOD_FALLOFF": 2.65018,
    "PREY_FALLOFF": 0.0107309,
    "THREAT_FALLOFF": 7.33429e-08,
    "THREAT_PANIC_DIST": 4.28118e-05,
    "THREAT_PANIC_MULT": 4.10837e-05,
    "THREAT_IGNORE_DIST": 96.1882,
    "SPLIT_REACH": 51.3259,
    "W_SPLIT_ZONE": 2.34311e-05,
    "WALL_MARGIN": 3.84723e-08,
    "FRESH_MASS": 2.65367,
    "FRESH_CAUTION": 1.63301e-06,
    "VETO_ENABLED": False,
    "VETO_MARGIN": 7.78972e-07,
    "VETO_HORIZON": 6.92704,
    "VETO_SOFT_MASS": 2.59411,
    "LOCK_ENABLED": False,
    "LOCK_MIN_VALUE": 2.88981e-05,
    "LOCK_TICKS_MAX": 59.8079,
    "LOCK_ABANDON_T": 53.4703,
    "W_LOCK": 0.000474477,
    "LOCK_THREAT_BREAK": 6.73872e-05,
    "LUNGE_DISC_PANIC": 0.795288,
    "SPLIT_RUN_ENABLED": False,
    "SPLIT_RUN_MAX_MASS": 1.66606e-09,
    "SPLIT_RUN_TRIGGER": 0.0238602,
    "EAT_RATIO": 1.2,
    "SAFETY_RATIO": 1.15,
    "LEAD_TICKS": 1.45996,
    "LUNGE_ALARM_DIST": 0.000151841,
    "W_LUNGE_ALARM": 0.0133181,
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 284.994,
    "SPLIT_SAFETY_RATIO": 0.000540652,
    "SPLIT_MAX_BLOBS": 1.60151,
    "SPLIT_THREAT_CLEARANCE": 2.98218e-07,
    "SPLIT_MIN_MASS": 2.0,
    "SPLIT_MAX_MASS": 289.509,
    "VIRUS_DANGER_MASS_RATIO": 1.2,
    "VIRUS_AVOID_DIST": 0.910765,
    "VIRUS_FEAST_CLEAR": 4.07921,
    # (BOOT_* and W_VIRUS_FOOD amputated 07-09: dead code paths, wasted mutations)
    "FEAST_MIN_MASS": 7.41605,
    "FEAST_SLOT_SAT": 0.66278,
    "W_VIRUS_FEAST": 0.237598,
    ...
    "CYCLE_MIN_MASS": 44.5968,       # only cycle once BIG (don't fragment a small bank); bounds (4,40)
    ...
    "HUNTER_REPEL_MULT": 1.82048,     # virus repulsion mult when hunter near (was 2.0)
    "WALL_ESC_MARGIN_MULT": 0.938622,  # wall-escape activation margin (was 1.2)
    "WALL_ESC_NUDGE": 0.1,        # outward nudge strength (was 0.2)
    "LUNGE_DISC_FRAC": 0.45,       # deep-disc panic fraction (was 0.9)
    "SPLIT_ALIGN_MIN": 0.431749,       # prey-lunge alignment gate (was 0.7)
    "DEATH_COST_SCALE": 19.4789,     # bank-size fear scaling divisor (was 25)
    "VIRUS_FEAST_FALLOFF": 1.5,   # feast attraction falloff exponent (was 1/d)
    ...
    "CORNER_TUCK": 1.94178,
    "CORNER_MARGIN": -0.0541233,
    # ORGAN: sized threat gate (only count watchers that can actually eat us)
    "THREAT_SIZE_GATE": 0.0358575,      # 0=all threats count (legacy); >0: hunter_near only if threat mass >= ours*gate
    # ORGAN: virus shield (hide behind viruses; big pursuers pop crossing them)
    "W_VIRUS_SHIELD": 0.105315,        # 0=off; attraction to viruses when fleeing while small
    "SHIELD_MAX_MASS": 4.1512,       # only shield when total below this (we pass under gate)
    ...
    "W_PIECE_GUARD": 0.304709,         # 0=off; extra regroup for GUARD_TICKS after our shatter
    "PIECE_GUARD_TICKS": 7.80559,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 0.253192,          # 0=off; regroup when fragmented + no viruses visible
    ...
    "FOOD_HUNGER_EXP": 0.166246,       # 0=off; food force *= (1/total_mass)^exp
    ...
    "ENDGAME_FEAR_MULT": 1.64692,     # threat-force mult in endgame; 1.0=neutral
    "EXPECTED_ROUNDS": 1400.0,
    # ORGAN: speed-aware threat pricing (slow giants can't catch us outside envelope)
    "W_ENVELOPE_SCALE": 0.16392,      # 0=off; scales threat force by kill-envelope proximity
    # ORGAN: per-player aggression memory (avoid players who hunt US specifically)
    "AGGRO_ON": 0.194195,              # >0.5 on
    "W_AGGRO": 0.119557,               # extra threat weight vs measured aggressors
    ...
    "WEALTH_START": 25.7672,         # own total mass where wealth fear begins
    "WEALTH_EXP": 0.439967,            # curve shape
    ...
    "CAMP_WINDOW_LO": 8.84696,       # rounds since consumption when attraction starts
    "CAMP_WINDOW_HI": 60,       # ... and ends (site pruned)
    "CAMP_MAX_MASS": 35.149,        # too rich to camp (banking dominates)
    # ORGAN W3: GRUDGE MEMORY — per-opponent caution after they cost us mass
    "W_GRUDGE": 0.196872,              # 0=off; extra threat weight vs players who ate our blobs
    "GRUDGE_DECAY": 0.9995,        # per-tick decay
    # ORGAN W4: RANK POSTURE — visible-field standings condition greed/fear
    "W_RANK_GUARD": 0.0585065,          # 0=off; threat mult when leading the visible field
    "W_RANK_AGGRO": 1.02171,          # 0=off; prey mult when trailing the visible field
    ...
    "PROF_ON": 0.132613,                 # MASTER GATE: <=0.5 -> entire profiler off (zero compute, zero state)
    "PROF_ELITE_T": 0.85,           # EMA score above -> ELITE
    "PROF_STUPID_T": 0.193628,          # EMA score below -> STUPID
    "PROF_RADIUS": 8.86047,            # elite-proximity radius gating feast boldness
    "PROF_PREY_STUPID": 0.310675,        # extra prey force vs STUPID targets
    "PROF_PREY_ELITE_DISC": 0.158726,    # prey force discount vs ELITE targets (0..1)
    "PROF_THREAT_STUPID_DISC": 0.0810245, # threat force discount for STUPID giants (0..1)
    "PROF_THREAT_ELITE_MULT": 0.0957846,  # extra threat force vs proven-ELITE hunters
    "PROF_FEAST_BOLD": 0.138447,         # virus-feast/cycle boldness when no ELITE within PROF_RADIUS

...
CONFIG = {
    ...
    # Targets: 0=none, 1=prey residual, 2=escape residual, 3=virus residual,
    #          4=center residual, 5=base-force gain, 6=guarded split bias.
    "ARCH_ON": 1.0,
    "ARCH_SPLIT_THRESHOLD": 0.65,
    "ARCH_SPLIT_THREAT_CLEAR": 12.0,
    "ARCH_MAX_ACTIVE": 8.0,      # hard cap; graph has 11 slots but only this many may execute
    ...
    "ARCH_N0_K": 2.0,
    "ARCH_N0_BIAS": 0.0,
    "ARCH_N0_TARGET": 1.0,   # prey residual
    "ARCH_N0_GAIN": 0.30,
    ...
    "ARCH_N1_K": 2.0,
    "ARCH_N1_BIAS": 0.0,
    "ARCH_N1_TARGET": 2.0,   # escape residual
    "ARCH_N1_GAIN": 0.40,
    ...
    "ARCH_N2_K": 2.0,
    "ARCH_N2_BIAS": 0.0,
    "ARCH_N2_TARGET": 6.0,   # guarded split bias
    "ARCH_N2_GAIN": 1.0,
...
def apply_architecture(game: Game, tracker: Tracker, info: dict,
    ...
    """Evaluate an 11-slot typed residual computation graph.

    Mutation can change node activation, inputs, operations, graph connections,
    output targets, and gains. Later nodes may consume earlier node outputs.
    Only ARCH_MAX_ACTIVE enabled slots execute, preserving a bounded search.
    Disabling ARCH_ON exactly recovers the original chassis.
    """
    ...
    base_mag = _clip(math.hypot(fx, fy) / 20.0, 0.0, 1.0)
    ...
    if total >= max(vis_mass.values(), default=0.0):
        rank_lead = 1.0

    values = [
        1.0,                          # 0 constant
        tick_frac,                    # 1 time
        _clip(total / 80.0, 0, 1),    # 2 mass
        wealth,                       # 3 wealth
        _clip(len(blobs) / 16.0, 0, 1), # 4 blob count
        threat_pressure,              # 5
        prey_opportunity,             # 6
        virus_visible,                # 7
        edge_pressure,                # 8
        ...
        rank_lead,                    # 10
        base_mag,                     # 11
        1.0 if threat_pressure > 0.35 else 0.0, # 12 hunter near
        late_phase,                   # 13
        feast_ready,                  # 14
        _clip((1.0 - threat_pressure) * prey_opportunity, 0, 1), # 15 safe opportunity
    ]

    prey_dir = _nearest_direction(cx, cy, info["prey"], away=False)
    escape_dir = _nearest_direction(cx, cy, info["threats"], away=True)
    if st.visible_viruses:
        v = min(st.visible_viruses, key=lambda q: (q.pos[0]-cx)**2 + (q.pos[1]-cy)**2)
        virus_dir = unit(v.pos[0]-cx, v.pos[1]-cy)[:2]
    else:
        virus_dir = (0.0, 0.0)
    ...
    for i in range(11):
        ...
        if not enabled:
            # Preserve the stable node-output index even when a slot is dormant.
            values.append(0.0)
            continue
        active_used += 1
        a_idx = int(round(CONFIG[f"ARCH_N{i}_A"]))
        b_idx = int(round(CONFIG[f"ARCH_N{i}_B"]))
        max_idx = len(values) - 1
        a = values[max(0, min(max_idx, a_idx))]
        b = values[max(0, min(max_idx, b_idx))]
        out = _arch_op(
            int(round(CONFIG[f"ARCH_N{i}_OP"])), a, b,
            CONFIG[f"ARCH_N{i}_K"], CONFIG[f"ARCH_N{i}_BIAS"])
        values.append(out)
        target = int(round(CONFIG[f"ARCH_N{i}_TARGET"]))
        gain = CONFIG[f"ARCH_N{i}_GAIN"]
        amount = gain * out
        if target == 1:
            fx += prey_dir[0] * amount
            fy += prey_dir[1] * amount
        elif target == 2:
            fx += escape_dir[0] * amount
            fy += escape_dir[1] * amount
        elif target == 3:
            fx += virus_dir[0] * amount
            fy += virus_dir[1] * amount
        elif target == 4:
            fx += center_dir[0] * amount
            fy += center_dir[1] * amount
        elif target == 5:
            scale = max(0.1, min(3.0, 1.0 + amount))
            fx *= scale
            fy *= scale
        elif target == 6:
            split_bias += amount

    return fx, fy, split_bias


def architecture_safe_split(game: Game, info: dict) -> bool:
    """Hard safety shell around architecture-generated split requests."""
            ...
            match query:
                case QueryMovePlayer():
                    ...
                    try:
                        ...
                        # ARCH-EVO residual graph: may reshape movement and cast a guarded split vote.
                        fx, fy, arch_split_bias = apply_architecture(game, tracker, info, fx, fy)

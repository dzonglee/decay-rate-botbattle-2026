...
CONFIG = {
    ...
    "W_VIRUS_BIG": 4.39848,
    "W_WALL": 0.00179466,
    "W_REGROUP": 1.53016e-10,
    "FOOD_FALLOFF": 2.0945,
    "PREY_FALLOFF": 0.000323751,
    "THREAT_FALLOFF": 2.65207e-05,
    "THREAT_PANIC_DIST": 1.10481e-06,
    "THREAT_PANIC_MULT": 0.272217,
    "THREAT_IGNORE_DIST": 26.3218,
    "SPLIT_REACH": 51.3259,
    "W_SPLIT_ZONE": 0.0009444,
    "WALL_MARGIN": 4.00117e-06,
    "FRESH_MASS": 0.81,
    "FRESH_CAUTION": 0.00018901,
    "VETO_ENABLED": False,
    "VETO_MARGIN": 0.130742,
    "VETO_HORIZON": 2,
    "VETO_SOFT_MASS": 1.46443,
    "LOCK_ENABLED": False,
    "LOCK_MIN_VALUE": 0.0116998,
    "LOCK_TICKS_MAX": 42.3134,
    "LOCK_ABANDON_T": 29.3809,
    "W_LOCK": 8.27858e-05,
    "LOCK_THREAT_BREAK": 8.90132e-05,
    "LUNGE_DISC_PANIC": 0.795288,
    "SPLIT_RUN_ENABLED": False,
    "SPLIT_RUN_MAX_MASS": 1.44418e-07,
    "SPLIT_RUN_TRIGGER": 1.3276e-05,
    "EAT_RATIO": 1.2,
    "SAFETY_RATIO": 1.39192,
    "LEAD_TICKS": 1,
    "LUNGE_ALARM_DIST": 7.69007e-05,
    "W_LUNGE_ALARM": 0.220098,
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 17.2027,
    "SPLIT_SAFETY_RATIO": 0.000234581,
    "SPLIT_MAX_BLOBS": 1.10682,
    "SPLIT_THREAT_CLEARANCE": 0.00240705,
    "SPLIT_MIN_MASS": 2.0,
    "SPLIT_MAX_MASS": 214.343,
    "VIRUS_DANGER_MASS_RATIO": 1.2,
    "VIRUS_AVOID_DIST": 0.3,
    "VIRUS_FEAST_CLEAR": 3.13459,
    # (BOOT_* and W_VIRUS_FOOD amputated 07-09: dead code paths, wasted mutations)
    "FEAST_MIN_MASS": 4.88497,
    "FEAST_SLOT_SAT": 0.3,
    "W_VIRUS_FEAST": 7.14624,
    # --- SPLIT-FEAST CYCLE ORGAN (Team-15 virus economy; matches 1719/1721/1723) ---
    "SPLIT_CYCLE_ON": 0.673298,        # vetoable master switch (>0.5 = on); bounds (0,1)
    "CYCLE_MIN_MASS": 48.6045,       # only cycle once BIG (don't fragment a small bank); bounds (4,40)
    "CYCLE_THREAT_CLEAR": 3.12583,   # only cycle when threats are FAR (avoid deaths-while-fragmented); bounds (2,30)
    ...
    "HUNTER_AVOID_MULT": 2.01922,     # virus-avoid radius mult when hunter near (was 1.5)
    "HUNTER_REPEL_MULT": 2.72293,     # virus repulsion mult when hunter near (was 2.0)
    "WALL_ESC_MARGIN_MULT": 0.814863,  # wall-escape activation margin (was 1.2)
    "WALL_ESC_NUDGE": 0.173628,        # outward nudge strength (was 0.2)
    "LUNGE_DISC_FRAC": 0.486413,       # deep-disc panic fraction (was 0.9)
    "SPLIT_ALIGN_MIN": 0.643148,       # prey-lunge alignment gate (was 0.7)
    "DEATH_COST_SCALE": 27.5311,     # bank-size fear scaling divisor (was 25)
    "VIRUS_FEAST_FALLOFF": 0.793542,   # feast attraction falloff exponent (was 1/d)
    "CYCLE_TARGET_BLOBS": 8.54839,   # cycle split ceiling (was 16)
    # ORGAN: corner-siege fix (uncatchable corner-tucked prey -> zero force)
    "CORNER_SKIP_ON": 0.0221497,        # >0.5 on; bounds (0,1)
    "CORNER_TUCK": 1.61327,
    "CORNER_MARGIN": 0.0462181,
    # ORGAN: sized threat gate (only count watchers that can actually eat us)
    "THREAT_SIZE_GATE": 0.000179619,      # 0=all threats count (legacy); >0: hunter_near only if threat mass >= ours*gate
    # ORGAN: virus shield (hide behind viruses; big pursuers pop crossing them)
    "W_VIRUS_SHIELD": 0.000300819,        # 0=off; attraction to viruses when fleeing while small
    "SHIELD_MAX_MASS": 2.88895,       # only shield when total below this (we pass under gate)
    # ORGAN: fragment hunting (counter the split-feast meta: eat cyclers mid-cycle)
    "W_FRAG_HUNT": 0.00378994,           # 0=off; prey-force mult vs players at high blob count
    "FRAG_HUNT_MIN_BLOBS": 2.1391,   # enemy blob count to trigger
    # ORGAN: piece guard (post-consumption regroup surge; heals the loss30 wound)
    "W_PIECE_GUARD": 0.0011938,         # 0=off; extra regroup for GUARD_TICKS after our shatter
    "PIECE_GUARD_TICKS": 25.357,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 0.000130272,          # 0=off; regroup when fragmented + no viruses visible
    # ORGAN: map positioning (center vs edge preference; evolution picks sign)
    "W_CENTER": 6.84264e-06,              # 0=off; + seeks center, - seeks edges
    # ORGAN: hunger-scaled food greed (small bots chase pellets harder)
    "FOOD_HUNGER_EXP": 0.000336596,       # 0=off; food force *= (1/total_mass)^exp
    # ORGAN: stall kick (anti-deadlock: displacement ~0 while mass decays -> kick)
    "STALL_KICK_ON": 0.00167316,         # >0.5 on
    "STALL_TICKS": 10,
    "STALL_DIST": 0.2,
    "W_STALL_KICK": 0.161042,
    # ORGAN: endgame posture (protect the bank late; final mass IS the score)
    "ENDGAME_START": 0.825466,         # fraction of EXPECTED_ROUNDS after which endgame fear applies; 1.0=off
    "ENDGAME_FEAR_MULT": 1.15749,     # threat-force mult in endgame; 1.0=neutral
    "EXPECTED_ROUNDS": 1400.0,
    # ORGAN: speed-aware threat pricing (slow giants can't catch us outside envelope)
    "W_ENVELOPE_SCALE": 1.56584e-05,      # 0=off; scales threat force by kill-envelope proximity
    # ORGAN: per-player aggression memory (avoid players who hunt US specifically)
    "AGGRO_ON": 0.00136377,              # >0.5 on
    "W_AGGRO": 9.61732e-05,               # extra threat weight vs measured aggressors
    "AGGRO_DECAY": 0.999,
}

...
CONFIG = {
    # --- force weights ---
    "W_FOOD": 2.90022,           # attraction per food pellet
    "W_PREY": 18.5539,          # attraction toward edible enemy blobs (x their mass)
    "W_THREAT": 27.7941,        # repulsion from blobs that can eat us (x their mass)
    "W_VIRUS_BIG": 30.0,     # repulsion from viruses when we are big/unsplit-averse
    "W_WALL": 1.0054,           # repulsion from arena walls inside the margin
    "W_REGROUP": 0.0028054,        # pull our own blobs toward our centroid when split

    # --- distance shaping ---
    "FOOD_FALLOFF": 3.94295,     # force ~ w / (d^falloff); 1 favours clusters over nearest
    "PREY_FALLOFF": 0.33355,
    "THREAT_FALLOFF": 1.09479,   # steep: distant threats barely matter, close ones dominate
    "THREAT_PANIC_DIST": 1.09757,   # inside this, threat force is multiplied
    "THREAT_PANIC_MULT": 8.86198,
    "THREAT_IGNORE_DIST": 8.05884,  # beyond this, big-but-slow blobs are ignored (keep farming)
    "SPLIT_REACH": 51.3259,         # engine: eject speed 1.6, drag 0.82 -> ~8.9 units max lunge
    "W_SPLIT_ZONE": 2.69588,       # repulsion from being inside a split-capable threat's lunge radius
    "WALL_MARGIN": 1.21197,      # start repelling this far from a wall
    "FRESH_MASS": 0.178626,       # below this total mass we are in survival mode
    "FRESH_CAUTION": 0.264971,    # fear multiplier (and greed divisor) while fresh
    # ===== GEOMETRY ORGANS (hybrid challenger) =====
    "VETO_ENABLED": True,       # constitutional veto: lethal directions cannot win
    "VETO_MARGIN": 1.0,         # padding added to every kill envelope
    "VETO_HORIZON": 8.0,        # ticks of our travel checked along each direction
    "VETO_SOFT_MASS": 3.0,      # below this bank, no vetoes (fresh = expendable)
    "LOCK_ENABLED": True,       # commitment latch: hunt one target to conclusion
    "LOCK_MIN_VALUE": 1.2,      # ignore prey worth less mass than this
    "LOCK_TICKS_MAX": 120,      # forced re-election after this many ticks
    "LOCK_ABANDON_T": 60.0,     # abandon if intercept time exceeds this (ticks)
    "W_LOCK": 25.0,             # goal-pull weight for the locked target
    "LOCK_THREAT_BREAK": 6.0,   # drop the hunt if a threat closes within this
    ...
    "SPLIT_RUN_MAX_MASS": 0.602749,  # only worth it while the bank is small
    "SPLIT_RUN_TRIGGER": 5.01684,    # pursuer inside this and closing -> consider it

    # --- eat/threat classification ---
    "EAT_RATIO": 1.2,        # engine rule: must be >= 20% larger (by radius)
    "SAFETY_RATIO": 1.09859,    # treat anything within 12% of edible-us as a threat too

    # --- prediction ---
    "LEAD_TICKS": 0.15003,         # extrapolate enemy velocity this many ticks ahead
    "LUNGE_ALARM_DIST": 0.0428457,   # react to hostile split intent within this range
    "W_LUNGE_ALARM": 3.00673,      # extra flee force when a bigger blob splits at us

    # --- split attack ---
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 16.4377,      # only lunge if predicted prey within this distance
    "SPLIT_SAFETY_RATIO": 0.0301273,  # each half must be >= this x prey radius (buffer over 1.2)
    "SPLIT_MAX_BLOBS": 1.73122,        # never split if we'd exceed this many blobs
    "SPLIT_THREAT_CLEARANCE": 1.43715,  # no threat within this distance when we split
    "SPLIT_MIN_MASS": 2.0,       # engine minimum
    "SPLIT_MAX_MASS": 52.5109,      # above this total mass, never lunge: protect the bank

    # --- virus handling ---
    "VIRUS_DANGER_MASS_RATIO": 1.2,  # engine: consume needs blob.mass > 1.5*1.2 = 1.8
    "VIRUS_AVOID_DIST": 4.0,
    ...
    "VIRUS_FEAST_CLEAR": 26.5531,  # no threat within this distance -> virus is food, not hazard

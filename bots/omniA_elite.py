...
CONFIG = {
    ...
    "W_VIRUS_BIG": 23.4165,
    "W_WALL": 0.0644703,
    "W_REGROUP": 9.64361e-07,
    "FOOD_FALLOFF": 2.41205,
    "PREY_FALLOFF": 0.0130753,
    "THREAT_FALLOFF": 0.0210506,
    "THREAT_PANIC_DIST": 0.387704,
    "THREAT_PANIC_MULT": 0.0939855,
    "THREAT_IGNORE_DIST": 11.7478,
    "SPLIT_REACH": 51.3259,
    "W_SPLIT_ZONE": 0.013192,
    "WALL_MARGIN": 0.0162707,
    "FRESH_MASS": 1.40155,
    "FRESH_CAUTION": 0.0607241,
    "VETO_ENABLED": False,
    "VETO_MARGIN": 0.182265,
    "VETO_HORIZON": 2.0454,
    "VETO_SOFT_MASS": 0.865941,
    "LOCK_ENABLED": False,
    "LOCK_MIN_VALUE": 0.121164,
    "LOCK_TICKS_MAX": 20,
    "LOCK_ABANDON_T": 21.6264,
    "W_LOCK": 0.570404,
    "LOCK_THREAT_BREAK": 0.0677855,
    "LUNGE_DISC_PANIC": 0.795288,
    "SPLIT_RUN_ENABLED": False,
    "SPLIT_RUN_MAX_MASS": 0.00010766,
    "SPLIT_RUN_TRIGGER": 0.0069309,
    "EAT_RATIO": 1.2,
    "SAFETY_RATIO": 1.37519,
    "LEAD_TICKS": 1.9791,
    "LUNGE_ALARM_DIST": 0.00406659,
    "W_LUNGE_ALARM": 0.771349,
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 31.552,
    "SPLIT_SAFETY_RATIO": 0.000631495,
    "SPLIT_MAX_BLOBS": 1.55725,
    "SPLIT_THREAT_CLEARANCE": 0.0202772,
    "SPLIT_MIN_MASS": 2.0,
    "SPLIT_MAX_MASS": 232.932,
    "VIRUS_DANGER_MASS_RATIO": 1.2,
    "VIRUS_AVOID_DIST": 1.72275,
    "VIRUS_FEAST_CLEAR": 2,
    # (BOOT_* and W_VIRUS_FOOD amputated 07-09: dead code paths, wasted mutations)
    "FEAST_MIN_MASS": 4.53989,
    "FEAST_SLOT_SAT": 0.356754,
    "W_VIRUS_FEAST": 0.216186,
    # --- SPLIT-FEAST CYCLE ORGAN (Team-15 virus economy; matches 1719/1721/1723) ---
    "SPLIT_CYCLE_ON": 0.933577,        # vetoable master switch (>0.5 = on); bounds (0,1)
    "CYCLE_MIN_MASS": 60,       # only cycle once BIG (don't fragment a small bank); bounds (4,40)
    "CYCLE_THREAT_CLEAR": 2.93044,   # only cycle when threats are FAR (avoid deaths-while-fragmented); bounds (2,30)
    ...
    "HUNTER_AVOID_MULT": 0.921096,     # virus-avoid radius mult when hunter near (was 1.5)
    "HUNTER_REPEL_MULT": 3,     # virus repulsion mult when hunter near (was 2.0)
    "WALL_ESC_MARGIN_MULT": 1.0929,  # wall-escape activation margin (was 1.2)
    "WALL_ESC_NUDGE": 0.119784,        # outward nudge strength (was 0.2)
    "LUNGE_DISC_FRAC": 0.922685,       # deep-disc panic fraction (was 0.9)
    "SPLIT_ALIGN_MIN": 0.471143,       # prey-lunge alignment gate (was 0.7)
    "DEATH_COST_SCALE": 32.7638,     # bank-size fear scaling divisor (was 25)
    "VIRUS_FEAST_FALLOFF": 0.5,   # feast attraction falloff exponent (was 1/d)
    "CYCLE_TARGET_BLOBS": 4.84465,   # cycle split ceiling (was 16)
    # ORGAN: corner-siege fix (uncatchable corner-tucked prey -> zero force)
    "CORNER_SKIP_ON": 0.230362,        # >0.5 on; bounds (0,1)
    "CORNER_TUCK": 0.611483,
    "CORNER_MARGIN": 0.121457,
    # ORGAN: sized threat gate (only count watchers that can actually eat us)
    "THREAT_SIZE_GATE": 0.240573,      # 0=all threats count (legacy); >0: hunter_near only if threat mass >= ours*gate
    # ORGAN: virus shield (hide behind viruses; big pursuers pop crossing them)
    "W_VIRUS_SHIELD": 0.00909073,        # 0=off; attraction to viruses when fleeing while small
    "SHIELD_MAX_MASS": 6.37765,       # only shield when total below this (we pass under gate)
    # ORGAN: fragment hunting (counter the split-feast meta: eat cyclers mid-cycle)
    "W_FRAG_HUNT": 0.0363029,           # 0=off; prey-force mult vs players at high blob count
    "FRAG_HUNT_MIN_BLOBS": 3.79889,   # enemy blob count to trigger
    # ORGAN: piece guard (post-consumption regroup surge; heals the loss30 wound)
    "W_PIECE_GUARD": 0.000757703,         # 0=off; extra regroup for GUARD_TICKS after our shatter
    "PIECE_GUARD_TICKS": 3.48785,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 0.142617,          # 0=off; regroup when fragmented + no viruses visible
    # ORGAN: map positioning (center vs edge preference; evolution picks sign)
    "W_CENTER": 0.0704227,              # 0=off; + seeks center, - seeks edges
    # ORGAN: hunger-scaled food greed (small bots chase pellets harder)
    "FOOD_HUNGER_EXP": 0.0140798,       # 0=off; food force *= (1/total_mass)^exp
    # ORGAN: stall kick (anti-deadlock: displacement ~0 while mass decays -> kick)
    "STALL_KICK_ON": 0.671387,         # >0.5 on
    "STALL_TICKS": 13.1802,
    "STALL_DIST": 0.380866,
    "W_STALL_KICK": 1.2368,
    # ORGAN: endgame posture (protect the bank late; final mass IS the score)
    "ENDGAME_START": 0.834676,         # fraction of EXPECTED_ROUNDS after which endgame fear applies; 1.0=off
    "ENDGAME_FEAR_MULT": 2.95819,     # threat-force mult in endgame; 1.0=neutral
    "EXPECTED_ROUNDS": 1400.0,
    # ORGAN: speed-aware threat pricing (slow giants can't catch us outside envelope)
    "W_ENVELOPE_SCALE": 0.124266,      # 0=off; scales threat force by kill-envelope proximity
    # ORGAN: per-player aggression memory (avoid players who hunt US specifically)
    "AGGRO_ON": 0.0763281,              # >0.5 on
    "W_AGGRO": 0.0827864,               # extra threat weight vs measured aggressors
    "AGGRO_DECAY": 0.999,
}

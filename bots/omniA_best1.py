...
CONFIG = {
    ...
    "W_VIRUS_BIG": 7.58002,
    "W_WALL": 0.00441871,
    "W_REGROUP": 3.87101e-10,
    "FOOD_FALLOFF": 2.58955,
    "PREY_FALLOFF": 0.000271462,
    "THREAT_FALLOFF": 9.12868e-06,
    "THREAT_PANIC_DIST": 0.000107603,
    "THREAT_PANIC_MULT": 3.58194,
    "THREAT_IGNORE_DIST": 19.0739,
    "SPLIT_REACH": 51.3259,
    "W_SPLIT_ZONE": 0.00243844,
    "WALL_MARGIN": 2.12175e-05,
    "FRESH_MASS": 1.39295,
    "FRESH_CAUTION": 0.00021364,
    "VETO_ENABLED": False,
    "VETO_MARGIN": 2.11579,
    "VETO_HORIZON": 9.79397,
    "VETO_SOFT_MASS": 0.81,
    "LOCK_ENABLED": False,
    "LOCK_MIN_VALUE": 0.00883621,
    "LOCK_TICKS_MAX": 40.7447,
    "LOCK_ABANDON_T": 197.741,
    "W_LOCK": 0.00251853,
    "LOCK_THREAT_BREAK": 0.000246144,
    "LUNGE_DISC_PANIC": 0.795288,
    "SPLIT_RUN_ENABLED": False,
    "SPLIT_RUN_MAX_MASS": 2.40207e-06,
    "SPLIT_RUN_TRIGGER": 2.06409e-05,
    "EAT_RATIO": 1.2,
    "SAFETY_RATIO": 1.15,
    "LEAD_TICKS": 1,
    "LUNGE_ALARM_DIST": 0.000513809,
    "W_LUNGE_ALARM": 0.091771,
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 15.3991,
    "SPLIT_SAFETY_RATIO": 3.60626e-05,
    "SPLIT_MAX_BLOBS": 1.31244,
    "SPLIT_THREAT_CLEARANCE": 0.00435632,
    "SPLIT_MIN_MASS": 2.0,
    "SPLIT_MAX_MASS": 111.935,
    "VIRUS_DANGER_MASS_RATIO": 1.2,
    "VIRUS_AVOID_DIST": 0.708359,
    "VIRUS_FEAST_CLEAR": 5.924,
    # (BOOT_* and W_VIRUS_FOOD amputated 07-09: dead code paths, wasted mutations)
    "FEAST_MIN_MASS": 4,
    "FEAST_SLOT_SAT": 0.35226,
    "W_VIRUS_FEAST": 2.62458,
    ...
    "HUNTER_AVOID_MULT": 0.825168,     # virus-avoid radius mult when hunter near (was 1.5)
    "HUNTER_REPEL_MULT": 1.02905,     # virus repulsion mult when hunter near (was 2.0)
    "WALL_ESC_MARGIN_MULT": 0.769738,  # wall-escape activation margin (was 1.2)
    "WALL_ESC_NUDGE": 0.1,        # outward nudge strength (was 0.2)
    "LUNGE_DISC_FRAC": 0.580206,       # deep-disc panic fraction (was 0.9)
    "SPLIT_ALIGN_MIN": 0.415579,       # prey-lunge alignment gate (was 0.7)
    "DEATH_COST_SCALE": 14.6098,     # bank-size fear scaling divisor (was 25)
    "VIRUS_FEAST_FALLOFF": 0.628071,   # feast attraction falloff exponent (was 1/d)
    "CYCLE_TARGET_BLOBS": 8.9468,   # cycle split ceiling (was 16)
    # ORGAN: corner-siege fix (uncatchable corner-tucked prey -> zero force)
    "CORNER_SKIP_ON": 0.0523108,        # >0.5 on; bounds (0,1)
    "CORNER_TUCK": 4,
    "CORNER_MARGIN": 0.0581276,
    # ORGAN: sized threat gate (only count watchers that can actually eat us)
    "THREAT_SIZE_GATE": 0.000187721,      # 0=all threats count (legacy); >0: hunter_near only if threat mass >= ours*gate
    # ORGAN: virus shield (hide behind viruses; big pursuers pop crossing them)
    "W_VIRUS_SHIELD": 0.000153569,        # 0=off; attraction to viruses when fleeing while small
    "SHIELD_MAX_MASS": 4.57652,       # only shield when total below this (we pass under gate)
    # ORGAN: fragment hunting (counter the split-feast meta: eat cyclers mid-cycle)
    "W_FRAG_HUNT": 0.000594357,           # 0=off; prey-force mult vs players at high blob count
    "FRAG_HUNT_MIN_BLOBS": 2,   # enemy blob count to trigger
    # ORGAN: piece guard (post-consumption regroup surge; heals the loss30 wound)
    "W_PIECE_GUARD": 0.000363293,         # 0=off; extra regroup for GUARD_TICKS after our shatter
    "PIECE_GUARD_TICKS": 13.9288,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 0.000815824,          # 0=off; regroup when fragmented + no viruses visible
    # ORGAN: map positioning (center vs edge preference; evolution picks sign)
    "W_CENTER": 2.01179e-05,              # 0=off; + seeks center, - seeks edges
    # ORGAN: hunger-scaled food greed (small bots chase pellets harder)
    "FOOD_HUNGER_EXP": 6.21653e-05,       # 0=off; food force *= (1/total_mass)^exp
    # ORGAN: stall kick (anti-deadlock: displacement ~0 while mass decays -> kick)
    "STALL_KICK_ON": 0.00206321,         # >0.5 on
    "STALL_TICKS": 10,
    "STALL_DIST": 0.2,
    "W_STALL_KICK": 0.0617085,
    # ORGAN: endgame posture (protect the bank late; final mass IS the score)
    "ENDGAME_START": 0.321911,         # fraction of EXPECTED_ROUNDS after which endgame fear applies; 1.0=off
    "ENDGAME_FEAR_MULT": 1.00813,     # threat-force mult in endgame; 1.0=neutral
    "EXPECTED_ROUNDS": 1400.0,
    # ORGAN: speed-aware threat pricing (slow giants can't catch us outside envelope)
    "W_ENVELOPE_SCALE": 0.000186171,      # 0=off; scales threat force by kill-envelope proximity
    # ORGAN: per-player aggression memory (avoid players who hunt US specifically)
    "AGGRO_ON": 0.0120031,              # >0.5 on
    "W_AGGRO": 0.00311141,               # extra threat weight vs measured aggressors
    "AGGRO_DECAY": 0.999,
}

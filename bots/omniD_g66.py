...
CONFIG = {
    ...
    "W_VIRUS_BIG": 211.549,
    "W_WALL": 0.0498614,
    "W_REGROUP": 2.41474e-06,
    "FOOD_FALLOFF": 1.52764,
    "PREY_FALLOFF": 0.0565642,
    "THREAT_FALLOFF": 0.00532913,
    "THREAT_PANIC_DIST": 0.363621,
    "THREAT_PANIC_MULT": 0.415316,
    "THREAT_IGNORE_DIST": 23.3,
    "SPLIT_REACH": 51.3259,
    "W_SPLIT_ZONE": 0.0832604,
    "WALL_MARGIN": 0.000576333,
    "FRESH_MASS": 1.09224,
    "FRESH_CAUTION": 0.0114446,
    "VETO_ENABLED": False,
    "VETO_MARGIN": 0.347327,
    "VETO_HORIZON": 2.79509,
    "VETO_SOFT_MASS": 3.77232,
    "LOCK_ENABLED": False,
    "LOCK_MIN_VALUE": 0.539712,
    "LOCK_TICKS_MAX": 34.123,
    "LOCK_ABANDON_T": 94.8129,
    "W_LOCK": 1.46462,
    "LOCK_THREAT_BREAK": 0.00626092,
    "LUNGE_DISC_PANIC": 0.795288,
    "SPLIT_RUN_ENABLED": False,
    "SPLIT_RUN_MAX_MASS": 0.000641115,
    "SPLIT_RUN_TRIGGER": 0.00257369,
    "EAT_RATIO": 1.2,
    "SAFETY_RATIO": 1.33343,
    "LEAD_TICKS": 1.23115,
    "LUNGE_ALARM_DIST": 0.0477619,
    "W_LUNGE_ALARM": 0.0277691,
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 35.6637,
    "SPLIT_SAFETY_RATIO": 0.00532434,
    "SPLIT_MAX_BLOBS": 1.01175,
    "SPLIT_THREAT_CLEARANCE": 0.0436801,
    "SPLIT_MIN_MASS": 2.0,
    "SPLIT_MAX_MASS": 149.96,
    "VIRUS_DANGER_MASS_RATIO": 1.2,
    "VIRUS_AVOID_DIST": 3.83006,
    "VIRUS_FEAST_CLEAR": 2,
    # (BOOT_* and W_VIRUS_FOOD amputated 07-09: dead code paths, wasted mutations)
    "FEAST_MIN_MASS": 8.78664,
    "FEAST_SLOT_SAT": 0.35932,
    "W_VIRUS_FEAST": 0.896652,
    ...
    "CYCLE_MIN_MASS": 45.9524,       # only cycle once BIG (don't fragment a small bank); bounds (4,40)
    ...
    "HUNTER_AVOID_MULT": 0.985415,     # virus-avoid radius mult when hunter near (was 1.5)
    "HUNTER_REPEL_MULT": 1.34934,     # virus repulsion mult when hunter near (was 2.0)
    "WALL_ESC_MARGIN_MULT": 0.621793,  # wall-escape activation margin (was 1.2)
    "WALL_ESC_NUDGE": 0.267896,        # outward nudge strength (was 0.2)
    "LUNGE_DISC_FRAC": 1.35,       # deep-disc panic fraction (was 0.9)
    "SPLIT_ALIGN_MIN": 0.373779,       # prey-lunge alignment gate (was 0.7)
    "DEATH_COST_SCALE": 19.8739,     # bank-size fear scaling divisor (was 25)
    "VIRUS_FEAST_FALLOFF": 0.774328,   # feast attraction falloff exponent (was 1/d)
    "CYCLE_TARGET_BLOBS": 9.61385,   # cycle split ceiling (was 16)
    # ORGAN: corner-siege fix (uncatchable corner-tucked prey -> zero force)
    "CORNER_SKIP_ON": 1,        # >0.5 on; bounds (0,1)
    "CORNER_TUCK": 0.640389,
    "CORNER_MARGIN": 0.0916084,
    # ORGAN: sized threat gate (only count watchers that can actually eat us)
    "THREAT_SIZE_GATE": 0.000403735,      # 0=all threats count (legacy); >0: hunter_near only if threat mass >= ours*gate
    # ORGAN: virus shield (hide behind viruses; big pursuers pop crossing them)
    "W_VIRUS_SHIELD": 0.0336947,        # 0=off; attraction to viruses when fleeing while small
    ...
    "W_FRAG_HUNT": 0.00708323,           # 0=off; prey-force mult vs players at high blob count
    "FRAG_HUNT_MIN_BLOBS": 10.7168,   # enemy blob count to trigger
    # ORGAN: piece guard (post-consumption regroup surge; heals the loss30 wound)
    "W_PIECE_GUARD": 0.244835,         # 0=off; extra regroup for GUARD_TICKS after our shatter
    "PIECE_GUARD_TICKS": 3.10515,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 0.0167855,          # 0=off; regroup when fragmented + no viruses visible
    # ORGAN: map positioning (center vs edge preference; evolution picks sign)
    "W_CENTER": 0.0560869,              # 0=off; + seeks center, - seeks edges
    # ORGAN: hunger-scaled food greed (small bots chase pellets harder)
    "FOOD_HUNGER_EXP": 0.647909,       # 0=off; food force *= (1/total_mass)^exp
    # ORGAN: stall kick (anti-deadlock: displacement ~0 while mass decays -> kick)
    "STALL_KICK_ON": 0.0467925,         # >0.5 on
    "STALL_TICKS": 19.4348,
    "STALL_DIST": 0.2,
    "W_STALL_KICK": 0.139568,
    ...
    "ENDGAME_FEAR_MULT": 0.640712,     # threat-force mult in endgame; 1.0=neutral
    "EXPECTED_ROUNDS": 1400.0,
    # ORGAN: speed-aware threat pricing (slow giants can't catch us outside envelope)
    "W_ENVELOPE_SCALE": 0.115095,      # 0=off; scales threat force by kill-envelope proximity
    # ORGAN: per-player aggression memory (avoid players who hunt US specifically)
    "AGGRO_ON": 0.0430613,              # >0.5 on
    "W_AGGRO": 1.00784,               # extra threat weight vs measured aggressors
    "AGGRO_DECAY": 0.999,
}

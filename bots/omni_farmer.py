...
CONFIG = {
    ...
    "LG_ON": 1,                 # MASTER GATE: <=0.5 -> zero compute
    "LG_START": 0.85,             # phase threshold (fraction of EXPECTED_ROUNDS)
    "LG_THRESH": 2.07853,             # opportunity-minus-risk break-even point
    "LG_SHARP": 0.199917,             # sigmoid sharpness
    "LG_RADIUS": 12.2827,            # local scan radius for harvest/risk scoring
    "LG_PREY_HIGH": 2.56414,          # prey+feast force mult at full opportunity (1.0=neutral)
    "LG_FEAR_LOW": 0.406128,          # threat force mult at full opportunity (1.0=neutral)
    ...
    "FARM_W_CROWD": 0.8,        # CONTINUOUS crowd-density repulsion from ALL visible players
    "FARM_CROWD_FF": 1.2,       # crowd falloff (weak, long-range: routes bend around traffic)
}
...
# --- OMNI-2.1 corner refuge: the mirror of the corner fix (gate = weight > 0) ---
    ...
    if CONFIG["FARM_ON"] > 0.5:
        # positional isolation (Team-15 elaboration): weak ALWAYS-ON repulsion from every
        # visible player -> foraging routes bend around traffic; isolation is the equilibrium.
        if CONFIG["FARM_W_CROWD"] > 0:
            for _b in st.visible_blobs:
                if _b.player_id == me.player_id: continue
                ux, uy, d = unit(_b.pos[0] - cx, _b.pos[1] - cy)
                if d < EPS: continue
                _f = CONFIG["FARM_W_CROWD"] * mass(_b.radius) / (d ** CONFIG["FARM_CROWD_FF"] + EPS) / 10.0
                fx -= ux * _f
                fy -= uy * _f

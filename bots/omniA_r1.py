...
CONFIG = {
    ...
    "W_VIRUS_BIG": 2.40906,
    "W_WALL": 0.00119729,
    "W_REGROUP": 1.35336e-10,
    "FOOD_FALLOFF": 3.10226,
    "PREY_FALLOFF": 0.000340414,
    "THREAT_FALLOFF": 6.92225e-06,
    "THREAT_PANIC_DIST": 1.62073e-06,
    "THREAT_PANIC_MULT": 0.195865,
    "THREAT_IGNORE_DIST": 24.9609,
    "SPLIT_REACH": 51.3259,
    "W_SPLIT_ZONE": 0.000603738,
    "WALL_MARGIN": 3.67759e-06,
    "FRESH_MASS": 0.872601,
    "FRESH_CAUTION": 0.00016232,
    "VETO_ENABLED": False,
    "VETO_MARGIN": 0.132203,
    "VETO_HORIZON": 2.04325,
    "VETO_SOFT_MASS": 1.05308,
    "LOCK_ENABLED": False,
    "LOCK_MIN_VALUE": 0.00692616,
    "LOCK_TICKS_MAX": 75.3965,
    "LOCK_ABANDON_T": 60.7496,
    "W_LOCK": 0.000372689,
    "LOCK_THREAT_BREAK": 8.81814e-05,
    "LUNGE_DISC_PANIC": 0.795288,
    "SPLIT_RUN_ENABLED": False,
    "SPLIT_RUN_MAX_MASS": 5.64385e-08,
    "SPLIT_RUN_TRIGGER": 9.64497e-06,
    "EAT_RATIO": 1.2,
    "SAFETY_RATIO": 1.32075,
    "LEAD_TICKS": 1.22863,
    "LUNGE_ALARM_DIST": 6.30968e-05,
    "W_LUNGE_ALARM": 0.185791,
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 19.289,
    "SPLIT_SAFETY_RATIO": 0.000179096,
    "SPLIT_MAX_BLOBS": 1.44995,
    "SPLIT_THREAT_CLEARANCE": 0.00300012,
    "SPLIT_MIN_MASS": 2.0,
    "SPLIT_MAX_MASS": 243.131,
    "VIRUS_DANGER_MASS_RATIO": 1.2,
    "VIRUS_AVOID_DIST": 0.3,
    "VIRUS_FEAST_CLEAR": 4.284,
    # (BOOT_* and W_VIRUS_FOOD amputated 07-09: dead code paths, wasted mutations)
    "FEAST_MIN_MASS": 4,
    "FEAST_SLOT_SAT": 0.35096,
    "W_VIRUS_FEAST": 2.83074,
    # --- SPLIT-FEAST CYCLE ORGAN (Team-15 virus economy; matches 1719/1721/1723) ---
    "SPLIT_CYCLE_ON": 0.779928,        # vetoable master switch (>0.5 = on); bounds (0,1)
    "CYCLE_MIN_MASS": 40,       # only cycle once BIG (don't fragment a small bank); bounds (4,40)
    "CYCLE_THREAT_CLEAR": 4.42305,   # only cycle when threats are FAR (avoid deaths-while-fragmented); bounds (2,30)
    ...
    "HUNTER_AVOID_MULT": 0.778328,     # virus-avoid radius mult when hunter near (was 1.5)
    "HUNTER_REPEL_MULT": 2.46298,     # virus repulsion mult when hunter near (was 2.0)
    "WALL_ESC_MARGIN_MULT": 0.965489,  # wall-escape activation margin (was 1.2)
    "WALL_ESC_NUDGE": 0.222464,        # outward nudge strength (was 0.2)
    "LUNGE_DISC_FRAC": 0.45,       # deep-disc panic fraction (was 0.9)
    "SPLIT_ALIGN_MIN": 0.653653,       # prey-lunge alignment gate (was 0.7)
    "DEATH_COST_SCALE": 20.971,     # bank-size fear scaling divisor (was 25)
    "VIRUS_FEAST_FALLOFF": 0.544801,   # feast attraction falloff exponent (was 1/d)
    "CYCLE_TARGET_BLOBS": 5.725,   # cycle split ceiling (was 16)
    # ORGAN: corner-siege fix (uncatchable corner-tucked prey -> zero force)
    "CORNER_SKIP_ON": 0.0451757,        # >0.5 on; bounds (0,1)
    "CORNER_TUCK": 1.26391,
    "CORNER_MARGIN": 0.118832,
    # ORGAN: sized threat gate (only count watchers that can actually eat us)
    "THREAT_SIZE_GATE": 0.000256332,      # 0=all threats count (legacy); >0: hunter_near only if threat mass >= ours*gate
    # ORGAN: virus shield (hide behind viruses; big pursuers pop crossing them)
    "W_VIRUS_SHIELD": 0.000227261,        # 0=off; attraction to viruses when fleeing while small
    "SHIELD_MAX_MASS": 2.50695,       # only shield when total below this (we pass under gate)
    # ORGAN: fragment hunting (counter the split-feast meta: eat cyclers mid-cycle)
    "W_FRAG_HUNT": 0.00273753,           # 0=off; prey-force mult vs players at high blob count
    "FRAG_HUNT_MIN_BLOBS": 3.65806,   # enemy blob count to trigger
    # ORGAN: piece guard (post-consumption regroup surge; heals the loss30 wound)
    "W_PIECE_GUARD": 0.00104938,         # 0=off; extra regroup for GUARD_TICKS after our shatter
    "PIECE_GUARD_TICKS": 36.2774,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 0.000137951,          # 0=off; regroup when fragmented + no viruses visible
    # ORGAN: map positioning (center vs edge preference; evolution picks sign)
    "W_CENTER": 1.11334e-05,              # 0=off; + seeks center, - seeks edges
    # ORGAN: hunger-scaled food greed (small bots chase pellets harder)
    "FOOD_HUNGER_EXP": 0.00026137,       # 0=off; food force *= (1/total_mass)^exp
    # ORGAN: stall kick (anti-deadlock: displacement ~0 while mass decays -> kick)
    "STALL_KICK_ON": 0.000972507,         # >0.5 on
    "STALL_TICKS": 21.4909,
    "STALL_DIST": 0.299518,
    "W_STALL_KICK": 0.100575,
    # ORGAN: endgame posture (protect the bank late; final mass IS the score)
    "ENDGAME_START": 0.374207,         # fraction of EXPECTED_ROUNDS after which endgame fear applies; 1.0=off
    "ENDGAME_FEAR_MULT": 0.86534,     # threat-force mult in endgame; 1.0=neutral
    "EXPECTED_ROUNDS": 1400.0,
    # ORGAN: speed-aware threat pricing (slow giants can't catch us outside envelope)
    "W_ENVELOPE_SCALE": 2.25001e-05,      # 0=off; scales threat force by kill-envelope proximity
    # ORGAN: per-player aggression memory (avoid players who hunt US specifically)
    "AGGRO_ON": 0.0031203,              # >0.5 on
    "W_AGGRO": 0.000109268,               # extra threat weight vs measured aggressors
    "AGGRO_DECAY": 0.9,
}

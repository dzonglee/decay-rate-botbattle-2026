...
CONFIG = {
    ...
    "W_VIRUS_BIG": 63.9069,
    "W_WALL": 0.0898899,
    "W_REGROUP": 3.26068e-05,
    "FOOD_FALLOFF": 2.55371,
    "PREY_FALLOFF": 0.764652,
    "THREAT_FALLOFF": 0.0252366,
    "THREAT_PANIC_DIST": 0.828698,
    "THREAT_PANIC_MULT": 0.06694,
    "THREAT_IGNORE_DIST": 26.646,
    "SPLIT_REACH": 51.3259,
    "W_SPLIT_ZONE": 0.256861,
    "WALL_MARGIN": 0.0190634,
    "FRESH_MASS": 1.85182,
    "FRESH_CAUTION": 0.00429703,
    "VETO_ENABLED": False,
    "VETO_MARGIN": 0.607719,
    "VETO_HORIZON": 4.83835,
    "VETO_SOFT_MASS": 1.01859,
    "LOCK_ENABLED": False,
    "LOCK_MIN_VALUE": 0.459195,
    "LOCK_TICKS_MAX": 34.7801,
    "LOCK_ABANDON_T": 15.5746,
    "W_LOCK": 2.36116,
    "LOCK_THREAT_BREAK": 0.0101643,
    "LUNGE_DISC_PANIC": 0.795288,
    "SPLIT_RUN_ENABLED": False,
    "SPLIT_RUN_MAX_MASS": 0.00123637,
    "SPLIT_RUN_TRIGGER": 0.0115351,
    "EAT_RATIO": 1.2,
    "SAFETY_RATIO": 1.15,
    "LEAD_TICKS": 1,
    "LUNGE_ALARM_DIST": 0.0105385,
    "W_LUNGE_ALARM": 0.484257,
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 24.4554,
    "SPLIT_SAFETY_RATIO": 0.00741743,
    "SPLIT_MAX_BLOBS": 1.75919,
    "SPLIT_THREAT_CLEARANCE": 0.0355209,
    "SPLIT_MIN_MASS": 2.0,
    "SPLIT_MAX_MASS": 113.748,
    "VIRUS_DANGER_MASS_RATIO": 1.2,
    "VIRUS_AVOID_DIST": 1.79777,
    "VIRUS_FEAST_CLEAR": 2.56434,
    # (BOOT_* and W_VIRUS_FOOD amputated 07-09: dead code paths, wasted mutations)
    "FEAST_MIN_MASS": 4,
    "FEAST_SLOT_SAT": 0.438115,
    "W_VIRUS_FEAST": 2.6929,
    # --- SPLIT-FEAST CYCLE ORGAN (Team-15 virus economy; matches 1719/1721/1723) ---
    "SPLIT_CYCLE_ON": 1,        # vetoable master switch (>0.5 = on); bounds (0,1)
    "CYCLE_MIN_MASS": 50.2155,       # only cycle once BIG (don't fragment a small bank); bounds (4,40)
    "CYCLE_THREAT_CLEAR": 20.7529,   # only cycle when threats are FAR (avoid deaths-while-fragmented); bounds (2,30)
    ...
    "HUNTER_AVOID_MULT": 1.75573,     # virus-avoid radius mult when hunter near (was 1.5)
    "HUNTER_REPEL_MULT": 2.71567,     # virus repulsion mult when hunter near (was 2.0)
    "WALL_ESC_MARGIN_MULT": 1.42193,  # wall-escape activation margin (was 1.2)
    "WALL_ESC_NUDGE": 0.241599,        # outward nudge strength (was 0.2)
    "LUNGE_DISC_FRAC": 0.834982,       # deep-disc panic fraction (was 0.9)
    "SPLIT_ALIGN_MIN": 0.515633,       # prey-lunge alignment gate (was 0.7)
    "DEATH_COST_SCALE": 17.7839,     # bank-size fear scaling divisor (was 25)
    "VIRUS_FEAST_FALLOFF": 0.605433,   # feast attraction falloff exponent (was 1/d)
    "CYCLE_TARGET_BLOBS": 5.74658,   # cycle split ceiling (was 16)
    # ORGAN: corner-siege fix (uncatchable corner-tucked prey -> zero force)
    "CORNER_SKIP_ON": 1,        # >0.5 on; bounds (0,1)
    "CORNER_TUCK": 1.60256,
    "CORNER_MARGIN": 0.416214,
    # ORGAN: sized threat gate (only count watchers that can actually eat us)
    "THREAT_SIZE_GATE": 0.0459212,      # 0=all threats count (legacy); >0: hunter_near only if threat mass >= ours*gate
    # ORGAN: virus shield (hide behind viruses; big pursuers pop crossing them)
    "W_VIRUS_SHIELD": 0.107051,        # 0=off; attraction to viruses when fleeing while small
    "SHIELD_MAX_MASS": 7.08275,       # only shield when total below this (we pass under gate)
    # ORGAN: fragment hunting (counter the split-feast meta: eat cyclers mid-cycle)
    "W_FRAG_HUNT": 0.10341,           # 0=off; prey-force mult vs players at high blob count
    "FRAG_HUNT_MIN_BLOBS": 4.5474,   # enemy blob count to trigger
    # ORGAN: piece guard (post-consumption regroup surge; heals the loss30 wound)
    "W_PIECE_GUARD": 0.0963911,         # 0=off; extra regroup for GUARD_TICKS after our shatter
    "PIECE_GUARD_TICKS": 8.47053,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 0.237322,          # 0=off; regroup when fragmented + no viruses visible
    # ORGAN: map positioning (center vs edge preference; evolution picks sign)
    "W_CENTER": 0.138405,              # 0=off; + seeks center, - seeks edges
    # ORGAN: hunger-scaled food greed (small bots chase pellets harder)
    "FOOD_HUNGER_EXP": 0.0263775,       # 0=off; food force *= (1/total_mass)^exp
    # ORGAN: stall kick (anti-deadlock: displacement ~0 while mass decays -> kick)
    "STALL_KICK_ON": 0.417861,         # >0.5 on
    "STALL_TICKS": 55.0624,
    "STALL_DIST": 0.678841,
    "W_STALL_KICK": 2.67159,
    # ORGAN: endgame posture (protect the bank late; final mass IS the score)
    "ENDGAME_START": 0.953873,         # fraction of EXPECTED_ROUNDS after which endgame fear applies; 1.0=off
    "ENDGAME_FEAR_MULT": 2.00636,     # threat-force mult in endgame; 1.0=neutral
    "EXPECTED_ROUNDS": 1400.0,
    # ORGAN: speed-aware threat pricing (slow giants can't catch us outside envelope)
    "W_ENVELOPE_SCALE": 0.0864984,      # 0=off; scales threat force by kill-envelope proximity
    # ORGAN: per-player aggression memory (avoid players who hunt US specifically)
    "AGGRO_ON": 0.139563,              # >0.5 on
    "W_AGGRO": 1.38045,               # extra threat weight vs measured aggressors
    ...
    "W_WEALTH_FEAR": 0.0275998,         # 0=off; threat force mult grows with own mass above WEALTH_START
    "WEALTH_START": 54.4821,         # own total mass where wealth fear begins
    "WEALTH_EXP": 0.300085,            # curve shape
    # ORGAN W2: VIRUS-RESPAWN CAMPING — engine law: virus respawns ~30 rounds after consumption
    "W_CAMP": 0.178235,                # 0=off; attraction to due-respawn sites inside the window
    "CAMP_WINDOW_LO": 16.1634,       # rounds since consumption when attraction starts
    "CAMP_WINDOW_HI": 56.0535,       # ... and ends (site pruned)
    "CAMP_MAX_MASS": 101.578,        # too rich to camp (banking dominates)
    # ORGAN W3: GRUDGE MEMORY — per-opponent caution after they cost us mass
    "W_GRUDGE": 0.0631405,              # 0=off; extra threat weight vs players who ate our blobs
    "GRUDGE_DECAY": 0.9995,        # per-tick decay
    # ORGAN W4: RANK POSTURE — visible-field standings condition greed/fear
    "W_RANK_GUARD": 1.81201,          # 0=off; threat mult when leading the visible field
    "W_RANK_AGGRO": 0.131856,          # 0=off; prey mult when trailing the visible field
    ...
    "PROF_ON": 0.267149,                 # MASTER GATE: <=0.5 -> entire profiler off (zero compute, zero state)
    "PROF_ELITE_T": 0.5,           # EMA score above -> ELITE
    "PROF_STUPID_T": 0.286982,          # EMA score below -> STUPID
    "PROF_RADIUS": 9.66514,            # elite-proximity radius gating feast boldness
    "PROF_PREY_STUPID": 0.103087,        # extra prey force vs STUPID targets
    "PROF_PREY_ELITE_DISC": 0.397655,    # prey force discount vs ELITE targets (0..1)
    "PROF_THREAT_STUPID_DISC": 0.480029, # threat force discount for STUPID giants (0..1)
    "PROF_THREAT_ELITE_MULT": 0.0791241,  # extra threat force vs proven-ELITE hunters
    "PROF_FEAST_BOLD": 0.171634,         # virus-feast/cycle boldness when no ELITE within PROF_RADIUS
    # ===== OMNI-2.1 corner refuge (gate = weight > 0) =====
    "W_CORNER_REFUGE": 0.115192,         # when hunted by an uncatchable-for-us giant: tuck into ITS unreachable corner
    "AGGRO_DECAY": 0.905732,
}

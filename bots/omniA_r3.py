...
CONFIG = {
    ...
    "W_VIRUS_BIG": 2.40906,
    "W_WALL": 0.00220259,
    "W_REGROUP": 1.52574e-10,
    "FOOD_FALLOFF": 2.03185,
    "PREY_FALLOFF": 0.000340414,
    "THREAT_FALLOFF": 1.27992e-05,
    "THREAT_PANIC_DIST": 1.14918e-06,
    "THREAT_PANIC_MULT": 0.357703,
    "THREAT_IGNORE_DIST": 28.6241,
    "SPLIT_REACH": 51.3259,
    "W_SPLIT_ZONE": 0.000826131,
    "WALL_MARGIN": 4.53757e-06,
    "FRESH_MASS": 0.81,
    "FRESH_CAUTION": 0.000229422,
    "VETO_ENABLED": False,
    "VETO_MARGIN": 0.0360379,
    "VETO_HORIZON": 2.16472,
    "VETO_SOFT_MASS": 1.25825,
    "LOCK_ENABLED": False,
    "LOCK_MIN_VALUE": 0.00516037,
    "LOCK_TICKS_MAX": 54.4909,
    "LOCK_ABANDON_T": 32.6538,
    "W_LOCK": 0.000799319,
    "LOCK_THREAT_BREAK": 8.7591e-05,
    "LUNGE_DISC_PANIC": 0.795288,
    "SPLIT_RUN_ENABLED": False,
    "SPLIT_RUN_MAX_MASS": 1.19149e-07,
    "SPLIT_RUN_TRIGGER": 2.87401e-06,
    "EAT_RATIO": 1.2,
    "SAFETY_RATIO": 2,
    "LEAD_TICKS": 1.34691,
    "LUNGE_ALARM_DIST": 6.96923e-05,
    "W_LUNGE_ALARM": 0.255446,
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 26.6332,
    "SPLIT_SAFETY_RATIO": 0.000190495,
    "SPLIT_MAX_BLOBS": 1.16692,
    "SPLIT_THREAT_CLEARANCE": 0.00292034,
    "SPLIT_MIN_MASS": 2.0,
    "SPLIT_MAX_MASS": 372.383,
    "VIRUS_DANGER_MASS_RATIO": 1.2,
    "VIRUS_AVOID_DIST": 0.3,
    "VIRUS_FEAST_CLEAR": 10.1399,
    # (BOOT_* and W_VIRUS_FOOD amputated 07-09: dead code paths, wasted mutations)
    "FEAST_MIN_MASS": 8.69985,
    "FEAST_SLOT_SAT": 0.344401,
    "W_VIRUS_FEAST": 3.72524,
    # --- SPLIT-FEAST CYCLE ORGAN (Team-15 virus economy; matches 1719/1721/1723) ---
    "SPLIT_CYCLE_ON": 0.893,        # vetoable master switch (>0.5 = on); bounds (0,1)
    "CYCLE_MIN_MASS": 41.619,       # only cycle once BIG (don't fragment a small bank); bounds (4,40)
    "CYCLE_THREAT_CLEAR": 2.11129,   # only cycle when threats are FAR (avoid deaths-while-fragmented); bounds (2,30)
    ...
    "HUNTER_REPEL_MULT": 2.13728,     # virus repulsion mult when hunter near (was 2.0)
    "WALL_ESC_MARGIN_MULT": 1.01375,  # wall-escape activation margin (was 1.2)
    "WALL_ESC_NUDGE": 0.160711,        # outward nudge strength (was 0.2)
    "LUNGE_DISC_FRAC": 0.45,       # deep-disc panic fraction (was 0.9)
    "SPLIT_ALIGN_MIN": 0.966163,       # prey-lunge alignment gate (was 0.7)
    "DEATH_COST_SCALE": 17.8342,     # bank-size fear scaling divisor (was 25)
    "VIRUS_FEAST_FALLOFF": 0.806053,   # feast attraction falloff exponent (was 1/d)
    "CYCLE_TARGET_BLOBS": 4.80591,   # cycle split ceiling (was 16)
    # ORGAN: corner-siege fix (uncatchable corner-tucked prey -> zero force)
    "CORNER_SKIP_ON": 0.0201631,        # >0.5 on; bounds (0,1)
    "CORNER_TUCK": 0.5,
    "CORNER_MARGIN": 0.0639876,
    # ORGAN: sized threat gate (only count watchers that can actually eat us)
    "THREAT_SIZE_GATE": 0.000132227,      # 0=all threats count (legacy); >0: hunter_near only if threat mass >= ours*gate
    # ORGAN: virus shield (hide behind viruses; big pursuers pop crossing them)
    "W_VIRUS_SHIELD": 0.000140892,        # 0=off; attraction to viruses when fleeing while small
    "SHIELD_MAX_MASS": 3.42587,       # only shield when total below this (we pass under gate)
    # ORGAN: fragment hunting (counter the split-feast meta: eat cyclers mid-cycle)
    "W_FRAG_HUNT": 0.00303595,           # 0=off; prey-force mult vs players at high blob count
    "FRAG_HUNT_MIN_BLOBS": 3.18991,   # enemy blob count to trigger
    # ORGAN: piece guard (post-consumption regroup surge; heals the loss30 wound)
    "W_PIECE_GUARD": 0.00210195,         # 0=off; extra regroup for GUARD_TICKS after our shatter
    "PIECE_GUARD_TICKS": 32.0537,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 0.000119293,          # 0=off; regroup when fragmented + no viruses visible
    # ORGAN: map positioning (center vs edge preference; evolution picks sign)
    "W_CENTER": 1.16679e-05,              # 0=off; + seeks center, - seeks edges
    # ORGAN: hunger-scaled food greed (small bots chase pellets harder)
    "FOOD_HUNGER_EXP": 0.000697274,       # 0=off; food force *= (1/total_mass)^exp
    # ORGAN: stall kick (anti-deadlock: displacement ~0 while mass decays -> kick)
    "STALL_KICK_ON": 0.00068336,         # >0.5 on
    "STALL_TICKS": 10.3279,
    "STALL_DIST": 0.377808,
    "W_STALL_KICK": 0.0684004,
    # ORGAN: endgame posture (protect the bank late; final mass IS the score)
    "ENDGAME_START": 0.437055,         # fraction of EXPECTED_ROUNDS after which endgame fear applies; 1.0=off
    "ENDGAME_FEAR_MULT": 0.896952,     # threat-force mult in endgame; 1.0=neutral
    "EXPECTED_ROUNDS": 1400.0,
    # ORGAN: speed-aware threat pricing (slow giants can't catch us outside envelope)
    "W_ENVELOPE_SCALE": 3.57531e-05,      # 0=off; scales threat force by kill-envelope proximity
    # ORGAN: per-player aggression memory (avoid players who hunt US specifically)
    "AGGRO_ON": 0.00156926,              # >0.5 on

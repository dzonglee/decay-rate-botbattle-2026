"""
split_feaster_v2b - prey-lunge GATED (bust fix). Adds two gates to the prey-lunge
path (should_split) ONLY -- the feast cycle is untouched: (a) LUNGE_MIN_MASS=45 (never
lunge below the viable-fragment floor), (b) SPLIT_THREAT_CLEARANCE 0.0376->10 (never
lunge with a threat within 10u). The prey-lunge at mass>=4 into predators was the bust
cause; the cycle was mechanically correct and stays as-is.
...
CONFIG = {
    ...
    "SPLIT_THREAT_CLEARANCE": 10.0,        # v2b: block prey-lunge if ANY threat within 10u (was 0.0376 = off)
    "LUNGE_MIN_MASS": 45.0,        # v2b: no prey-lunge below the viable-fragment floor (halves stay >~2.9, uneatable)
    "SPLIT_MIN_MASS": 2.0,
...
def should_split(game: Game, info: dict, fx: float, fy: float) -> bool:
    ...
    me = game.state.me
    my_blobs = list(me.blobs.values())
    if sum(mass(b.radius) for b in my_blobs) < CONFIG["LUNGE_MIN_MASS"]:
        return False   # v2b: prey-lunge only once fragments are viable

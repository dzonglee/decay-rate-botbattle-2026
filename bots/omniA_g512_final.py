...
CONFIG = {
    ...
    "W_VIRUS_BIG": 1.94207,
    "W_WALL": 0.000395809,
    "W_REGROUP": 9.31986e-11,
    "FOOD_FALLOFF": 1.49016,
    "PREY_FALLOFF": 0.000299349,
    "THREAT_FALLOFF": 3.93074e-06,
    "THREAT_PANIC_DIST": 3.80367e-07,
    "THREAT_PANIC_MULT": 0.0740021,
    "THREAT_IGNORE_DIST": 27.0727,
    "SPLIT_REACH": 51.3259,
    "W_SPLIT_ZONE": 2.17731e-05,
    "WALL_MARGIN": 6.41084e-06,
    "FRESH_MASS": 1.35422,
    "FRESH_CAUTION": 8.25927e-05,
    "VETO_ENABLED": False,
    "VETO_MARGIN": 0.134814,
    "VETO_HORIZON": 2,
    "VETO_SOFT_MASS": 1.27425,
    "LOCK_ENABLED": False,
    "LOCK_MIN_VALUE": 0.000468376,
    "LOCK_TICKS_MAX": 74.0391,
    "LOCK_ABANDON_T": 13.2681,
    "W_LOCK": 0.00024812,
    "LOCK_THREAT_BREAK": 5.94625e-06,
    "LUNGE_DISC_PANIC": 0.795288,
    "SPLIT_RUN_ENABLED": False,
    "SPLIT_RUN_MAX_MASS": 2.82733e-07,
    "SPLIT_RUN_TRIGGER": 1.60412e-06,
    "EAT_RATIO": 1.2,
    "SAFETY_RATIO": 2,
    "LEAD_TICKS": 1,
    "LUNGE_ALARM_DIST": 6.35076e-05,
    "W_LUNGE_ALARM": 0.0105271,
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 30.0175,
    "SPLIT_SAFETY_RATIO": 1.75699e-05,
    "SPLIT_MAX_BLOBS": 1.29784,
    "SPLIT_THREAT_CLEARANCE": 0.000271124,
    "SPLIT_MIN_MASS": 2.0,
    "SPLIT_MAX_MASS": 173.908,
    "VIRUS_DANGER_MASS_RATIO": 1.2,
    "VIRUS_AVOID_DIST": 0.316616,
    "VIRUS_FEAST_CLEAR": 2.25195,
    # (BOOT_* and W_VIRUS_FOOD amputated 07-09: dead code paths, wasted mutations)
    "FEAST_MIN_MASS": 6.4665,
    "FEAST_SLOT_SAT": 0.325387,
    "W_VIRUS_FEAST": 14.0467,
    ...
    "CYCLE_THREAT_CLEAR": 2.16181,   # only cycle when threats are FAR (avoid deaths-while-fragmented); bounds (2,30)
    ...
    "HUNTER_AVOID_MULT": 0.781028,     # virus-avoid radius mult when hunter near (was 1.5)
    "HUNTER_REPEL_MULT": 2.99068,     # virus repulsion mult when hunter near (was 2.0)
    "WALL_ESC_MARGIN_MULT": 0.611598,  # wall-escape activation margin (was 1.2)
    "WALL_ESC_NUDGE": 0.14926,        # outward nudge strength (was 0.2)
    "LUNGE_DISC_FRAC": 0.931861,       # deep-disc panic fraction (was 0.9)
    "SPLIT_ALIGN_MIN": 0.473251,       # prey-lunge alignment gate (was 0.7)
    "DEATH_COST_SCALE": 37.5,     # bank-size fear scaling divisor (was 25)
    "VIRUS_FEAST_FALLOFF": 1.32878,   # feast attraction falloff exponent (was 1/d)
    "CYCLE_TARGET_BLOBS": 6.60046,   # cycle split ceiling (was 16)
    # ORGAN: corner-siege fix (uncatchable corner-tucked prey -> zero force)
    "CORNER_SKIP_ON": 0.0129486,        # >0.5 on; bounds (0,1)
    "CORNER_TUCK": 0.5,
    "CORNER_MARGIN": 0.0305373,
    # ORGAN: sized threat gate (only count watchers that can actually eat us)
    "THREAT_SIZE_GATE": 0.000935526,      # 0=all threats count (legacy); >0: hunter_near only if threat mass >= ours*gate
    # ORGAN: virus shield (hide behind viruses; big pursuers pop crossing them)
    "W_VIRUS_SHIELD": 0.000282482,        # 0=off; attraction to viruses when fleeing while small
    "SHIELD_MAX_MASS": 2.60574,       # only shield when total below this (we pass under gate)
    # ORGAN: fragment hunting (counter the split-feast meta: eat cyclers mid-cycle)
    "W_FRAG_HUNT": 0.00738783,           # 0=off; prey-force mult vs players at high blob count
    "FRAG_HUNT_MIN_BLOBS": 3.73152,   # enemy blob count to trigger
    # ORGAN: piece guard (post-consumption regroup surge; heals the loss30 wound)
    "W_PIECE_GUARD": 0.000723456,         # 0=off; extra regroup for GUARD_TICKS after our shatter
    "PIECE_GUARD_TICKS": 4.4458,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 4.88441e-05,          # 0=off; regroup when fragmented + no viruses visible
    # ORGAN: map positioning (center vs edge preference; evolution picks sign)
    "W_CENTER": 4.33559e-06,              # 0=off; + seeks center, - seeks edges
    # ORGAN: hunger-scaled food greed (small bots chase pellets harder)
    "FOOD_HUNGER_EXP": 8.83347e-05,       # 0=off; food force *= (1/total_mass)^exp
    # ORGAN: stall kick (anti-deadlock: displacement ~0 while mass decays -> kick)
    "STALL_KICK_ON": 0.000894327,         # >0.5 on
    "STALL_TICKS": 42.7426,
    "STALL_DIST": 0.206439,
    "W_STALL_KICK": 0.452944,
    # ORGAN: endgame posture (protect the bank late; final mass IS the score)
    "ENDGAME_START": 0.727823,         # fraction of EXPECTED_ROUNDS after which endgame fear applies; 1.0=off
    "ENDGAME_FEAR_MULT": 0.548818,     # threat-force mult in endgame; 1.0=neutral
    "EXPECTED_ROUNDS": 1400.0,
    # ORGAN: speed-aware threat pricing (slow giants can't catch us outside envelope)
    "W_ENVELOPE_SCALE": 1.74318e-05,      # 0=off; scales threat force by kill-envelope proximity
    # ORGAN: per-player aggression memory (avoid players who hunt US specifically)
    "AGGRO_ON": 0.000224342,              # >0.5 on
    "W_AGGRO": 3.80522e-06,               # extra threat weight vs measured aggressors
    "AGGRO_DECAY": 0.9,
}

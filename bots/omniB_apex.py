...
CONFIG = {
    ...
    "W_PREY": 7.99711,
    "W_THREAT": 1,
    "W_VIRUS_BIG": 0.0230095,
    "W_WALL": 6.86099e-07,
    "W_REGROUP": 3.6654e-09,
    "FOOD_FALLOFF": 0.961168,
    "PREY_FALLOFF": 0.103332,
    "THREAT_FALLOFF": 1.73802e-07,
    "THREAT_PANIC_DIST": 0.328214,
    "THREAT_PANIC_MULT": 9.314e-05,
    "THREAT_IGNORE_DIST": 24.5883,
    "SPLIT_REACH": 51.3259,
    "W_SPLIT_ZONE": 7.53552e-05,
    "WALL_MARGIN": 1.39646e-06,
    "FRESH_MASS": 1.29656,
    "FRESH_CAUTION": 0.00048191,
    "VETO_ENABLED": False,
    "VETO_MARGIN": 9.89248e-06,
    "VETO_HORIZON": 4.53091,
    "VETO_SOFT_MASS": 0.81,
    "LOCK_ENABLED": False,
    "LOCK_MIN_VALUE": 0.00156796,
    "LOCK_TICKS_MAX": 27.4061,
    "LOCK_ABANDON_T": 100.473,
    "W_LOCK": 0.00174351,
    "LOCK_THREAT_BREAK": 3.8187e-05,
    "LUNGE_DISC_PANIC": 0.795288,
    "SPLIT_RUN_ENABLED": False,
    "SPLIT_RUN_MAX_MASS": 7.40398e-08,
    "SPLIT_RUN_TRIGGER": 0.00470297,
    "EAT_RATIO": 1.2,
    "SAFETY_RATIO": 1.15,
    "LEAD_TICKS": 1.83625,
    "LUNGE_ALARM_DIST": 5.07317e-05,
    "W_LUNGE_ALARM": 0.0339788,
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 21.8398,
    "SPLIT_SAFETY_RATIO": 6.45875e-07,
    "SPLIT_MAX_BLOBS": 1.22991,
    "SPLIT_THREAT_CLEARANCE": 3.82613e-05,
    "SPLIT_MIN_MASS": 2.0,
    "SPLIT_MAX_MASS": 64.7699,
    "VIRUS_DANGER_MASS_RATIO": 1.2,
    "VIRUS_AVOID_DIST": 5.37258,
    "VIRUS_FEAST_CLEAR": 2.60936,
    # (BOOT_* and W_VIRUS_FOOD amputated 07-09: dead code paths, wasted mutations)
    "FEAST_MIN_MASS": 28.3515,
    "FEAST_SLOT_SAT": 0.605058,
    "W_VIRUS_FEAST": 0.0122153,
    # --- SPLIT-FEAST CYCLE ORGAN (Team-15 virus economy; matches 1719/1721/1723) ---
    "SPLIT_CYCLE_ON": 3.39207e-05,        # vetoable master switch (>0.5 = on); bounds (0,1)
    "CYCLE_MIN_MASS": 7.70498,       # only cycle once BIG (don't fragment a small bank); bounds (4,40)
    "CYCLE_THREAT_CLEAR": 22.4434,   # only cycle when threats are FAR (avoid deaths-while-fragmented); bounds (2,30)
    ...
    "HUNTER_REPEL_MULT": 1.76364,     # virus repulsion mult when hunter near (was 2.0)
    "WALL_ESC_MARGIN_MULT": 1.21728,  # wall-escape activation margin (was 1.2)
    "WALL_ESC_NUDGE": 0.10208,        # outward nudge strength (was 0.2)
    "LUNGE_DISC_FRAC": 0.584617,       # deep-disc panic fraction (was 0.9)
    "SPLIT_ALIGN_MIN": 0.65943,       # prey-lunge alignment gate (was 0.7)
    "DEATH_COST_SCALE": 12.76,     # bank-size fear scaling divisor (was 25)
    "VIRUS_FEAST_FALLOFF": 0.817314,   # feast attraction falloff exponent (was 1/d)
    "CYCLE_TARGET_BLOBS": 5.86073,   # cycle split ceiling (was 16)
    # ORGAN: corner-siege fix (uncatchable corner-tucked prey -> zero force)
    "CORNER_SKIP_ON": 0.181088,        # >0.5 on; bounds (0,1)
    "CORNER_TUCK": 2.08534,
    "CORNER_MARGIN": 8.06277e-06,
    # ORGAN: sized threat gate (only count watchers that can actually eat us)
    "THREAT_SIZE_GATE": 0.00177011,      # 0=all threats count (legacy); >0: hunter_near only if threat mass >= ours*gate
    # ORGAN: virus shield (hide behind viruses; big pursuers pop crossing them)
    "W_VIRUS_SHIELD": 0.00273478,        # 0=off; attraction to viruses when fleeing while small
    "SHIELD_MAX_MASS": 2.11605,       # only shield when total below this (we pass under gate)
    # ORGAN: fragment hunting (counter the split-feast meta: eat cyclers mid-cycle)
    "W_FRAG_HUNT": 0.000287298,           # 0=off; prey-force mult vs players at high blob count
    "FRAG_HUNT_MIN_BLOBS": 3.80218,   # enemy blob count to trigger
    # ORGAN: piece guard (post-consumption regroup surge; heals the loss30 wound)
    "W_PIECE_GUARD": 0.00363937,         # 0=off; extra regroup for GUARD_TICKS after our shatter
    "PIECE_GUARD_TICKS": 3.73045,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 7.94013e-07,          # 0=off; regroup when fragmented + no viruses visible
    # ORGAN: map positioning (center vs edge preference; evolution picks sign)
    "W_CENTER": 6.05382e-05,              # 0=off; + seeks center, - seeks edges
    # ORGAN: hunger-scaled food greed (small bots chase pellets harder)
    "FOOD_HUNGER_EXP": 1.03102e-05,       # 0=off; food force *= (1/total_mass)^exp
    # ORGAN: stall kick (anti-deadlock: displacement ~0 while mass decays -> kick)
    "STALL_KICK_ON": 0.000981286,         # >0.5 on
    "STALL_TICKS": 14.239,
    "STALL_DIST": 0.2,
    "W_STALL_KICK": 0.187298,
    # ORGAN: endgame posture (protect the bank late; final mass IS the score)
    "ENDGAME_START": 0.408157,         # fraction of EXPECTED_ROUNDS after which endgame fear applies; 1.0=off
    "ENDGAME_FEAR_MULT": 0.633256,     # threat-force mult in endgame; 1.0=neutral
    "EXPECTED_ROUNDS": 1400.0,
    # ORGAN: speed-aware threat pricing (slow giants can't catch us outside envelope)
    "W_ENVELOPE_SCALE": 0.0961747,      # 0=off; scales threat force by kill-envelope proximity
    # ORGAN: per-player aggression memory (avoid players who hunt US specifically)
    "AGGRO_ON": 1.03968e-06,              # >0.5 on
    "W_AGGRO": 8.163e-06,               # extra threat weight vs measured aggressors
    "AGGRO_DECAY": 0.9,
}

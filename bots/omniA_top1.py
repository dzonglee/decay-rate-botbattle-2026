...
CONFIG = {
    ...
    "W_PREY": 1.94045,
    "W_THREAT": 4.30832,
    "W_VIRUS_BIG": 0.000697234,
    "W_WALL": 1.29543e-06,
    "W_REGROUP": 4.05737e-09,
    "FOOD_FALLOFF": 1.65225,
    "PREY_FALLOFF": 0.0153677,
    "THREAT_FALLOFF": 8.67293e-08,
    "THREAT_PANIC_DIST": 4.56123e-05,
    "THREAT_PANIC_MULT": 4.10837e-05,
    "THREAT_IGNORE_DIST": 48.2971,
    "SPLIT_REACH": 51.3259,
    "W_SPLIT_ZONE": 2.83039e-05,
    "WALL_MARGIN": 6.05123e-08,
    "FRESH_MASS": 2.35123,
    "FRESH_CAUTION": 2.78611e-06,
    "VETO_ENABLED": False,
    "VETO_MARGIN": 7.91288e-07,
    "VETO_HORIZON": 6.92704,
    "VETO_SOFT_MASS": 2.46119,
    "LOCK_ENABLED": False,
    "LOCK_MIN_VALUE": 2.96727e-05,
    "LOCK_TICKS_MAX": 37.0274,
    "LOCK_ABANDON_T": 53.4703,
    "W_LOCK": 0.000474477,
    "LOCK_THREAT_BREAK": 7.84325e-05,
    "LUNGE_DISC_PANIC": 0.795288,
    "SPLIT_RUN_ENABLED": False,
    "SPLIT_RUN_MAX_MASS": 1.23233e-09,
    "SPLIT_RUN_TRIGGER": 0.0238602,
    "EAT_RATIO": 1.2,
    "SAFETY_RATIO": 1.15,
    "LEAD_TICKS": 1.45996,
    "LUNGE_ALARM_DIST": 0.0011505,
    "W_LUNGE_ALARM": 0.0160239,
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 256.472,
    "SPLIT_SAFETY_RATIO": 0.000449067,
    "SPLIT_MAX_BLOBS": 1.35278,
    "SPLIT_THREAT_CLEARANCE": 2.01101e-07,
    "SPLIT_MIN_MASS": 2.0,
    "SPLIT_MAX_MASS": 302.349,
    "VIRUS_DANGER_MASS_RATIO": 1.2,
    "VIRUS_AVOID_DIST": 0.830721,
    "VIRUS_FEAST_CLEAR": 3.57992,
    # (BOOT_* and W_VIRUS_FOOD amputated 07-09: dead code paths, wasted mutations)
    "FEAST_MIN_MASS": 7.41605,
    "FEAST_SLOT_SAT": 0.529346,
    "W_VIRUS_FEAST": 0.294007,
    # --- SPLIT-FEAST CYCLE ORGAN (Team-15 virus economy; matches 1719/1721/1723) ---
    "SPLIT_CYCLE_ON": 0.575354,        # vetoable master switch (>0.5 = on); bounds (0,1)
    "CYCLE_MIN_MASS": 45.8195,       # only cycle once BIG (don't fragment a small bank); bounds (4,40)
    "CYCLE_THREAT_CLEAR": 3.36084,   # only cycle when threats are FAR (avoid deaths-while-fragmented); bounds (2,30)
    ...
    "HUNTER_REPEL_MULT": 1.59152,     # virus repulsion mult when hunter near (was 2.0)
    "WALL_ESC_MARGIN_MULT": 1.3874,  # wall-escape activation margin (was 1.2)
    "WALL_ESC_NUDGE": 0.1,        # outward nudge strength (was 0.2)
    "LUNGE_DISC_FRAC": 0.45,       # deep-disc panic fraction (was 0.9)
    "SPLIT_ALIGN_MIN": 0.414679,       # prey-lunge alignment gate (was 0.7)
    "DEATH_COST_SCALE": 12.5,     # bank-size fear scaling divisor (was 25)
    "VIRUS_FEAST_FALLOFF": 0.903141,   # feast attraction falloff exponent (was 1/d)
    "CYCLE_TARGET_BLOBS": 4,   # cycle split ceiling (was 16)
    # ORGAN: corner-siege fix (uncatchable corner-tucked prey -> zero force)
    "CORNER_SKIP_ON": 0.141058,        # >0.5 on; bounds (0,1)
    "CORNER_TUCK": 1.53186,
    "CORNER_MARGIN": 0.0475545,
    # ORGAN: sized threat gate (only count watchers that can actually eat us)
    "THREAT_SIZE_GATE": 0.062196,      # 0=all threats count (legacy); >0: hunter_near only if threat mass >= ours*gate
    # ORGAN: virus shield (hide behind viruses; big pursuers pop crossing them)
    "W_VIRUS_SHIELD": 0.126136,        # 0=off; attraction to viruses when fleeing while small
    "SHIELD_MAX_MASS": 4.13267,       # only shield when total below this (we pass under gate)
    # ORGAN: fragment hunting (counter the split-feast meta: eat cyclers mid-cycle)
    "W_FRAG_HUNT": 0.5135,           # 0=off; prey-force mult vs players at high blob count
    "FRAG_HUNT_MIN_BLOBS": 6.14281,   # enemy blob count to trigger
    # ORGAN: piece guard (post-consumption regroup surge; heals the loss30 wound)
    "W_PIECE_GUARD": 0.166486,         # 0=off; extra regroup for GUARD_TICKS after our shatter
    "PIECE_GUARD_TICKS": 2,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 0.14001,          # 0=off; regroup when fragmented + no viruses visible
    # ORGAN: map positioning (center vs edge preference; evolution picks sign)
    "W_CENTER": 5.4806e-05,              # 0=off; + seeks center, - seeks edges
    # ORGAN: hunger-scaled food greed (small bots chase pellets harder)
    "FOOD_HUNGER_EXP": 0.101033,       # 0=off; food force *= (1/total_mass)^exp
    # ORGAN: stall kick (anti-deadlock: displacement ~0 while mass decays -> kick)
    "STALL_KICK_ON": 0.135506,         # >0.5 on
    "STALL_TICKS": 12.6219,
    "STALL_DIST": 0.688036,
    "W_STALL_KICK": 0.147473,
    # ORGAN: endgame posture (protect the bank late; final mass IS the score)
    "ENDGAME_START": 0.902375,         # fraction of EXPECTED_ROUNDS after which endgame fear applies; 1.0=off
    "ENDGAME_FEAR_MULT": 1.70763,     # threat-force mult in endgame; 1.0=neutral
    "EXPECTED_ROUNDS": 1400.0,
    # ORGAN: speed-aware threat pricing (slow giants can't catch us outside envelope)
    "W_ENVELOPE_SCALE": 0.125042,      # 0=off; scales threat force by kill-envelope proximity
    # ORGAN: per-player aggression memory (avoid players who hunt US specifically)
    "AGGRO_ON": 0.110471,              # >0.5 on
    "W_AGGRO": 0.322792,               # extra threat weight vs measured aggressors
    ...
    "W_WEALTH_FEAR": 0.0911555,         # 0=off; threat force mult grows with own mass above WEALTH_START
    "WEALTH_START": 22.1466,         # own total mass where wealth fear begins
    "WEALTH_EXP": 0.377269,            # curve shape
    ...
    "CAMP_WINDOW_LO": 6.88541,       # rounds since consumption when attraction starts
    "CAMP_WINDOW_HI": 60,       # ... and ends (site pruned)
    "CAMP_MAX_MASS": 59.1879,        # too rich to camp (banking dominates)
    # ORGAN W3: GRUDGE MEMORY — per-opponent caution after they cost us mass
    "W_GRUDGE": 0.178892,              # 0=off; extra threat weight vs players who ate our blobs
    "GRUDGE_DECAY": 0.9995,        # per-tick decay
    # ORGAN W4: RANK POSTURE — visible-field standings condition greed/fear
    "W_RANK_GUARD": 0.064863,          # 0=off; threat mult when leading the visible field
    "W_RANK_AGGRO": 0.947692,          # 0=off; prey mult when trailing the visible field
    # ORGAN W5: VIRUS SLOT TIMING — pieces=max(1,17-blobs): saturated consumption is free mass
    "VIRUS_SLOT_EXP": 0.0966353,        # 0=off; feast force *= (blobs/16)^exp
    # ===== OMNI-2.1 PROFILER (in-body stupid/mid/elite classifier; Chris's order 2026-07-10) =====
    "PROF_ON": 0.142433,                 # MASTER GATE: <=0.5 -> entire profiler off (zero compute, zero state)
    "PROF_ELITE_T": 0.85,           # EMA score above -> ELITE
    "PROF_STUPID_T": 0.355539,          # EMA score below -> STUPID
    "PROF_RADIUS": 11.8446,            # elite-proximity radius gating feast boldness
    "PROF_PREY_STUPID": 0.195265,        # extra prey force vs STUPID targets
    "PROF_PREY_ELITE_DISC": 0.18945,    # prey force discount vs ELITE targets (0..1)
    "PROF_THREAT_STUPID_DISC": 0.141797, # threat force discount for STUPID giants (0..1)
    "PROF_THREAT_ELITE_MULT": 0.363921,  # extra threat force vs proven-ELITE hunters
    "PROF_FEAST_BOLD": 0.192908,         # virus-feast/cycle boldness when no ELITE within PROF_RADIUS
    # ===== OMNI-2.1 corner refuge (gate = weight > 0) =====
    "W_CORNER_REFUGE": 1.02175,         # when hunted by an uncatchable-for-us giant: tuck into ITS unreachable corner
    "AGGRO_DECAY": 0.9,
}

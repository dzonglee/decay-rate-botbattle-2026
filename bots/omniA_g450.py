...
CONFIG = {
    ...
    "W_VIRUS_BIG": 4.96888,
    "W_WALL": 0.000572903,
    "W_REGROUP": 5.96984e-11,
    "FOOD_FALLOFF": 4.0326,
    "PREY_FALLOFF": 0.000467861,
    "THREAT_FALLOFF": 8.50869e-06,
    "THREAT_PANIC_DIST": 1.24878e-06,
    "THREAT_PANIC_MULT": 0.172503,
    "THREAT_IGNORE_DIST": 21.4413,
    "SPLIT_REACH": 51.3259,
    "W_SPLIT_ZONE": 0.000529739,
    "WALL_MARGIN": 6.09428e-06,
    "FRESH_MASS": 1.15958,
    "FRESH_CAUTION": 4.31759e-05,
    "VETO_ENABLED": False,
    "VETO_MARGIN": 0.16309,
    "VETO_HORIZON": 2.53763,
    "VETO_SOFT_MASS": 1.25586,
    "LOCK_ENABLED": False,
    "LOCK_MIN_VALUE": 0.00132042,
    "LOCK_TICKS_MAX": 29.1621,
    "LOCK_ABANDON_T": 31.2188,
    "W_LOCK": 0.0004845,
    "LOCK_THREAT_BREAK": 2.9378e-05,
    "LUNGE_DISC_PANIC": 0.795288,
    "SPLIT_RUN_ENABLED": False,
    "SPLIT_RUN_MAX_MASS": 1.057e-07,
    "SPLIT_RUN_TRIGGER": 5.73297e-06,
    "EAT_RATIO": 1.2,
    "SAFETY_RATIO": 1.25168,
    "LEAD_TICKS": 1.61875,
    "LUNGE_ALARM_DIST": 6.18758e-05,
    "W_LUNGE_ALARM": 0.0610125,
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 22.1703,
    "SPLIT_SAFETY_RATIO": 8.54383e-05,
    "SPLIT_MAX_BLOBS": 1.14153,
    "SPLIT_THREAT_CLEARANCE": 0.00499738,
    "SPLIT_MIN_MASS": 2.0,
    "SPLIT_MAX_MASS": 240.075,
    "VIRUS_DANGER_MASS_RATIO": 1.2,
    "VIRUS_AVOID_DIST": 0.3,
    "VIRUS_FEAST_CLEAR": 19.4124,
    # (BOOT_* and W_VIRUS_FOOD amputated 07-09: dead code paths, wasted mutations)
    "FEAST_MIN_MASS": 5.84937,
    "FEAST_SLOT_SAT": 0.438802,
    "W_VIRUS_FEAST": 17.5242,
    ...
    "CYCLE_THREAT_CLEAR": 5.50246,   # only cycle when threats are FAR (avoid deaths-while-fragmented); bounds (2,30)
    ...
    "HUNTER_REPEL_MULT": 1.85572,     # virus repulsion mult when hunter near (was 2.0)
    "WALL_ESC_MARGIN_MULT": 0.6,  # wall-escape activation margin (was 1.2)
    "WALL_ESC_NUDGE": 0.17098,        # outward nudge strength (was 0.2)
    "LUNGE_DISC_FRAC": 0.860039,       # deep-disc panic fraction (was 0.9)
    "SPLIT_ALIGN_MIN": 0.439779,       # prey-lunge alignment gate (was 0.7)
    "DEATH_COST_SCALE": 25.0391,     # bank-size fear scaling divisor (was 25)
    ...
    "CORNER_SKIP_ON": 0.0478844,        # >0.5 on; bounds (0,1)
    "CORNER_TUCK": 1.20381,
    "CORNER_MARGIN": 0.145235,
    # ORGAN: sized threat gate (only count watchers that can actually eat us)
    "THREAT_SIZE_GATE": 0.000260098,      # 0=all threats count (legacy); >0: hunter_near only if threat mass >= ours*gate
    # ORGAN: virus shield (hide behind viruses; big pursuers pop crossing them)
    "W_VIRUS_SHIELD": 0.000200187,        # 0=off; attraction to viruses when fleeing while small
    ...
    "W_FRAG_HUNT": 0.00997001,           # 0=off; prey-force mult vs players at high blob count
    "FRAG_HUNT_MIN_BLOBS": 2.16369,   # enemy blob count to trigger
    # ORGAN: piece guard (post-consumption regroup surge; heals the loss30 wound)
    "W_PIECE_GUARD": 0.00240583,         # 0=off; extra regroup for GUARD_TICKS after our shatter
    "PIECE_GUARD_TICKS": 14.1801,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 0.000181853,          # 0=off; regroup when fragmented + no viruses visible
    # ORGAN: map positioning (center vs edge preference; evolution picks sign)
    "W_CENTER": 1.9934e-05,              # 0=off; + seeks center, - seeks edges
    # ORGAN: hunger-scaled food greed (small bots chase pellets harder)
    "FOOD_HUNGER_EXP": 0.000236288,       # 0=off; food force *= (1/total_mass)^exp
    # ORGAN: stall kick (anti-deadlock: displacement ~0 while mass decays -> kick)
    "STALL_KICK_ON": 0.000472825,         # >0.5 on
    "STALL_TICKS": 16.6547,
    "STALL_DIST": 0.41203,
    "W_STALL_KICK": 0.251249,
    # ORGAN: endgame posture (protect the bank late; final mass IS the score)
    "ENDGAME_START": 0.481086,         # fraction of EXPECTED_ROUNDS after which endgame fear applies; 1.0=off
    "ENDGAME_FEAR_MULT": 0.74627,     # threat-force mult in endgame; 1.0=neutral
    "EXPECTED_ROUNDS": 1400.0,
    # ORGAN: speed-aware threat pricing (slow giants can't catch us outside envelope)
    "W_ENVELOPE_SCALE": 3.0643e-05,      # 0=off; scales threat force by kill-envelope proximity
    # ORGAN: per-player aggression memory (avoid players who hunt US specifically)
    "AGGRO_ON": 0.00102792,              # >0.5 on
    "W_AGGRO": 3.7564e-05,               # extra threat weight vs measured aggressors
    "AGGRO_DECAY": 0.9,
}

...
CONFIG = {
    ...
    "W_PREY": 1.97544,
    "W_THREAT": 2.10698,
    "W_VIRUS_BIG": 0.174023,
    "W_WALL": 0.000156962,
    "W_REGROUP": 1.15427e-09,
    "FOOD_FALLOFF": 1.21319,
    "PREY_FALLOFF": 0.00291435,
    "THREAT_FALLOFF": 3.84884e-07,
    "THREAT_PANIC_DIST": 0.0351916,
    "THREAT_PANIC_MULT": 0.00890238,
    "THREAT_IGNORE_DIST": 35.9308,
    "SPLIT_REACH": 0.747983,
    "W_SPLIT_ZONE": 4.52698e-06,
    "WALL_MARGIN": 4.47497e-07,
    "FRESH_MASS": 1.79773,
    "FRESH_CAUTION": 8.05687e-07,
    "VETO_ENABLED": False,
    "VETO_MARGIN": 1.20082e-07,
    "VETO_HORIZON": 2.6319,
    "VETO_SOFT_MASS": 0.910041,
    "LOCK_ENABLED": False,
    "LOCK_MIN_VALUE": 1.0036e-05,
    "LOCK_TICKS_MAX": 27.2651,
    "LOCK_ABANDON_T": 10,
    "W_LOCK": 0.00111352,
    "LOCK_THREAT_BREAK": 0.000134086,
    "LUNGE_DISC_PANIC": 0.014742,
    "SPLIT_RUN_ENABLED": False,
    "SPLIT_RUN_MAX_MASS": 2.64615e-07,
    "SPLIT_RUN_TRIGGER": 0.00211148,
    "EAT_RATIO": 1.28088,
    "SAFETY_RATIO": 1.69584,
    "LEAD_TICKS": 1.40564,
    "LUNGE_ALARM_DIST": 3.7615e-07,
    "W_LUNGE_ALARM": 0.00112056,
    "SPLIT_ENABLED": True,
    "SPLIT_MAX_RANGE": 27.6544,
    "SPLIT_SAFETY_RATIO": 1.12441e-05,
    "SPLIT_MAX_BLOBS": 1.7799,
    "SPLIT_THREAT_CLEARANCE": 0.000143216,
    "SPLIT_MIN_MASS": 7.45903,
    "SPLIT_MAX_MASS": 102.164,
    "VIRUS_DANGER_MASS_RATIO": 8.86417,
    "VIRUS_AVOID_DIST": 2.13379,
    "VIRUS_FEAST_CLEAR": 4.05927,
    # (BOOT_* and W_VIRUS_FOOD amputated 07-09: dead code paths, wasted mutations)
    "FEAST_MIN_MASS": 5.43894,
    "FEAST_SLOT_SAT": 1.47537,
    "W_VIRUS_FEAST": 0.00296821,
    ...
    "CYCLE_THREAT_CLEAR": 3.70838,   # only cycle when threats are FAR (avoid deaths-while-fragmented); bounds (2,30)
    ...
    "HUNTER_AVOID_MULT": 0.949981,     # virus-avoid radius mult when hunter near (was 1.5)
    "HUNTER_REPEL_MULT": 2.39955,     # virus repulsion mult when hunter near (was 2.0)
    "WALL_ESC_MARGIN_MULT": 0.701622,  # wall-escape activation margin (was 1.2)
    "WALL_ESC_NUDGE": 0.1,        # outward nudge strength (was 0.2)
    "LUNGE_DISC_FRAC": 0.45,       # deep-disc panic fraction (was 0.9)
    "SPLIT_ALIGN_MIN": 0.387619,       # prey-lunge alignment gate (was 0.7)
    "DEATH_COST_SCALE": 17.1708,     # bank-size fear scaling divisor (was 25)
    "VIRUS_FEAST_FALLOFF": 0.742853,   # feast attraction falloff exponent (was 1/d)
    "CYCLE_TARGET_BLOBS": 6.91386,   # cycle split ceiling (was 16)
    # ORGAN: corner-siege fix (uncatchable corner-tucked prey -> zero force)
    "CORNER_SKIP_ON": 0.00297666,        # >0.5 on; bounds (0,1)
    "CORNER_TUCK": 2.2003,
    "CORNER_MARGIN": 4.99064e-06,
    # ORGAN: sized threat gate (only count watchers that can actually eat us)
    "THREAT_SIZE_GATE": 0.000298092,      # 0=all threats count (legacy); >0: hunter_near only if threat mass >= ours*gate
    # ORGAN: virus shield (hide behind viruses; big pursuers pop crossing them)
    "W_VIRUS_SHIELD": 0.00415721,        # 0=off; attraction to viruses when fleeing while small
    "SHIELD_MAX_MASS": 6.095,       # only shield when total below this (we pass under gate)
    # ORGAN: fragment hunting (counter the split-feast meta: eat cyclers mid-cycle)
    "W_FRAG_HUNT": 6.54792e-05,           # 0=off; prey-force mult vs players at high blob count
    "FRAG_HUNT_MIN_BLOBS": 2,   # enemy blob count to trigger
    # ORGAN: piece guard (post-consumption regroup surge; heals the loss30 wound)
    "W_PIECE_GUARD": 5.1185e-05,         # 0=off; extra regroup for GUARD_TICKS after our shatter
    "PIECE_GUARD_TICKS": 4.62529,
    # ORGAN: idle merge drive (cycle regroup half: pull together when no virus work)
    "W_MERGE_IDLE": 0.0135172,          # 0=off; regroup when fragmented + no viruses visible
    # ORGAN: map positioning (center vs edge preference; evolution picks sign)
    "W_CENTER": 4.46184e-05,              # 0=off; + seeks center, - seeks edges
    # ORGAN: hunger-scaled food greed (small bots chase pellets harder)
    "FOOD_HUNGER_EXP": 7.43682e-06,       # 0=off; food force *= (1/total_mass)^exp
    # ORGAN: stall kick (anti-deadlock: displacement ~0 while mass decays -> kick)
    "STALL_KICK_ON": 0.00467546,         # >0.5 on
    "STALL_TICKS": 10,
    "STALL_DIST": 0.2,
    "W_STALL_KICK": 0.00274595,
    ...
    "ENDGAME_FEAR_MULT": 1.11443,     # threat-force mult in endgame; 1.0=neutral
    "EXPECTED_ROUNDS": 829.435,
    # ORGAN: speed-aware threat pricing (slow giants can't catch us outside envelope)
    "W_ENVELOPE_SCALE": 0.0152235,      # 0=off; scales threat force by kill-envelope proximity
    # ORGAN: per-player aggression memory (avoid players who hunt US specifically)
    "AGGRO_ON": 1.32088e-06,              # >0.5 on
    "W_AGGRO": 1.33674e-06,               # extra threat weight vs measured aggressors
    "AGGRO_DECAY": 0.9,
}
